...
Pure-numpy forward mirrors EarlyPoolEncoder (streaming_encoders.py).
Optionally cross-checks against PyTorch EarlyPoolEncoder when available.
...
def torch_cross_check(shapes: B3Shapes, calib: np.ndarray, weights: B3Weights, stages: Dict[str, np.ndarray]) -> None:
    ...
    # Import EarlyPoolEncoder from experiment package when available
...
def main() -> int:
    ...
    parser.add_argument("--out", type=str, default="hw_b3/runs/default_sw")
