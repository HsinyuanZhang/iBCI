#!/usr/bin/env python3
"""CPU-only preflight for the fold-0 native-M1 source decoder.

It constructs only the three permitted held-in-calib source sessions.  The
outer target, minival, held-out, formal, EvalAI, and model/GPU paths are never
opened.  The output is a non-overwritable receipt for the later decoder run.
"""
...
from src.data.falcon_m1_source_only_decoder_datamodule import (  # noqa: E402
    FOLD0_SOURCES, FOLD0_TARGET, M1SourceOnlyDecoderFold0DataModule,
)


OUT = ROOT / "sua_exploration" / "results" / "m1_afc4_source_decoder_fold0_preflight_v2"
...
def build() -> dict[str, object]:
    data = M1SourceOnlyDecoderFold0DataModule(
        task="m1", data_dir=DATA, source_session_names=list(FOLD0_SOURCES),
        heldin_session_names=list(FOLD0_SOURCES), batch_size=32, window_size=100,
        ...
        loso_fold=None, rotation_id=0, include_heldout_in_fit=False,
        include_heldout_in_test=False, query_start_trial=0, heldin_query_start_trial=0,
        heldin_query_end_trial=None, allow_empty_heldout_query=False, num_workers=0,
    ...
    if not set(batch_sessions).issubset(set(FOLD0_SOURCES)):
    ...
    if manifest["outer_left_out"] != FOLD0_TARGET or tuple(manifest["train_sessions"]) != FOLD0_SOURCES:
        raise RuntimeError("fold-0 source manifest mismatch")
    ...
    return {
        "schema": "m1_afc4_source_only_decoder_fold0_preflight_v2",
