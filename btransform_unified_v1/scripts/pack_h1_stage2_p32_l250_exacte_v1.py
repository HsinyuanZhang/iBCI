#!/usr/bin/env python3
"""Pack H1 stage-2 P32 L=250 into an exact-E Docker image.

New dest only. Does not touch the P16 e24 dest or overwrite its candidate.
Does not register.
"""
...
DEST = WORKSPACE_ROOT / "tfpd_exploration/submissions/evalai_h1_projadd_p32_l250_exacte_v1"
E24_DEST = WORKSPACE_ROOT / "tfpd_exploration/submissions/evalai_h1_projadd_exacte_v1"
WINNER_PATH = (
    PACKAGE_ROOT
    / "results/h1_stage2_full13/pair_20260906T133535Z/overnight/h1_l250_p32_winner.json"
)
...
def main() -> dict:
    ...
    plan.require(window == 250, f"this packer is L=250 only, got {window}")
    ...
    payload_name = "h1_projadd_p32_stage2_s42_ema_e24_L250.pkl"
    ...
    payload = {
        ...
        "metadata": {
            "kind": "proj_add",
            "cell": "H1-STAGE2-L250-P32-PROJ-ADD",
            "seed": 42,
            "epoch": 24,
            "view": "EMA",
            "proj_dim": PROJ_DIM,
    ...
    (DEST / "artifacts" / "payload.receipt.json").write_text(
        json.dumps(
            {
                "schema": "h1_projadd_p32_stage2_exacte_payload_v1",
    ...
    method_name = "H1 proj_add P32 stage-2 EMA e24 L250 exact-E w0"
    method_label = (
        "H1 proj_add P32 CausalPE4 stage-2 full-13 retrain (seed42 EMA e24 L=250) "
    ...
    method_description = (
        ...
        "stage-2 full-13 retrain, seed 42, endpoint24 EMA, L=250. CAL-1 deploy M3 "
        "(budget=3). Official HO unread; minival13 EMA eq 0.614 is diagnostic after "
        "the all-13 retrain. Exact-E, dataloader_workers=0."
    )
    budget_disclosure = (
    ...
    image_tag = f"spint-t4-h1:projadd-p32-s42-ema-e24-L250-w0-{payload_sha[:8]}"
    (DEST / "Dockerfile").write_text(
        ...
        'LABEL ai.eval.candidate="h1_projadd_p32_stage2_s42_ema_e24_L250_w0"\n'
    ...
    candidate = {
        "arm": "h1_projadd_p32_stage2_s42_ema_e24_L250_w0",
        "image_id": image_id,
        "image_tag": image_tag,
        "image_size": int(size),
        ...
        "hold_reason": "packed ahead of 08:00 +0800 quota reset; do not overwrite P16 e24",
    }
