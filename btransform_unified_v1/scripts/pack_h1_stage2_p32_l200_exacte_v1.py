#!/usr/bin/env python3
"""Pack H1 stage-2 P32 L=200 into an exact-E Docker image.

New dest only. Does not touch the P16 e24 dest or the L=250 P32 dest.
Does not register.
"""
...
DEST = WORKSPACE_ROOT / "tfpd_exploration/submissions/evalai_h1_projadd_p32_l200_exacte_v1"
E24_DEST = WORKSPACE_ROOT / "tfpd_exploration/submissions/evalai_h1_projadd_exacte_v1"
L250_DEST = WORKSPACE_ROOT / "tfpd_exploration/submissions/evalai_h1_projadd_p32_l250_exacte_v1"
WINNER_PATH = (
    PACKAGE_ROOT
    / "results/h1_stage2_full13/pair_20260906T133535Z/overnight/h1_l200_p32_winner.json"
)
...
def main() -> dict:
    ...
    plan.require(DEST.resolve() != E24_DEST.resolve(), "refusing to pack into the P16 e24 dest")
    plan.require(DEST.resolve() != L250_DEST.resolve(), "refusing to pack into the L=250 P32 dest")
    plan.require((DEST / "h1_trf_falcon_decoder.py").is_file(), "missing P32 Falcon decoder")
    plan.require((DEST / "decode.py").is_file(), "missing decode.py")
    ...
    plan.require(int(arm.get("proj_dim", -1)) == PROJ_DIM, "winner is not P32")
    window = int(arm["window"])
    plan.require(window == 200, f"this packer is L=200 only, got {window}")
    ...
    payload_name = "h1_projadd_p32_stage2_s42_ema_e24_L200.pkl"
    ...
    payload = {
        ...
        "metadata": {
            "kind": "proj_add",
            "cell": "H1-STAGE2-L200-P32-PROJ-ADD",
            "seed": 42,
            "epoch": 24,
            "view": "EMA",
            "proj_dim": PROJ_DIM,
    ...
    plan.require(int(decoder.decoder.cfg.token_in) == TOKEN_IN, "runtime token_in is not 36")
    plan.require(int(decoder.decoder.cfg.proj_out) == PROJ_DIM, "runtime proj_out is not 32")
    ...
    (DEST / "artifacts" / "payload.receipt.json").write_text(
        json.dumps(
            {
                "schema": "h1_projadd_p32_l200_stage2_exacte_payload_v1",
    ...
    method_name = "H1 proj_add P32 stage-2 EMA e24 L200 exact-E w0"
    method_label = (
        "H1 proj_add P32 CausalPE4 stage-2 full-13 retrain (seed42 EMA e24 L=200) "
        "exact-E w0; not SPINT; not a C2 identity swap; not the P16 e24 image"
    )
    method_description = (
        "H1 proj_add P32 (Linear 700->32, token_in=36, two-group local+P) CausalPE4 "
        "stage-2 full-13 retrain, seed 42, endpoint24 EMA, L=200. CAL-1 deploy M3 "
        "(budget=3). Official HO unread; minival13 EMA eq 0.593 is diagnostic after "
        "the all-13 retrain. Exam 01-20 0.435 vs L250 P32 0.389. Exact-E, dataloader_workers=0."
    )
    budget_disclosure = (
        "H1 CAL-1 deploy M3 (budget=3); C2 e15 frozen materializer; static banks; no TTA. "
        "Checkpoint is preregistered endpoint24 EMA. Local minival is diagnostic only."
    )
    image_tag = f"spint-t4-h1:projadd-p32-s42-ema-e24-L200-w0-{payload_sha[:8]}"
    (DEST / "Dockerfile").write_text(
        ...
        'LABEL ai.eval.token_in="36"\n'
        'LABEL ai.eval.candidate="h1_projadd_p32_stage2_s42_ema_e24_L200_w0"\n'
    ...
    candidate = {
        "arm": "h1_projadd_p32_stage2_s42_ema_e24_L200_w0",
        "image_id": image_id,
        "image_tag": image_tag,
        "image_size": int(size),
        ...
        "hold_reason": "user asked to pack L=200 P32; do not overwrite P16 e24 or L=250 P32",
    }
