#!/usr/bin/env python3
"""Score sealed M2 proj_add EMA checkpoints on the official ext6 face.

Surface = the same six locally-visible held-out-calib query banks that
picked 581973 (query_start_trial=0). Cache is read-only. Hidden/test NWB
are not opened. EvalAI is not contacted.

Pick rule (locked before reading numbers; same as 581973 / MOVE-T4):
...
CKPT_ROOT = PACKAGE_ROOT / "results/m2_projadd/20260906_104300"
...
def main() -> dict[str, Any]:
    ...
    model = BTransformerUnifiedDecoderIdentity(GEOMETRY, seed=SEED, identity_mode="proj_add").to(device)
    ...
    selected = _pick(rows)
    payload = {
        "schema": "btransform_unified_v1_m2_projadd_ext6_epoch_pick_v1",
        "cell": "M2-PROJADD-V1",
