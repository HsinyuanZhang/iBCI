#!/usr/bin/env python3
"""Start M1 concat D4 train only on a demonstrably idle GPU.

Does not kill or share with RIFT / other compute. Two idle samples 12s apart.
After train: score + ext6 pick e18:24. Does not pack or EvalAI-submit.
"""
...
SCR = ROOT / "btransform_unified_v1/scripts/m1_concat_eort_series.py"
PY = Path("/home/xinyuan/miniconda3/envs/spint/bin/python")
IDLE_MEM = 600
IDLE_UTIL = 5
SAMPLE_S = 12.0
POLL_S = 30.0
ENV_FLAG = "BTRANSFORM_M1_CONCAT_EORT_TRAIN"
...
def _two_sample() -> dict | None:
    ...
    if not shared:
        return None
    # Prefer GPU1 when both are idle (standing rule: do not take GPU0 unless needed).
...
def main() -> int:
    if len(sys.argv) != 2:
        print("usage: launch_m1_concat_eort_when_free.py <root>", file=sys.stderr)
        return 2
