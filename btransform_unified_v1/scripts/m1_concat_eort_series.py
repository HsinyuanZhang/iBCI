#!/usr/bin/env python3
"""M1 concat D4 fullsession cell for the BT-EORT series.

Identity: token = cat(local16, E0 100, T4), token_in=120. NOT proj_add.
NOT SPINT. Does not overwrite P16 d2, P32 depth, or CONCAT_W100 dests.

Face: stage-2 ALL 4 held-in. Recipe: seed 42, AdamW 1e-4, EMA 0.9995, 24 ep.
Pick: ext6 e18:24 on visible HO-calib trio. register:false. No EvalAI here.

BT-EORT pack is a later stage after pick (see pack_m1_concat_eort_v1.py).
Spoken series name is BT-EORT; path/env literals keep historical *_eort*.
"""
...
TOKEN_IN = mp.M1_TOKEN_IN_CONCAT
TRAIN_DEPTH = 4
TRAIN_ENV_FLAG = "BTRANSFORM_M1_CONCAT_EORT_TRAIN"

d2.TRAIN_ENV_FLAG = TRAIN_ENV_FLAG
d2.EXPECTED_PARAMS_BY_DEPTH = {4: int(mp.M1_CONCAT_PARAM_COUNT)}
d2.DEPTH_CELLS = (4,)
d2.DEPTH_DIR = {4: "depth4"}


def cell_id(depth: int) -> str:
    if int(depth) != TRAIN_DEPTH:
        raise ValueError(f"M1 concat E-ORT skeleton is D4 only, got {depth}")
    return "M1-CONCAT-D4"
...
def _build_model(depth: int) -> BTransformerUnifiedDecoderIdentity:
    if int(depth) != TRAIN_DEPTH:
        raise ValueError(f"M1 concat E-ORT skeleton is D4 only, got {depth}")
    model = mp.build_m1_concat_model(seed=d2.SEED)
    ...
    plan.require(model.init_meta["temporal_layers"] == N_LAYERS, "temporal depth drift")
    plan.require(len(model.temporal.blocks) == TRAIN_DEPTH, "temporal block count drift")
    n_params = int(sum(p.numel() for p in model.parameters()))
    plan.require(
        n_params == int(mp.M1_CONCAT_PARAM_COUNT),
        f"param count {n_params} != concat {mp.M1_CONCAT_PARAM_COUNT}",
    )
    return model
...
def run_probe(root: Path, dest: Path, face: str) -> dict:
    """Fullsession inventory without proj_add P-zero / fold checks."""
    import numpy as np
    import torch

    if face != "fullsession":
        raise plan.BTransformerUnifiedError("M1 concat E-ORT probe is fullsession only")
    t0 = time.monotonic()
    ...
    inventory = {
        "schema": "btransform_unified_v1_m1_concat_eort_session_inventory",
        "route": "M1 concat D4 E-ORT skeleton 2026-09-07",
        "face": face,
        "utc": d2.utc_iso(),
        "train_protocol": "stage-2 full-session 4 held-in; identity_mode=concat token_in=120",
        ...
        "depth_cells": {
            "4": {
                ...
                "expected_decoder_params": int(mp.M1_CONCAT_PARAM_COUNT),
            }
        },
        "banks": bank_report,
        ...
        "self_checks": {
            ...
            "causal_check": causal,
            "proj_add_degeneration": "skipped (concat has no e0_proj)",
        },
    ...
    print(
        f"[m1-concat-eort] probe: windows={inventory['total_train_windows']} "
        f"upd/ep={updates_per_epoch} params={n_params}",
        flush=True,
    )
    return inventory
...
def main() -> int:
    parser = argparse.ArgumentParser(description="M1 concat D4 E-ORT series cell")
    ...
    print(
        f"[m1-concat-eort] cell={cell_id(args.depth)} stage={args.stage} face={args.face} "
        f"root={root} dest={dest} gpu={args.cuda_visible} token_in={TOKEN_IN}",
        flush=True,
    )
    ...
    if gpu_stage or args.stage == "preflight":
        ...
        print(f"[m1-concat-eort] preflight ok={pre['ok']} foreign={pre['foreign_pids_on_target']}", flush=True)
        if gpu_stage and not pre["ok"]:
            print(f"[m1-concat-eort] BLOCKED: GPU{args.cuda_visible} not clean", file=sys.stderr)
            return 2
    ...
    if args.stage in ("train", "all"):
        ...
        print(f"[m1-concat-eort] train done in {summary['elapsed_s']:.0f}s", flush=True)
