"""Guarded EvalAI push/register for the M2 proj_add P32 exact-E pick.

Use /usr/bin/python3 with the user EvalAI site-packages on sys.path.
POST only when active submissions < 3. On POST timeout, GET the team/phase
list before any retry. No frozen-SPINT checkpoint SHA label — this image
is a new proj_add P32 payload, not the 25d7bc72 mainline.
"""
...
def _register(candidate: dict[str, Any], state: dict[str, object], runtime: tuple[Any, Any, Any, str, str]) -> dict[str, object]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="m2_trf_evalai_") as tmp:
