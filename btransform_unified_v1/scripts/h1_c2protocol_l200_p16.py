"""H1 L=200 P16, C2 protocol: 32 ep, no mid-train val, M7/M5/M4/M3, HO-M3 pick.

Train set = C2 all-source 13 held-in sessions. GPU1 only. Not SPINT.
Does not overwrite historical stage-2 / e24 roots.
"""
...
from src.data.h1_m4_eb_pilot import index_heldin_calib, interpolate_trial_identity, load_record  # noqa: E402

TRAIN_ENV_FLAG = "BTRANSFORM_H1_C2PROTOCOL_TRAIN"
WINDOW = 200
PROJ_DIM = 16
EPOCHS = 32
SEED = plan.SEED
...
def build_cal1_banks(train_banks: dict[str, TaskBank]) -> dict[str, dict[int, TaskBank]]:
    paths = index_heldin_calib(DATA_ROOT)
    out: dict[str, dict[int, TaskBank]] = {}
    for session, base in train_banks.items():
        record = load_record(paths[session])
        plan.require(len(record.trial_values) >= 7, f"{session}: need >=7 calib trials for M7")
        _, payload_carrier = adapters._h1_payload_arrays(session)
        first7 = [interpolate_trial_identity(record, float(v)) for v in record.trial_values[:7]]
        by_m: dict[int, TaskBank] = {3: base}
        for budget in (7, 5, 4):
            activity = np.stack(first7[:budget], axis=0)
            e0, hc = _materialize_e0(activity, payload_carrier)
            ...
            meta.update(
                {
                    ...
                    "estimator": (
                        f"C2 protocol CAL-1: first {budget} held-in-calib trials, "
                        "C2 materializer, deploy H-C carrier (payload M3)"
                    ),
                }
            )
            by_m[budget] = dataclasses.replace(base, E0=e0, carrier=hc, calibration_meta=meta)
        out[session] = by_m
        print(f"[c2protocol] CAL-1 banks {session} M7/M5/M4/M3 ready", flush=True)
    return out
...
def run_train(dest: Path) -> dict[str, Any]:
    ...
    _seal(
        dest / "run_meta.json",
        {
            "schema": "btransform_unified_v1_h1_c2protocol_l200_p16_meta",
            "window": WINDOW,
            "proj_dim": PROJ_DIM,
            "epochs": EPOCHS,
    ...
    (dest / "PROGRESS_10MIN.md").write_text(
        "# C2-protocol L200 P16 (32ep, no mid-val)\n\n"
        "| ep | train_mse | ruling |\n|----|-----------|--------|\n",
        encoding="utf-8",
    )
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(
            f"[c2protocol L200 P16] ep{epoch}/{EPOCHS} loss={row['train_mse']:.5f} ({row['seconds']:.0f}s)",
            flush=True,
        )
        torch.save(
            {
                "schema": "btransform_unified_v1_h1_c2protocol_ckpt",
                "window": WINDOW,
                "proj_dim": PROJ_DIM,
                "epoch": epoch,
                "global_step": global_step,
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        curve.append(ho_row)
        print(
            f"[c2protocol HO-M3] e{epoch} {HO_SELECTION_METRIC}={ho_metrics['r2_mean']:.4f} "
            f"worst={ho_metrics['worst_session_r2']:.4f}",
            flush=True,
        )
    picked = select_epoch(curve)
    selection = {
        "schema": "btransform_unified_v1_h1_c2protocol_ho_m3_selection",
    ...
    summary = {
        "schema": "btransform_unified_v1_h1_c2protocol_train_receipt",
        "status": "COMPLETED",
        "epochs": EPOCHS,
