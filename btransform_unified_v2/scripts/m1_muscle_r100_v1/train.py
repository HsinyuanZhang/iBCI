#!/usr/bin/env python3
"""Independent M1 muscle-carrier R100/D4 runner over the frozen four-source face."""
...
CELL = "M1-MUSCLE-R100-D4-JOINT-B3S-CONCAT-V1"
...
def _checkpoint_payload(model, optimizer, ema, *, epoch: int, step: int, smoke: bool, meta: Mapping[str, Any], device: torch.device) -> dict[str, Any]:
    return {"schema": "m1_muscle_r100_epoch_checkpoint_v1", "cell": CELL, "epoch": epoch, "global_step": step, "smoke": smoke,
...
def _validate_checkpoint(path: Path, meta: Mapping[str, Any], *, expected_epoch: int | None = None) -> dict[str, Any]:
    ...
    expected = {"schema": "m1_muscle_r100_epoch_checkpoint_v1", "cell": CELL, "smoke": False}
    ...
    for key in ("source_hashes", "source_contract", "initialization_sha256", "carrier_binding"):
...
def run_train(args: argparse.Namespace) -> dict[str, Any]:
    ...
    report = {"legacy_frozen_activity_bank_report": legacy_report,
              "carrier_replacement": "muscle_response16_svd4/global_rms",
    ...
    baseline = _validate_baseline(args.baseline_run, contract, init_sha)
    ...
    meta = {"schema": "m1_muscle_r100_train_v1", "status": "SMOKE" if smoke else "FORMAL", "cell": CELL, "task": "m1", "arm":args.arm, "variant": "recency", "identity_interface": "live_b3s_concat", "identity_e0_dim": 100, "concat_token_width": 120, "seed": args.seed, "sampler_seed":SEED,
    ...
    if args.resume is None:
        ...
        if existing.get("status") != "FORMAL" or any(existing.get(key) != meta[key] for key in ("cell", "arm", "seed", "sampler_seed", "source_hashes", "source_contract", "initialization_sha256", "carrier_binding", "fit_sha256", "baseline_contract", "carrier_arm_contract")):
    ...
    for epoch in range(first_epoch, args.epochs + 1):
        ...
        if smoke:
            _atomic_json(dest / "smoke_receipt.json", {"schema": "m1_muscle_r100_smoke_receipt_v1", "status": "COMPLETED", "cell": CELL,
    ...
    _atomic_json(dest / "train_receipt.json", {"schema": "m1_muscle_r100_train_receipt_v1", "status": "COMPLETED", "cell": CELL,
...
def run_score(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if meta.get("status") != "FORMAL" or meta.get("cell") != CELL or meta.get("arm") != args.arm or meta.get("seed") != args.seed or meta.get("source_hashes") != _source_hashes() or meta.get("b3s") != _b3s_provenance() or meta.get("carrier_binding") != binding or meta.get("fit_sha256") != binding["fit_sha256"]:
    ...
    required_receipt = {"schema": "m1_muscle_r100_train_receipt_v1", "status": "COMPLETED", "cell": CELL, "arm": args.arm,
    ...
    score_baseline = _validate_baseline(args.baseline_run, meta["source_contract"], meta["initialization_sha256"], ho_contract)
    progress_path = dest / "score_progress.json"
    initial_progress = {"schema": "m1_muscle_r100_score_progress_v1", "cell": CELL, "arm": args.arm, "seed": args.seed,
    ...
    if any(progress.get(key) != initial_progress[key] for key in ("schema", "cell", "arm", "seed", "sampler_seed", "source_hashes", "source_contract", "b3s", "ho_contract", "carrier_binding", "fit_sha256", "baseline_contract")):
    ...
    receipt = {"schema": "m1_muscle_r100_ho_calib_epoch_scan_v1", "status": "COMPLETED", "cell": CELL, "arm": args.arm,
               "seed": args.seed, "sampler_seed": SEED, "source_hashes": meta["source_hashes"], "source_contract": meta["source_contract"], "b3s": meta["b3s"], "carrier_binding": binding, "fit_sha256": binding["fit_sha256"], "baseline_contract": score_baseline, "ema_by_epoch": scores,
