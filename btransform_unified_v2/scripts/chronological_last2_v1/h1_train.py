"""Train/score H1 on the fixed chronological leave-last-two-dates split."""
...
def preflight(args):
    ...
    row={"schema":"h1_chronological_last2_preflight_v1","status":"PASSED","split":prep.split_contract(),"arm":args.arm,"source_only":True,"target_records_opened":0,"source_manifest_sha256":sha_file(args.prepared/"source"/"manifest.json"),**checks}
...
def train(args):
    ...
    for epoch in range(start,EPOCHS+1):
        ...
        val=evaluate(model,ema,vbanks,vdata,device);epoch_row={"epoch":epoch,"step":step,"train_mse":float(np.mean(losses)),"source_val_ema":val,"sampler_endpoint_keep_sha256":sampler_digest.hexdigest(),"elapsed_seconds":time.monotonic()-began};curve.append(epoch_row);print(json.dumps(epoch_row,sort_keys=True),flush=True);torch_atomic(run/f"ema_epoch_{epoch:03d}.pt",{"schema":"h1_chronological_last2_ema_v1","epoch":epoch,"ema":ema.state_dict(),"recipe_binding":recipe});torch_atomic(run/"resume_latest.pt",{"schema":"h1_chronological_last2_resume_v1","epoch":epoch,"step":step,"model":model.state_dict(),"opt":opt.state_dict(),"ema":ema.state_dict(),"curve":curve,"rng":rng_state(),"recipe_binding":recipe})
    selected=max(curve,key=lambda r:(r["source_val_ema"]["equal_date_mean"],-r["epoch"]));receipt={"schema":"h1_chronological_last2_train_receipt_v1","meta":meta,"recipe_binding":recipe,"selection":"earliest maximum source validation equal_date_mean; equal_session_mean diagnostic only","selected":selected,"curve":curve,"source_files":{s:{"train":sha_file(args.prepared/"source"/f"{s}.npz"),"validation":sha_file(args.prepared/"source"/f"{s}.val.npz")} for s in data},"checkpoints":{f"ema_epoch_{i:03d}":sha_file(run/f"ema_epoch_{i:03d}.pt") for i in range(1,EPOCHS+1)}};json_atomic(run/"selection.json",selected);json_atomic(run/"train_receipt.json",receipt);json_atomic(args.prepared/"source"/f"selection_seal_{args.arm}.json",{"schema":"h1_chronological_last2_selection_seal_v1","split":prep.split_contract(),"arm":args.arm,"source_manifest_sha256":sha_file(args.prepared/"source"/"manifest.json"),"train_receipt":str((run/"train_receipt.json").resolve()),"train_receipt_sha256":sha_file(run/"train_receipt.json"),"selected_epoch":selected["epoch"],"selected":selected});return receipt
...
def score_one(args,epoch,banks,data,activity,device,label):
    ...
    if state.get("recipe_binding")!=meta.get("recipe_binding") or state.get("schema")!="h1_chronological_last2_ema_v1" or int(state.get("epoch",-1))!=epoch or receipt.get("checkpoints",{}).get(f"ema_epoch_{epoch:03d}")!=sha_file(path):raise RuntimeError("checkpoint receipt/epoch binding mismatch")
...
def score(args):
    ...
    manifest,banks,data,activity=load_surface(args.prepared,"target",args.arm);epoch=int(receipt["selected"]["epoch"]);device=torch.device(args.device);out={"schema":"h1_chronological_last2_target_score_v1","split":prep.split_contract(),"arm":args.arm,"target_query_labels_used_for_gradients":False,"target_query_labels_used_for_selection":False,"selected_source_epoch":epoch,"selected_ema":score_one(args,epoch,banks,data,activity,device,"selected_ema"),"fixed_e32_ema":score_one(args,32,banks,data,activity,device,"fixed_e32_ema"),"target_manifest_sha256":sha_file(args.prepared/"target"/"manifest.json")};json_atomic(args.dest/"target_score.json",out);return out
