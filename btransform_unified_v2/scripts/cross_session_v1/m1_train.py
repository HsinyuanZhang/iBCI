#!/usr/bin/env python3
"""Executable four-fold source-only M1 Z/B/D trainer and score reader."""
...
from m1_data import FOLDS,contract,assert_disjoint,materialize_sources,materialize_target
...
def source_evidence(m)->dict[str,object]:
    return {"train_neural":{s:array_hash(v) for s,v in m["train"].neural_data.items()},"train_targets":{s:array_hash(v) for s,v in m["train"].covariate_data.items()},"train_window_starts":array_hash(np.asarray(m["train"].window_indices,dtype=object).astype(str)),"calib":{s:array_hash(v) for s,v in m["calib"].items()},"carrier":{s:array_hash(v.carrier) for s,v in m["banks"].items()},"rsyn3_dictionary_and_normalizer":{s:array_hash(v) for s,v in m["rsyn3"].items()}}
def metadata(a:argparse.Namespace)->dict[str,object]:
    row=contract(a.target); assert_disjoint(row)
    return {"schema":"cross_session_m1_v1","arm":a.arm,"seed":a.seed,"fold":a.target,"data_contract":row,"recipe":{"epochs":EPOCHS,"batch":BATCH,"lr":LR,"weight_decay":.01,"ema":EMA_DECAY,"unit_dropout":UNIT_DROPOUT,"context":100,"precision":"bf16 decoder; encoder fp32","attention_backend":"local"},"selection":{"surface":"source validation only [310,end)","rule":"earliest maximum equal-session EMA","target_query_labels_used":False},"reporting":{"target":"selected source-val EMA and predeclared EMA epoch24","target_gradients":0}}
...
def _set_rng(s): random.setstate(s["python"]);np.random.set_state(s["numpy"]);torch.set_rng_state(s["torch"]); torch.cuda.set_rng_state_all(s["cuda"]) if s["cuda"] is not None else None
...
def prepare(a):
    ...
    m=materialize_sources(a.target,cache=_source_cache(a)); atomic_json(a.dest/"prepare.json",{**metadata(a),"status":"PREPARED","source_sessions":list(m["sources"]),"basis_fit_scope":m["basis_scope"],"source_cache_files":sorted(p.name for p in _source_cache(a).glob("*"))})
def preflight(a):
    """Real, small numerical assertions; source-only materialization intentionally opens no target."""
    m=materialize_sources(a.target,cache=_source_cache(a)); device=torch.device(a.device); torch.manual_seed(a.seed); np.random.seed(a.seed); random.seed(a.seed)
    ...
    atomic_json(a.dest/"preflight.json",{**metadata(a),"status":"PASSED","checks":report,"source_only":True,"rawtrial_fullwindow_disjoint":True})
def _require_ready(a):
    p=a.dest/"preflight.json"
    if not p.is_file() or json.loads(p.read_text()).get("status")!="PASSED": raise RuntimeError("run --stage preflight successfully first")
def train(a):
    ...
    m=materialize_sources(a.target,cache=_source_cache(a));device=torch.device(a.device);torch.manual_seed(a.seed);np.random.seed(a.seed);random.seed(a.seed);model=CrossSessionM1Decoder(a.arm,seed=a.seed).to(device);_configure(model);_install(model,a.arm,m["banks"],m["calib"]);ema=DecoderEMA(model,EMA_DECAY);opt=factory.build_optimizer(model.named_parameters(),lr=LR,weight_decay=.01);start_epoch=1;step=0;best=None
    ...
    if a.resume and cp.exists():
        q=torch.load(cp,map_location=device,weights_only=False);model.load_state_dict(q["model"],strict=True);ema.load_state_dict(q["ema"]);opt.load_state_dict(q["optimizer"]);_set_rng(q["rng"]);start_epoch=q["epoch"]+1;step=q["step"];best=q["best"]
    init_hash=state_hash(model); evidence=source_evidence(m); batch_hash=hashlib.sha256(); started=time.monotonic(); batches=len(FixedSessionSampler(IndexedWindows(m["train"]),a.seed,1,True)); total=EPOCHS*batches
    for ep in range(start_epoch,EPOCHS+1):
        ...
        for bid,(x,y,s,v,_starts) in enumerate(loader(m["train"],a.seed,ep,True)):
            ...
            batch_hash.update(f"{ep}|{bid}|{s[0]}|".encode()); batch_hash.update(_keep(a.seed,ep,bid,torch.device("cpu")).numpy().tobytes()); nn.utils.clip_grad_norm_(model.parameters(),1.);opt.step();ema.update_after_step(model)
        ...
        atomic_torch(cp,{"model":model.state_dict(),"ema":ema.state_dict(),"optimizer":opt.state_dict(),"epoch":ep,"step":step,"best":best,"rng":_rng_state(),"meta":metadata(a)})
    hashes={p.name:sha(p) for p in [cp,a.dest/"selected_ema.pt"]}; sources={str(p):sha(p) for p in [Path(__file__),ROOT/"scripts/cross_session_v1/m1_data.py",ROOT/"src/btransform_unified_v2/cross_session_m1_model.py"]};atomic_json(a.dest/"train_receipt.json",{**metadata(a),"status":"COMPLETED","selected":best,"epochs":EPOCHS,"steps":step,"initialization_hash":init_hash,"batch_order_sha256":batch_hash.hexdigest(),"checkpoint_sha256":hashes,"source_code_sha256":sources,"actual_source_arrays":evidence,"source_arrays_scope":m["basis_scope"]})
def score(a):
    _require_ready(a); rec=json.loads((a.dest/"train_receipt.json").read_text());src=materialize_sources(a.target,cache=_source_cache(a));tar=materialize_target(a.target,src,a.arm);device=torch.device(a.device);q=torch.load(a.dest/"resume_latest.pt",map_location=device,weights_only=False);model=CrossSessionM1Decoder(a.arm,seed=a.seed).to(device);_configure(model);banks=src["banks"] if a.arm==Z_NONE else {**src["banks"],a.target:tar.get("target_bank",src["banks"][src["sources"][0]])};calib=src["calib"] if a.arm==Z_NONE else {**src["calib"],a.target:tar["target_calib"]};_install(model,a.arm,banks,calib);from btransform_unified_v1.ema import DecoderEMA;ema=DecoderEMA(model,EMA_DECAY);model.load_state_dict(q["model"],strict=True);ema.load_state_dict(q["ema"]); score_bank=banks[a.target] if a.arm != Z_NONE else src["banks"][src["sources"][0]];e24=_ema_eval(model,ema,tar["target"],{a.target:score_bank},device,a.dest/"target_epoch24_predictions.npz");pick=torch.load(a.dest/"selected_ema.pt",map_location=device,weights_only=False);ema.load_state_dict(pick["ema"]);chosen=_ema_eval(model,ema,tar["target"],{a.target:score_bank},device,a.dest/"target_selected_predictions.npz");atomic_json(a.dest/"score_receipt.json",{**metadata(a),"status":"COMPLETED","source_selected":rec["selected"],"target_selected_ema":chosen,"target_epoch24_ema":e24,"target_labels_used_for_selection":False,"target_audit_arrays":{"selected":"target_selected_predictions.npz","epoch24":"target_epoch24_predictions.npz"},"checkpoint_sha256":{p.name:sha(p) for p in [a.dest/"resume_latest.pt",a.dest/"selected_ema.pt"]}})
def main():
    p=argparse.ArgumentParser();p.add_argument("--dest",type=Path,required=True);p.add_argument("--target",choices=FOLDS,required=True);p.add_argument("--arm",choices=ARMS,required=True);p.add_argument("--seed",type=int,default=42);p.add_argument("--stage",choices=("prepare","preflight","train","score"),required=True);p.add_argument("--device",default="cuda:0");p.add_argument("--resume",action="store_true");a=p.parse_args();
    if a.seed != 42: raise ValueError("protocol primary seed is 42")
    {"prepare":prepare,"preflight":preflight,"train":train,"score":score}[a.stage](a)
