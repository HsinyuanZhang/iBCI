#!/usr/bin/env python3
"""Isolated M1 meanrate4 flat-versus-recency ablation runner.

This file deliberately imports the frozen multiseed runner privately and reuses
...
WS = ROOT.parent
FROZEN_RUNNER = ROOT / "scripts/m1_muscle_r100_multiseed_v1/train.py"
FROZEN_DIR = FROZEN_RUNNER.parent
MODEL_FILE = HERE / "m1_flat_model.py"
OUTPUT_ROOT = ROOT / "results/recency_flat_ablation_v1"
DEFAULT_REFERENCE = ROOT / "results/m1_muscle_r100_multiseed_v1/formal_mean_rate4_s42"
DEFAULT_PACK = ROOT / "results/m1_muscle_r100_multiseed_v1/carrier_mean_rate4_v1/carrier_pack.npz"
...
spec = importlib.util.spec_from_file_location("_frozen_m1_multiseed_runner", FROZEN_RUNNER)
if spec is None or spec.loader is None:
    raise RuntimeError("cannot privately load frozen M1 multiseed runner")
...
CELL = "M1-MUSCLE-R100-D4-JOINT-B3S-CONCAT-FLAT-V1"
SCHEMA = "m1_muscle_r100_flat_train_v1"
CHECKPOINT_SCHEMA = "m1_muscle_r100_flat_epoch_checkpoint_v1"
SCORE_SCHEMA = "m1_muscle_r100_flat_ho_calib_epoch_scan_v1"
PROGRESS_SCHEMA = "m1_muscle_r100_flat_score_progress_v1"
FROZEN_SAMPLER_SEED = frozen.FROZEN_SAMPLER_SEED
...
def _carrier_binding(path: Path) -> tuple[dict[str, np.ndarray], dict[str, Any]]:
    carriers, legacy_binding = frozen._carrier_binding(path)
    binding = dict(legacy_binding)
    binding.pop("binding_sha256", None)
    binding["frozen_multiseed_runner_py_sha256"] = binding.pop("multiseed_runner_py_sha256")
    binding["flat_runner_py_sha256"] = _sha_file(Path(__file__).resolve())
...
def _paired_reference(path: Path, *, arm: str, binding: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if meta.get("status") != "FORMAL" or meta.get("variant") != "recency":
        raise RuntimeError("paired reference must be a completed formal recency run")
    ...
    carrier_fields = ("carrier_variant", "carrier_pack_npz_sha256", "carrier_pack_receipt_sha256",
                      "carrier_pack_receipt_body", "fit_sha256")
...
def run_train(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if binding["carrier_variant"] != frozen.CANDIDATE_CARRIER_VARIANT:
        raise RuntimeError("flat ablation accepts only the frozen meanrate4 candidate carrier pack")
    ...
    banks = frozen._replace_carriers(legacy_banks, carriers, frozen.SOURCE_SESSIONS)
...
def main() -> None:
    ...
    if args.dest is None:
        args.dest = OUTPUT_ROOT / (
            "root_smoke_m1_flat_s42" if args.max_updates_smoke is not None
            else "formal_m1_meanrate4_flat_s42"
        )
