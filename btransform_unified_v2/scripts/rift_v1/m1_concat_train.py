...
CELL = "M1-RIFT-R100-D4-CONCAT-E100-RECENCY-V1"
...
def _source_hashes() -> dict[str, str]:
    paths = (Path(__file__), ROOT / "src/btransform_unified_v2/model.py", ROOT / "src/btransform_unified_v2/concat_model.py", ROOT / "src/btransform_unified_v2/temporal.py",
...
def _decoder(device: torch.device):
    ...
    model = RiftConcatDecoder("m1", context_bins=CONTEXT, bias_mode="recency", seed=SEED).to(device)
...
def _init_parity(dataset: Any, banks: Mapping[str, Any], device: torch.device) -> dict[str, Any]:
    """Actual M1 source-bank full-forward equality at the frozen seed-42 init."""
    ...
    baseline = RiftDecoder("m1", context_bins=CONTEXT, bias_mode="recency", seed=SEED, proj_dim=PROJ_DIM).to(device).eval()
    concat = RiftConcatDecoder("m1", context_bins=CONTEXT, bias_mode="recency", seed=SEED).to(device).eval()
    shared = dict(concat.named_parameters()); pairs = [(n,v) for n,v in baseline.named_parameters() if n in shared and v.shape == shared[n].shape]
    with torch.inference_mode():
        left = baseline(x.to(device), banks[session], input_valid_mask=valid)
        right = concat(x.to(device), banks[session], input_valid_mask=valid)
    ...
    return {"status":"PASSED","batch_size":len(x),"session":session,"valid_zero_bin_count":zero_count,"output_max_abs":maximum,"threshold":2e-6,"shared_same_parameter_count":len(pairs),"shared_all_byte_equal":True,"baseline_total_parameters":sum(v.numel() for v in baseline.parameters()),"concat_total_parameters":sum(v.numel() for v in concat.parameters()),"baseline_init_sha256":digest(baseline),"concat_init_sha256":digest(concat),"head_alias":{"owner_final_norm_is_model":concat._frontend_owner.final_norm is concat.final_norm,"owner_readout_is_model":concat._frontend_owner.readout is concat.readout}}
...
def _checkpoint_payload(model, optimizer, ema, *, epoch: int, step: int, smoke: bool, meta: Mapping[str, Any], device: torch.device) -> dict[str, Any]:
    return {"schema": "m1_rift_concat_epoch_checkpoint_v1", "cell": CELL, "epoch": epoch, "global_step": step, "smoke": smoke,
            "config": {"seed": SEED, "context_bins": CONTEXT, "proj_dim": None, "epochs": EPOCHS, "batch": BATCH, "lr": LR, "bias_mode": "recency", "attention_backend": "local"},
...
def _validate_checkpoint(path: Path, meta: Mapping[str, Any], *, expected_epoch: int | None = None) -> dict[str, Any]:
    ...
    expected = {"schema": "m1_rift_concat_epoch_checkpoint_v1", "cell": CELL, "smoke": False}
    ...
    if config != {"seed": SEED, "context_bins": CONTEXT, "proj_dim": None, "epochs": EPOCHS, "batch": BATCH, "lr": LR, "bias_mode": "recency", "attention_backend": "local"}:
...
def run_train(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if smoke:
        ...
        smoke_preflight = {"init_parity": _init_parity(dataset, banks, device), "streaming_parity": _assert_full_stream_parity(model, _material, device), "repeatable_eval": _assert_ho_repeatable(model, ema, _material, device)}
    meta = {"schema": "m1_rift_concat_train_v1", "status": "SMOKE" if smoke else "FORMAL", "cell": CELL, "task": "m1", "variant": "recency", "identity_interface": "concat", "identity_e0_dim": 100, "concat_token_width": 120, "seed": SEED,
    ...
    for epoch in range(first_epoch, args.epochs + 1):
        ...
        if smoke:
            _atomic_json(dest / "smoke_receipt.json", {"status": "COMPLETED", "cell": CELL, "steps": step, "checkpoint_sha256": checkpoint_sha, "cuda_initialized": torch.cuda.is_initialized(), "finite_loss": bool(np.isfinite(losses[-1])), "all_gradients_finite": grad_finite, "preflight": smoke_preflight})
...
def run_score(args: argparse.Namespace) -> dict[str, Any]:
    ...
    initial_progress = {"schema": "m1_rift_concat_score_progress_v1", "cell": CELL, "source_hashes": meta["source_hashes"], "ho_contract": ho_contract, "completed": {}}
    ...
    receipt = {"schema": "m1_rift_concat_ho_calib_epoch_scan_v1", "status": "COMPLETED", "cell": CELL, "ema_by_epoch": scores,
