#!/usr/bin/env python3
"""Immutable ext6 EMA epoch picker for formal M2 RIFT runs.
...
SCHEMA = "m2_rift_ext6_epoch_pick_v2"
...
def manifest_for(run: Path, cache: Path, joint_m33_root: Path | None) -> dict[str, Any]:
    ...
    if meta.get("schema") == "m2_rift_concat_train_v1":
        ...
        required_receipt.update({"schema":"m2_rift_joint_train_receipt_v2", "arm":meta.get("arm"), "sampler_seed":42})
    else: raise RuntimeError("unsupported run metadata schema")
    if int(meta.get("seed", -1)) != 42: raise RuntimeError("formal M2 epoch picker requires seed 42")
