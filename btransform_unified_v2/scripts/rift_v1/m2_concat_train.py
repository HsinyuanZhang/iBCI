#!/usr/bin/env python3
"""Formal matched M2 RIFT-R50-D4 concat trainer.
...
CELL = "M2-RIFT-R50-D4-CONCAT-E50-RECENCY-V1"
SEED = 42
CONTEXT = 50
PROJ_DIM = 16  # retained only for shared baseline-init comparison metadata
EPOCHS = 24
BATCH = 32
...
def _decoder(device: torch.device):
    ...
    model = RiftConcatDecoder("m2", context_bins=CONTEXT, bias_mode="recency", seed=SEED).to(device)
...
def _validate_ext4_report(report: Mapping[str, Any]) -> None:
    expected = dict(old_plan.EXT4_EXPECTED_WINDOWS)
    if report.get("partial") is not False or int(report.get("n_windows", -1)) != sum(expected.values()) or set(report.get("per_session", {})) != set(expected) or not math.isfinite(float(report.get("equal_session_mean", float("nan")))) or not math.isfinite(float(report.get("pooled_r2", float("nan")))) or any(int(report["per_session"][name].get("window_count", -1)) != count or not math.isfinite(float(report["per_session"][name].get("r2", float("nan")))) for name, count in expected.items()):
        raise RuntimeError("malformed ext4 report")
...
def _checkpoint(epoch: int, step: int, model: nn.Module, optimizer: torch.optim.Optimizer, ema: DecoderEMA, device: torch.device, manifest_digest: str, *, smoke: bool, cache_hashes: Mapping[str, Any], epochs: int) -> dict[str, Any]:
    return {"schema": "m2_rift_concat_epoch_checkpoint_v1", "cell": CELL, "epoch": epoch, "global_step": step,
            "seed": SEED, "context_bins": CONTEXT, "bias_mode": "recency", "attention_backend": "local", "identity_interface": "concat", "identity_e0_dim": 50, "proj_dim": None,
...
def _source_hashes() -> dict[str, str]:
    paths = [Path(__file__), ROOT / "src/btransform_unified_v2/model.py", ROOT / "src/btransform_unified_v2/concat_model.py", ROOT / "src/btransform_unified_v2/temporal.py", ROOT / "src/btransform_unified_v2/config.py", MANIFEST]
...
def _run_meta(model: nn.Module, manifest: Mapping[str, Any], train_dual, train_banks, mini_dual, mini_banks, ext_dual, ext_banks, *, smoke: bool, epochs: int) -> dict[str, Any]:
    return {"schema": "m2_rift_concat_train_v1", "status": "SMOKE" if smoke else "FORMAL", "cell": CELL, "seed": SEED,
            "task": "m2", "variant": "recency", "identity_interface": "concat", "identity_e0_dim": 50, "proj_dim": None,
...
def run_train(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.resume:
        ...
        required = {"schema": "m2_rift_concat_epoch_checkpoint_v1", "cell": CELL, "manifest_digest": MANIFEST_DIGEST, "seed": SEED, "context_bins": CONTEXT, "bias_mode": "recency", "attention_backend": "local", "identity_e0_dim": 50, "proj_dim": None, "smoke": False, "epochs": EPOCHS}
    ...
    if smoke:
        _atomic_json(dest / "smoke_receipt.json", {"schema": "m2_rift_concat_smoke_receipt_v1", "cell": CELL, "status": "COMPLETED", "epoch": epoch, "global_step": step, "device": str(device), "finite_train_mse": bool(math.isfinite(float(np.mean(losses)))), "partial_minival": True, "cuda_initialized": bool(torch.cuda.is_initialized()), "checkpoint": str(dest / f"epoch_{epoch:03d}.pt"), "source_hashes": current_meta["source_hashes"], "frozen_cache_hashes": cache_hashes, "finished_utc": datetime.now(timezone.utc).isoformat()})
    else:
        _atomic_json(dest / "train_receipt.json", {"schema": "m2_rift_concat_train_receipt_v1", "cell": CELL, "status": "COMPLETED", "epochs": EPOCHS, "global_step": step, "manifest_digest": manifest["digest"], "source_hashes": current_meta["source_hashes"], "frozen_cache_hashes": cache_hashes, "finished_utc": datetime.now(timezone.utc).isoformat()})
...
def score_stage(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if receipt.get("schema") != "m2_rift_concat_train_receipt_v1" or receipt.get("cell") != CELL or receipt.get("status") != "COMPLETED" or int(receipt.get("epochs", 0)) != EPOCHS or int(receipt.get("global_step", 0)) != EPOCHS * 3165 or receipt.get("source_hashes") != meta.get("source_hashes") or receipt.get("frozen_cache_hashes") != meta.get("frozen_cache_hashes"): raise RuntimeError("train receipt contract mismatch")
    ...
    progress = json.loads(progress_path.read_text()) if progress_path.is_file() else {"schema": "m2_rift_concat_ext4_progress_v1", "cell": CELL, "source_hashes": meta["source_hashes"], "frozen_cache_hashes": meta["frozen_cache_hashes"], "completed": {}}
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if prior is not None:
            if prior.get("checkpoint_sha256") != checkpoint_sha: raise RuntimeError("ext4 prior checkpoint hash drift")
            _validate_ext4_report(prior)
            continue
        state = torch.load(checkpoint_path, map_location=device, weights_only=False)
        required = {"schema": "m2_rift_concat_epoch_checkpoint_v1", "cell": CELL, "epoch": epoch, "smoke": False, "context_bins": CONTEXT, "bias_mode": "recency", "attention_backend": "local", "identity_interface": "concat", "identity_e0_dim": 50, "epochs": EPOCHS}
        if any(state.get(key) != value for key, value in required.items()) or state.get("source_hashes") != meta["source_hashes"] or state.get("frozen_cache_hashes") != meta["frozen_cache_hashes"]: raise RuntimeError("ext4 checkpoint contract mismatch")
    ...
    scan = {"schema": "m2_rift_concat_ext4_epoch_scan_v1", "cell": CELL, "surface": "ext4", "official_test_used": False, "ema_by_epoch": progress["completed"], "selection": {"rule": "earliest best equal_session_mean", "epoch": best_epoch, "equal_session_mean": values[best_epoch]}, "endpoint24": progress["completed"]["24"]}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Formal matched M2 RIFT concat trainer and ext4 scorer")
