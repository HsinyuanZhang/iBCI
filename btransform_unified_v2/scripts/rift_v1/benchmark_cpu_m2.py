...
def selected(run):
 ...
 if meta.get('status')!='FORMAL' or score.get('status')!='COMPLETED' or train.get('schema')!=expected_schema or train.get('status')!='COMPLETED' or score.get('cell')!=meta.get('cell') or train.get('cell')!=meta.get('cell') or score.get('official_test_used') is not False:raise RuntimeError('formal train/score schema binding failed')
 ...
 if not expected or sha(ck)!=expected or st.get('smoke') or st.get('source_hashes')!=meta['source_hashes'] or st.get('cache_hashes',st.get('frozen_cache_hashes'))!=meta.get('cache_hashes',meta.get('frozen_cache_hashes')):raise RuntimeError('selected checkpoint binding failed')
...
def joint_materialized(meta,st,banks):
 ...
 for k in cd:
  if k in ld and cd[k].shape==ld[k].shape:cd[k]=ld[k]
 ...
 for bank in banks:
  ...
  mats.append(TaskBank(session_id=bank.session_id,E0=np.ascontiguousarray(e0.numpy()),carrier=np.ascontiguousarray(direct),unit_mask=bank.unit_mask,X_store=bank.X_store,target_store=bank.target_store,window_ids=bank.window_ids,calibration_meta=dict(bank.calibration_meta)))
...
def parity(m,banks,raw):
 ...
 if mx>1e-5:raise RuntimeError(f'cached stream parity {mx}')
 ...
 with torch.inference_mode():
  for t in range(401):
   ...
   if t in (49,200,400):
    ...
    if d>1e-5:raise RuntimeError(f'oracle {t} {d}')
...
def measure(m,banks,raw):
 ...
 with torch.inference_mode():
  for r in range(3):
   ...
   for x in raw[1:301]:rt.advance(torch.from_numpy(x))
   vals=[]
   for x in raw[301:401]:t=time.perf_counter();rt.advance(torch.from_numpy(x));vals.append(time.perf_counter()-t)
...
def main():
 ...
 if kind=='concat': cached=concat(meta,st);live=None;mats=orig;matwall=None
 else: live,cached,mats,matwall=joint_materialized(meta,st,orig)
 streams,raw8=flows(dual,{b.session_id:b for b in mats}); cases={}; jointpar=None
 if live is not None:
  b=mats[0];startbin=int(dual[b.session_id].eligible_starts[0]);x=np.asarray(dual[b.session_id].X_store[startbin:startbin+50],np.float32)[None]
  with torch.inference_mode():d=float((live(torch.from_numpy(x),orig[0])-cached(torch.from_numpy(x),b)).abs().max())
  if d>1e-5:raise RuntimeError(f'joint materialization parity {d}')
  jointpar={'passed':True,'fullwindow_max_abs_error':d,'materialization_total_wall_seconds':matwall}
 for B in (1,8):
