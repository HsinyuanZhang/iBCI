#!/usr/bin/env python3
"""Independent M1 carrier-v4 R100/D4 runner over the frozen four-source face."""
...
WS = ROOT.parent
for candidate in (ROOT / "src", V4, V1 / "src", V1 / "scripts", WS):
...
from model import INFORMATION_ARMS, M1InformationArmDecoder, arm_contract

CELL_PREFIX = "M1-CARRIER-V4-R100-D4-JOINT-B3S"
SEED, CONTEXT, EPOCHS, BATCH, LR = 42, 100, 24, 32, 1e-4


def _cell(fusion: str, proj_dim: int) -> str:
    if fusion == "concat":
        if proj_dim != 16:
            raise ValueError("concat fusion is defined only for P16")
        return f"{CELL_PREFIX}-CONCAT-P16-V1"
    if fusion == "proj_add" and proj_dim in (16, 32):
        return f"{CELL_PREFIX}-PROJADD-P{proj_dim}-V1"
    raise ValueError(f"unsupported fusion/proj-dim: {fusion}/{proj_dim}")
...
def _carrier_binding(path: Path) -> tuple[dict[str, np.ndarray], dict[str, Any]]:
    """Load exactly seven canonical 64x4 carriers and bind both pack files."""
    pack = path.resolve()
    ...
    implementation = Path(__file__).with_name("carrier.py")
    ...
    binding = {
        ...
        "carrier_pack_receipt_body": dict(receipt), "carrier_py_sha256": _sha_file(implementation),
        "train_py_sha256": _sha_file(Path(__file__).resolve()), "fit_sha256": fit_sha256,
    }
...
def _baseline_run(args: argparse.Namespace) -> Path:
    if args.baseline_run is not None:
        return args.baseline_run.resolve()
    name = "m1_r100_joint_d_s42_formal_v1" if args.information_arm == "full" else "m1_r100_joint_b_s42_formal_v1"
    return ROOT / "results/rift_v1" / name


def _validate_baseline(baseline_run: Path, source_contract: Mapping[str, Any], initialization_sha256: str,
                       ho_contract: Mapping[str, Any] | None = None) -> dict[str, Any]:
    """Require exact legacy noncarrier data/sampler/init and HO endpoint identity."""
    ...
    if meta.get("initialization_sha256") != initialization_sha256:
        raise RuntimeError("baseline initialization SHA drift")
    out = {"baseline_run": str(root), "baseline_run_meta_sha256": _sha_file(root / "run_meta.json"),
           "source_noncarrier_contract_match": True, "initialization_sha256_match": True}
...
def _source_hashes() -> dict[str, str]:
    ...
    paths = (Path(__file__), Path(__file__).with_name("carrier.py"), Path(__file__).with_name("model.py"), Path(__file__).resolve().parents[1] / "common_estimator.py", V4 / "fusion_model.py", ROOT / "src/btransform_unified_v2/joint_m1_model.py", ROOT / "src/btransform_unified_v2/model.py", ROOT / "src/btransform_unified_v2/concat_model.py", ROOT / "src/btransform_unified_v2/temporal.py",
...
def _decoder(device: torch.device, information_arm: str, seed: int, *, fusion: str, proj_dim: int):
    model = M1InformationArmDecoder(information_arm, fusion=fusion, proj_dim=proj_dim, seed=seed).to(device)
...
def _ho_material(carriers: Mapping[str, np.ndarray]) -> dict[str, Any]:
    ...
    for session in HO:
        ...
        result[session] = {"dataset": dataset, "bank": bank, "calib10": opened["calib10"], "starts": starts,
                           ...
                           "window_count": len(starts), "carrier_meta": {"legacy": carrier_meta, "replacement": "carrier_v4_pack_T_only"}}
    return result
...
def _init_parity(dataset: Any, banks: Mapping[str, Any], source_calib: Mapping[str, Any], device: torch.device, *, information_arm: str, seed: int, fusion: str, proj_dim: int) -> dict[str, Any]:
    """Audit static equivalence after applying the selected E0/T transform."""
    ...
    if any(name != session for name in sessions):
        raise RuntimeError("parity batch session drift")
    x = x.clone(); zero_count = int((x == 0).sum())
    if zero_count == 0:
        x[0, 0, 0] = 0.; zero_count = 1
    valid = torch.ones((len(x), CONTEXT), dtype=torch.bool, device=device)
    frozen = build_static(task="m1", fusion=fusion, proj_dim=proj_dim, seed=seed, device=device).eval()
    joint = _decoder(device, information_arm, seed, fusion=fusion, proj_dim=proj_dim)
    joint.install_session_memory(banks, source_calib if joint.encoder_active else {}); joint.to(device); joint.eval()
    static = dict(frozen.named_parameters()); live = dict(joint.named_parameters())
    missing = sorted(set(static) - set(live))
    unequal = sorted(name for name, value in static.items() if name in live and value.shape == live[name].shape and not torch.equal(value, live[name]))
    shape_drift = sorted(name for name, value in static.items() if name in live and value.shape != live[name].shape)
    if missing or unequal or shape_drift:
        raise RuntimeError(f"{fusion} P{proj_dim} static/live initialization parameter drift")
    e0, carrier = joint._identity([session], device)
    transformed = dataclasses.replace(
        banks[session],
        E0=np.ascontiguousarray(e0[0].detach().cpu().numpy(), dtype=np.float32),
        carrier=np.ascontiguousarray(carrier[0].detach().cpu().numpy(), dtype=np.float32),
    )
    audit = audit_matched_initialization("m1", {"static": frozen, "live": joint}, actual_banks=transformed, x=x.to(device), input_valid_mask=valid)
    pair = audit["pairwise"]["static__live"]
    forward = audit["forward"]["max_abs_vs_anchor"]["live"]
    if not pair["non_fusion_shared_byte_equal"] or not pair["effective_fold_shape_equal"] or pair["effective_fold_max_abs"] != 0.0 or forward >= 2e-6:
        raise RuntimeError(f"{fusion} P{proj_dim} matched initialization audit failed")
    legacy_model = None
    legacy_forward = None
    if information_arm in ("full", "activity_only"):
        from fusion_model import JointM1FusionDecoder
        legacy_arm = "D_JOINT" if information_arm == "full" else "B_ACTIVITY_ONLY"
        legacy_model = JointM1FusionDecoder(legacy_arm, fusion=fusion, proj_dim=proj_dim, seed=seed).to(device).eval()
        legacy_model.install_session_memory(banks, source_calib); legacy_model.to(device)
        wrapper_parameters = dict(joint.named_parameters())
        legacy_parameters = dict(legacy_model.named_parameters())
        if set(wrapper_parameters) != set(legacy_parameters) or any(
            not torch.equal(wrapper_parameters[name], legacy_parameters[name]) for name in wrapper_parameters
        ):
            raise RuntimeError(f"{information_arm}: wrapper/legacy initialization drift")
        with torch.no_grad():
            legacy_forward = float((joint(x.to(device), banks[session], input_valid_mask=valid) - legacy_model(x.to(device), banks[session], input_valid_mask=valid)).abs().max())
        if legacy_forward != 0.0:
            raise RuntimeError(f"{information_arm}: wrapper/legacy forward drift")
    return {"status": "PASSED", "comparison": "fusion_specific_static_vs_information_transformed_live", "information_arm": information_arm, "fusion": fusion, "proj_dim": proj_dim,
            "seed": seed, "batch_size": len(x), "session": session, "valid_zero_bin_count": zero_count,
            "output_max_abs": forward, "threshold": 2e-6, "static_parameter_missing_from_live": missing,
            "static_parameter_shape_drift": shape_drift, "static_parameter_byte_unequal": unequal,
            "fusion_audit": audit,
            "legacy_exact_equivalence": None if legacy_model is None else {"legacy_arm": legacy_model.arm, "forward_max_abs": legacy_forward},
            "transformed_bank": {"e0_nonzero": int(np.count_nonzero(transformed.E0)), "carrier_nonzero": int(np.count_nonzero(transformed.carrier))}}
...
def _checkpoint_payload(model, optimizer, ema, *, epoch: int, step: int, smoke: bool, meta: Mapping[str, Any], device: torch.device) -> dict[str, Any]:
    return {"schema": "m1_carrier_v4_epoch_checkpoint_v1", "cell": meta["cell"], "information_arm": meta["information_arm"], "epoch": epoch, "global_step": step, "smoke": smoke,
            "config": {"information_arm":meta["information_arm"], "fusion":meta["fusion"], "proj_dim":meta["proj_dim"], "seed": meta["seed"], "sampler_seed":SEED, "context_bins": CONTEXT, "epochs": EPOCHS, "batch": BATCH, "lr": LR, "bias_mode": "recency", "attention_backend": "local", "b3s": meta["b3s"]},
            "source_hashes": meta["source_hashes"], "source_contract": meta["source_contract"], "initialization_sha256": meta["initialization_sha256"],
            "carrier_binding": meta["carrier_binding"], "fit_sha256": meta["carrier_binding"]["fit_sha256"], "information_arm_contract": meta["information_arm_contract"],
...
def _validate_checkpoint(path: Path, meta: Mapping[str, Any], *, expected_epoch: int | None = None) -> dict[str, Any]:
    ...
    expected = {"schema": "m1_carrier_v4_epoch_checkpoint_v1", "cell": meta["cell"], "information_arm": meta["information_arm"], "smoke": False}
    ...
    if config != {"information_arm":meta["information_arm"], "fusion":meta["fusion"], "proj_dim":meta["proj_dim"], "seed":meta["seed"], "sampler_seed":SEED, "context_bins": CONTEXT, "epochs": EPOCHS, "batch": BATCH, "lr": LR, "bias_mode": "recency", "attention_backend": "local", "b3s": meta["b3s"]}:
        raise RuntimeError(f"checkpoint configuration mismatch: {path}")
    for key in ("source_hashes", "source_contract", "initialization_sha256", "carrier_binding", "information_arm_contract"):
...
def run_train(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not smoke and args.epochs != EPOCHS:
        raise ValueError("formal M1 carrier v4 R100 requires exactly 24 epochs")
    ...
    report = {"legacy_frozen_activity_bank_report": legacy_report,
              "carrier_replacement": "sealed_source3_rsyn3_imported_commoncore_exact_m10",
    ...
    model = _decoder(device, args.information_arm, args.seed, fusion=args.fusion, proj_dim=args.proj_dim)
    model.install_session_memory(banks, source_calib if model.encoder_active else {}); model.to(device)
    init_sha = _initialization_sha(model); ema = DecoderEMA(model, decay=plan.EMA_DECAY)
    baseline_path = _baseline_run(args)
    baseline = (_validate_baseline(baseline_path, contract, init_sha) if args.information_arm in ("full", "activity_only") and args.fusion == "concat" and args.proj_dim == 16 else {"baseline_run": str(baseline_path), "legacy_full_init_comparison": "not_applicable_to_information_arm_or_fusion"})
    ...
    if smoke:
        ...
        model.install_session_memory({s:_material[s]["bank"] for s in HO}, {s:_material[s]["calib10"] for s in HO} if model.encoder_active else {}); model.to(device)
        smoke_preflight = {"init_parity": _init_parity(dataset, banks, source_calib, device, information_arm=args.information_arm, seed=args.seed, fusion=args.fusion, proj_dim=args.proj_dim), "streaming_parity": _assert_full_stream_parity(model, _material, device), "repeatable_eval": _assert_ho_repeatable(model, ema, _material, device), "live_encoder_parameter_sha256": _initialization_sha(model.encoder)}
    meta = {"schema": "m1_carrier_v4_train_v1", "status": "SMOKE" if smoke else "FORMAL", "cell": _cell(args.fusion, args.proj_dim), "task": "m1", "information_arm":args.information_arm, "fusion":args.fusion, "variant": "recency", "identity_interface": "live_b3s_fusion", "identity_e0_dim": 100, "fusion_token_width": 120 if args.fusion == "concat" else args.proj_dim + 4, "seed": args.seed, "sampler_seed":SEED,
            "context_bins": CONTEXT, "query_pad_bins": QUERY_PAD_BINS, "layer_windows": [25, 25, 25, 24], "depth": 4, "width": 256, "proj_dim": args.proj_dim, "attention_backend": "local",
            ...
            "information_arm_contract": arm_contract(args.information_arm),
    ...
    if args.resume is None:
        ...
        if existing.get("status") != "FORMAL" or any(existing.get(key) != meta[key] for key in ("cell", "information_arm", "fusion", "proj_dim", "seed", "sampler_seed", "source_hashes", "source_contract", "initialization_sha256", "carrier_binding", "fit_sha256", "baseline_contract", "information_arm_contract")):
    ...
    for epoch in range(first_epoch, args.epochs + 1):
        ...
        for batch_id, (x, y, sessions) in enumerate(loader):
            ...
            if smoke:
                ...
                if not grad_finite: raise FloatingPointError("nonfinite fusion gradient")
        ...
        if smoke:
            _atomic_json(dest / "smoke_receipt.json", {"schema": "m1_carrier_v4_smoke_receipt_v1", "status": "COMPLETED", "cell": meta["cell"],
                         "information_arm": args.information_arm, "fusion": args.fusion, "proj_dim": args.proj_dim, "seed": args.seed, "sampler_seed": SEED, "steps": step,
                         "checkpoint_sha256": checkpoint_sha, "source_hashes": meta["source_hashes"], "source_contract": meta["source_contract"], "carrier_binding": meta["carrier_binding"], "fit_sha256": meta["carrier_binding"]["fit_sha256"], "information_arm_contract": meta["information_arm_contract"], "baseline_contract": meta["baseline_contract"],
    ...
    _atomic_json(dest / "train_receipt.json", {"schema": "m1_carrier_v4_train_receipt_v1", "status": "COMPLETED", "cell": meta["cell"],
                 "information_arm": args.information_arm, "fusion": args.fusion, "proj_dim": args.proj_dim, "seed": args.seed, "sampler_seed": SEED, "epochs": EPOCHS, "steps": step,
                 "source_hashes": meta["source_hashes"], "source_contract": meta["source_contract"], "b3s": meta["b3s"], "carrier_binding": meta["carrier_binding"], "fit_sha256": meta["carrier_binding"]["fit_sha256"], "information_arm_contract": meta["information_arm_contract"], "baseline_contract": meta["baseline_contract"],
...
def run_score(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if meta.get("status") != "FORMAL" or meta.get("cell") != _cell(args.fusion, args.proj_dim) or meta.get("information_arm") != args.information_arm or meta.get("information_arm_contract") != arm_contract(args.information_arm) or meta.get("fusion") != args.fusion or meta.get("proj_dim") != args.proj_dim or meta.get("seed") != args.seed or meta.get("source_hashes") != _source_hashes() or meta.get("b3s") != _b3s_provenance() or meta.get("carrier_binding") != binding or meta.get("fit_sha256") != binding["fit_sha256"]:
        raise RuntimeError("score requires matching formal M1 carrier v4 R100 run")
    ...
    required_receipt = {"schema": "m1_carrier_v4_train_receipt_v1", "status": "COMPLETED", "cell": meta["cell"], "information_arm": args.information_arm, "fusion": args.fusion, "proj_dim": args.proj_dim,
                        "seed": args.seed, "sampler_seed": SEED, "epochs": EPOCHS, "steps": EPOCHS * UPDATES_PER_EPOCH,
                        "source_hashes": meta["source_hashes"], "source_contract": meta["source_contract"], "b3s": meta["b3s"], "information_arm_contract": meta["information_arm_contract"], "carrier_binding": binding, "fit_sha256": binding["fit_sha256"], "baseline_contract": meta["baseline_contract"]}
    ...
    device = torch.device(args.device); material = _ho_material(carriers); model = _decoder(device,args.information_arm,args.seed,fusion=args.fusion,proj_dim=args.proj_dim)
    model.install_session_memory({s:material[s]['bank'] for s in HO}, {s:material[s]['calib10'] for s in HO} if model.encoder_active else {}); model.to(device)
    ema = DecoderEMA(model, decay=plan.EMA_DECAY); ho_contract = _ho_contract(material)
    baseline_path = _baseline_run(args)
    score_baseline = (_validate_baseline(baseline_path, meta["source_contract"], meta["initialization_sha256"], ho_contract) if args.information_arm in ("full", "activity_only") and args.fusion == "concat" and args.proj_dim == 16 else meta["baseline_contract"])
    progress_path = dest / "score_progress.json"
    initial_progress = {"schema": "m1_carrier_v4_score_progress_v1", "cell": meta["cell"], "information_arm": args.information_arm, "fusion": args.fusion, "proj_dim": args.proj_dim, "seed": args.seed,
                        "sampler_seed": SEED, "source_hashes": meta["source_hashes"], "source_contract": meta["source_contract"],
                        "b3s": meta["b3s"], "ho_contract": ho_contract, "information_arm_contract": meta["information_arm_contract"], "carrier_binding": binding, "fit_sha256": binding["fit_sha256"], "baseline_contract": score_baseline, "completed": {}}
    progress = json.loads(progress_path.read_text()) if progress_path.is_file() else initial_progress
    if any(progress.get(key) != initial_progress[key] for key in ("schema", "cell", "information_arm", "fusion", "proj_dim", "seed", "sampler_seed", "source_hashes", "source_contract", "b3s", "ho_contract", "information_arm_contract", "carrier_binding", "fit_sha256", "baseline_contract")):
    ...
    receipt = {"schema": "m1_carrier_v4_ho_calib_epoch_scan_v1", "status": "COMPLETED", "cell": meta["cell"], "information_arm": args.information_arm, "fusion": args.fusion, "proj_dim": args.proj_dim,
               "seed": args.seed, "sampler_seed": SEED, "source_hashes": meta["source_hashes"], "source_contract": meta["source_contract"], "b3s": meta["b3s"], "information_arm_contract": meta["information_arm_contract"], "carrier_binding": binding, "fit_sha256": binding["fit_sha256"], "baseline_contract": score_baseline, "ema_by_epoch": scores,
...
def main() -> None:
    ...
    parser.add_argument("--dest", required=True, type=Path); parser.add_argument("--information-arm", choices=INFORMATION_ARMS, default="full"); parser.add_argument("--fusion", choices=("concat", "proj_add"), default="concat"); parser.add_argument("--proj-dim", type=int, choices=(16, 32), default=16); parser.add_argument("--seed",type=int,choices=(42,43,44),default=42); parser.add_argument("--stage", choices=("train", "score", "all"), default="train")
    parser.add_argument("--carrier-pack", required=True, type=Path, help="immutable seven-session M1 v4 carrier NPZ")
    parser.add_argument("--baseline-run", type=Path, help="optional legacy B/D run for exact full/activity-only concat contract checks")
    ...
    if args.fusion == "concat" and args.proj_dim != 16: parser.error("--fusion concat requires --proj-dim 16")
    ...
    if args.max_updates_smoke is None and args.stage == "train" and args.epochs != EPOCHS: parser.error("formal M1 carrier v4 R100 requires exactly 24 epochs")
