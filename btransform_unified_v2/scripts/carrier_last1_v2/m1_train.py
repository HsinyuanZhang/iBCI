...
def metadata(a:argparse.Namespace)->dict[str,object]:
    ...
    return {"schema":"m1_carrier_last1_v2","split_id":SPLIT_ID,"sources":list(SOURCE_SESSIONS),"targets":list(TARGET_SESSIONS),"arm":a.arm,"seed":a.seed,"data_contract":row,"recipe":{"epochs":EPOCHS,"batch":BATCH,"lr":LR,"weight_decay":.01,"ema":EMA_DECAY,"unit_dropout":UNIT_DROPOUT,"context":100,"precision":"bf16 decoder; encoder fp32","attention_backend":"local"},"selection":{"surface":"source validation only [310,end)","rule":"earliest maximum equal-session EMA","target_query_labels_used":False},"reporting":{"target":"selected source-val EMA and predeclared EMA epoch24","target_gradients":0}}
...
def main():
    p=argparse.ArgumentParser();p.add_argument("--dest",type=Path,required=True);p.add_argument("--arm",choices=ARMS,required=True);p.add_argument("--seed",type=int,default=42);p.add_argument("--stage",choices=("prepare","preflight","train"),required=True);p.add_argument("--device",default="cuda:0");p.add_argument("--resume",action="store_true");p.add_argument("--carrier-pack",type=Path);p.add_argument("--temporal-stage",choices=("inner","outer"),default="outer");a=p.parse_args();
    if a.carrier_pack: __import__("os").environ["SOURCE_CARRIER_PACK"]=str(a.carrier_pack.resolve())
