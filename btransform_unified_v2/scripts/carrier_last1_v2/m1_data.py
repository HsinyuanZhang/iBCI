"""Chronological M1 data contract: train first two dates, score last one."""
...
STAGE=os.environ.get("CARRIER_STAGE") or (sys.argv[sys.argv.index("--temporal-stage")+1] if "--temporal-stage" in sys.argv else "outer")
if STAGE not in ("inner", "outer"): raise ValueError("CARRIER_STAGE must be inner or outer")
SPLIT_ID = "m1_inner_last1_20120927" if STAGE == "inner" else "m1_outer_last1_20120928"
SOURCE_SESSIONS = ("ses-20120924", "ses-20120926") if STAGE == "inner" else ("ses-20120924", "ses-20120926", "ses-20120927")
TARGET_SESSIONS = ("ses-20120927",) if STAGE == "inner" else ("ses-20120928",)
FOLDS = SOURCE_SESSIONS + TARGET_SESSIONS
...
def _install_runtime() -> None:
    ...
    origin=Path(falcon_datamodule.__file__).resolve()
    if STREAMING not in origin.parents: raise RuntimeError(f"unexpected Falcon runtime origin: {origin}")

def nwb_path(session: str) -> Path:
    if session not in FOLDS: raise ValueError(f"unknown chronological M1 session {session!r}")
    return DATA / f"sub-MonkeyL-held-in-calib_{session}_behavior+ecephys.nwb"

def contract() -> dict[str, object]:
    return {"split_id": SPLIT_ID, "sources": list(SOURCE_SESSIONS), "targets": list(TARGET_SESSIONS),
            "source_support_trials": list(SUPPORT), "source_train_trials": list(TRAIN),
            "source_val_trials": [310, "end"], "target_support_trials": list(SUPPORT),
            "target_query_trials": list(QUERY), "target_query_labels_for_selection": False,
            "target_optimizer_steps": 0,
            "padding": "each range is sliced at its trial boundary then independently left-padded/reset"}

def assert_contract(row: Mapping[str, object]) -> None:
    expected = contract()
    for key, value in expected.items():
        if row.get(key) != value: raise RuntimeError(f"chronological M1 contract drift: {key}")
...
def _rectify(emg: np.ndarray) -> np.ndarray: return np.maximum(np.asarray(emg,dtype=np.float64),0.0)

def _carrier_pack(surface: str) -> dict[str, np.ndarray] | None:
    if surface not in ("source", "target"): raise ValueError(surface)
    raw=os.environ.get("SOURCE_CARRIER_PACK" if surface == "source" else "TARGET_CARRIER_PACK")
    if not raw: return None
    path=Path(raw)
    with np.load(path, allow_pickle=False) as z:
        meta=json.loads(str(z["metadata"].item()))
        if meta.get("schema") != "carrier_last1_v2_pack" or meta.get("dataset") != "m1": raise RuntimeError("invalid M1 carrier pack")
        required=set(SOURCE_SESSIONS if surface == "source" else TARGET_SESSIONS)
        if set(meta.get("sources",())) | set(meta.get("targets",())) != required: raise RuntimeError("carrier pack roster differs from split")
        out={name:np.ascontiguousarray(z[f"carrier/{name}"],np.float32) for name in required}
    if any(x.shape != (64,4) or not np.isfinite(x).all() for x in out.values()): raise RuntimeError("invalid M1 carrier-pack rows")
    return out

def _source_carriers(cache: Path|None) -> tuple[dict[str,np.ndarray],dict[str,np.ndarray]]:
    sources=SOURCE_SESSIONS; key=hashlib.sha256("|".join(sources).encode()).hexdigest(); path=cache/f"rsyn3_{key}.npz" if cache else None
    ...
    loaded={s:raw_data.load_support_bins(nwb_path(s),emg_trial_stop=310,neural_trial_stop=10) for s in sources}; basis=syn3.fit_source_nmf(np.concatenate([_rectify(loaded[s].emg) for s in sources])); raw={}
    for s in sources:
        z=loaded[s]; scores=syn3.project_basis(_rectify(z.emg[z.emg_trial_ids<10]),basis); w,i=syn3.fit_all_units(scores,z.rates); raw[s]=syn3.carrier_from_encoding(w,i)
    ...
    if path:
        ...
        path.with_suffix(".json").write_text(json.dumps({"split_id":SPLIT_ID,"sources":list(sources),"fit":"source EMG [0,310), source M10 only"},sort_keys=True))
...
def materialize_sources(*, cache: Path|None=None) -> dict[str,Any]:
    records=_records(SOURCE_SESSIONS); support={s:_slice_trials(records[s],*SUPPORT) for s in SOURCE_SESSIONS}; train_raw={s:_slice_trials(records[s],*TRAIN) for s in SOURCE_SESSIONS}; val_raw={s:_slice_trials(records[s],*VAL) for s in SOURCE_SESSIONS}; train,val=_dataset(train_raw,support),_dataset(val_raw,support); _assert_reset(train,TRAIN); _assert_reset(val,VAL)
    carriers,rsyn3=_source_carriers(cache); packed=_carrier_pack("source"); carriers={s:(packed[s] if packed else carriers[s]) for s in SOURCE_SESSIONS}; return {"sources":SOURCE_SESSIONS,"train":train,"val":val,"banks":{s:_bank(s,carriers[s]) for s in SOURCE_SESSIONS},"calib":{s:np.ascontiguousarray(train.calib_trialized_neural_features[s][:10],dtype=np.float32) for s in SOURCE_SESSIONS},"rsyn3":rsyn3,"basis_scope":"chronological sources only: EMG [0,310), carrier M10, source-only normalizer"}

def materialize_target(target: str, source: Mapping[str,Any], arm: str) -> dict[str,Any]:
    if target not in TARGET_SESSIONS: raise ValueError(f"target must be one of {TARGET_SESSIONS}")
    record=_records((target,))[target]
    if arm == "Z_NONE":
        q=_slice_trials(record,*QUERY)
        class QueryOnly:
            window_size=100
            def __init__(self):
        ...
        return {"target":QueryOnly(),"target_bank":_bank(target,np.zeros((64,4),dtype=np.float32))}
    ds=_dataset({target:_slice_trials(record,*QUERY)},{target:_slice_trials(record,*SUPPORT)}); _assert_reset(ds,QUERY)
    calib=np.ascontiguousarray(ds.calib_trialized_neural_features[target][:10],dtype=np.float32)
    if arm == "B_ACTIVITY_ONLY": return {"target":ds,"target_bank":_bank(target,np.zeros((64,4),dtype=np.float32)),"target_calib":calib}
    from tfpd_exploration.src.m1_emg_syn3_fcm_v1 import data as raw_data, syn3
    packed=_carrier_pack("target");
    if packed is not None: carrier=packed[target]
    else:
        frozen=source["rsyn3"]; z=raw_data.load_support_bins(nwb_path(target),emg_trial_stop=10,neural_trial_stop=10); scores=syn3.nnls_activations(syn3.apply_scale(_rectify(z.emg),frozen["basis_scale"]),frozen["basis_dictionary"]); w,i=syn3.fit_all_units(scores,z.rates); carrier=np.ascontiguousarray(syn3.normalize_carriers(syn3.carrier_from_encoding(w,i),frozen["normalizer_mean"],frozen["normalizer_scale"]),dtype=np.float32)
    return {"target":ds,"target_bank":_bank(target,carrier),"target_calib":calib}
