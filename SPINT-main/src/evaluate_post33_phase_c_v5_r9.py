#!/usr/bin/env python3
"""Exact selected-checkpoint SPINT outer evaluator worker for Phase-C v5.
...
co-locates ``model.net`` and the plain cached identity on CUDA, binds checks to
the profiler's actual CUDA input, and revalidates cache bytes after profiling.
All checkpoint, score, chronology, label, query, and payload behavior below is
intentionally identical to v4.
"""
...
from sua_exploration.mc_maze.m2_native_post33_phase_c_v4 import (
    ...
    validate_selected_checkpoint_snapshot, write_json_exclusive,
)
...
def main() -> None:
    ...
    try:
        ...
        # the deployment net/cache/device/mode boundary before the profiler
        # owns the same CPU->CUDA B=1 transfer as the historical evaluator.
        neural_cpu = datamodule.deployment_neural_window()
        prepared = prepare_cached_online_b1_after_trainer_test_v5(
            model, arm="spint", neural_cpu=neural_cpu,
        )
