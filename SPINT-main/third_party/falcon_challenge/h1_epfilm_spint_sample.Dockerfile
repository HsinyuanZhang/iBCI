...
ARG FILM_STATE_SHA256
ARG BATCH_SIZE=8

ADD ./local_data/h1_epfilm_evalai_v1/decoder.pt /data/decoder.pt
ADD ./third_party/ /third_party/
ADD ./src/ /src/
ADD ./third_party/falcon_challenge/h1_epfilm_spint_sample.py /decode.py

LABEL org.opencontainers.image.title="H1 EP-FILM SPINT" \
      org.opencontainers.image.description="Frozen all-source C1/M3 carrier-aware H1 decoder with exactly-M3 EP-FILM calibration-profile modulation and MAT7 readout calibration" \
      ibci.h1.package.sha256="${PACKAGE_SHA256}" \
      ibci.h1.checkpoint.sha256="${CHECKPOINT_SHA256}" \
      ibci.h1.film_state.sha256="${FILM_STATE_SHA256}" \
