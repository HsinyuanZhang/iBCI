#!/usr/bin/env bash
# CTXV2 Stage A GPU launch orchestrator — DO NOT RUN FROM AUTOMATED AGENTS.
#
# A human or supervising agent must execute this script only after both immutable
# receipts exist and have been independently verified:
#   - pilot_artifacts/h1_ctxv2_19250108/CTXV2_STAGE_A_PREFLIGHT_v1.json
#   - pilot_artifacts/h1_ctxv2_19250108/CTXV2_STAGE_A_CONFIG_PARITY_v1.json
#
# Refuses to overwrite existing logs or checkpoint directories.
set -euo pipefail
...
preflight_receipt="${artifact_root}/CTXV2_STAGE_A_PREFLIGHT_v1.json"
...
print_train_cmd() {
  ...
  echo "CUDA_VISIBLE_DEVICES=${gpu} PYTHONNOUSERSITE=1 PYTHONPATH=${project_root} ${python_bin} ${project_root}/src/train.py experiment=${experiment} hydra.run.dir=${run_dir} paths.output_dir=${run_dir} seed=42 2>&1 | tee ${log_path}"
}
...
echo "CTXV2 Stage A launch plan (two-wave, two-GPU)"
...
tmux new-session -d -s ctxv2_ctx \
  "cd ${project_root} && CUDA_VISIBLE_DEVICES=0 PYTHONNOUSERSITE=1 PYTHONPATH=. ${python_bin} src/train.py experiment=${ctx_experiment} hydra.run.dir=${ctx_run_dir} paths.output_dir=${ctx_run_dir} seed=42 2>&1 | tee ${ctx_log}"

tmux new-session -d -s ctxv2_hse5 \
  "cd ${project_root} && CUDA_VISIBLE_DEVICES=1 PYTHONNOUSERSITE=1 PYTHONPATH=. ${python_bin} src/train.py experiment=${hse5_experiment} hydra.run.dir=${hse5_run_dir} paths.output_dir=${hse5_run_dir} seed=42 2>&1 | tee ${hse5_log}"

tmux new-session -d -s ctxv2_zero \
  "cd ${project_root} && while tmux has-session -t ctxv2_ctx 2>/dev/null || tmux has-session -t ctxv2_hse5 2>/dev/null; do sleep 30; done; CUDA_VISIBLE_DEVICES=0 PYTHONNOUSERSITE=1 PYTHONPATH=. ${python_bin} src/train.py experiment=${zero_experiment} hydra.run.dir=${zero_run_dir} paths.output_dir=${zero_run_dir} seed=42 2>&1 | tee ${zero_log}"
