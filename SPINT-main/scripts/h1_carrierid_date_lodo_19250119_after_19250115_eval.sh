#!/usr/bin/env bash
# Explicit, fail-closed handoff from 19250115 terminal evaluation to the
# independently receipt-bound 19250119 source-only pair queue.
set -euo pipefail
...
readonly PREDECESSOR_PAIR_SESSION='h1_date_lodo_future_19250115_pair_s42_e49'
readonly PREDECESSOR_HELDOUT_SESSION='h1_date_lodo_19250115_heldout_once_after_pair_v1'
readonly EVALUATION_RECEIPT="$CANONICAL_REPO/pilot_artifacts/h1_carrierid_date_lodo_phase2/terminal_evaluations/H1_CARRIERID_DATE_LODO_PHASE2_19250115_HS_HC_TERMINAL_EVALUATION_v1.json"
...
readonly NEXT_OUTER_DATE='19250119'
readonly POLL_SECONDS=30

fail() {
    printf '[%s] H1 19250119 handoff FAIL-CLOSED: %s\n' "$(date -Is)" "$*" >&2
    exit 1
}
...
# Absence alone is not evidence that the one-shot evaluator ever ran.  A
# remain-on-exit pane is accepted as clean completion, while a normal tmux
# lifecycle may remove a session only after we observed a valid predecessor.
saw_valid_predecessor_state=0
printf '[%s] waiting for 19250115 held-out one-shot watcher\n' "$(date -Is)"
while true; do
    if ! tmux has-session -t "$PREDECESSOR_HELDOUT_SESSION" 2>/dev/null; then
        [[ "$saw_valid_predecessor_state" -eq 1 ]] \
            || fail "held-out watcher disappeared before an active or clean-dead state was observed"
        break
    fi
...
# This parses only the canonical immutable receipt and never inspects a model
# output, score, target record, or behavioral label.
...
if body.get("status") != "PASS_H1_CARRIERID_DATE_LODO_PHASE2_19250115_HS_HC_EVALUATED":
    raise SystemExit("canonical evaluation receipt status drift")
if body.get("outer_date") != "19250115":
...
# Only these two predecessor names may be cleaned, and only after receipt
# validation proves the held-out path completed without deployment updates.
...
printf '[%s] canonical 19250115 evaluation receipt accepted; handing off 19250119 source-only pair\n' "$(date -Is)"
cd "$CANONICAL_REPO"
"$PYTHON" "$FUTURE_QUEUE" \
    --launch \
...
# Queue launch returns only after the date-scoped source-pair tmux session has
# been created.  Attach the next target-free watcher chain immediately.
