#!/usr/bin/env python3
"""Terminal evaluator for the matched H1 M=4 EB fold-0 exploratory pilot."""
...
from src.h1_m4_eb_pilot_contract import (
...
def _validate_resolved_config(config_path: Path, arm: str) -> Any:
    ...
    for key, expected in required.items():
        if config.get(key) != expected:
            raise ValueError(f"{arm} resolved config drift at {key}")
    if (
        config.pilot.arm != arm
        or bool(config.pilot.train_residual) != (arm == "joint")
        or str(config.pilot.fold_date) != FOLD0_DATE
        ...
        or config.callbacks.fixed_epoch50.monitor is not None
        or int(config.callbacks.fixed_epoch50.every_n_epochs) != 50
        or float(config.model.optimizer.lr) != 5.0e-5
    ):
        raise ValueError(f"{arm} resolved config violates fixed pilot contract")
    return config
...
def _evaluate_model(model, dataset: H1M4EBStrictTargetDataset, device: torch.device) -> dict[str, Any]:
    ...
    if not batch_sizes or sum(batch_sizes) != len(dataset):
        raise RuntimeError("terminal evaluator omitted target samples")
    expected_last = len(dataset) % 32 or 32
    if batch_sizes[-1] != expected_last:
        raise RuntimeError("terminal evaluator did not preserve the final remainder batch")
    prediction = np.concatenate(predictions, axis=0)
    target = np.concatenate(targets, axis=0)
    metric = variance_weighted_r2(target, prediction)
    ...
    if set(sessions) != set(H1_M4_FOLD0_TARGET) or sum(counts.values()) != len(dataset):
        raise RuntimeError("terminal target concatenation omitted/reordered a fold-0 recording")
    return {
        **metric,
        "samples": len(dataset),
...
def evaluate_terminal_pilot(
    *,
    ...
    """Bind both terminal checkpoints before opening either target NWB."""
    ...
    # Rebuild the source-only manifest and plan after both checkpoint files are
    # bound, still before opening target.  This catches input/cache drift.
    source_module = H1M4EBFold0DataModule(
        task="h1",
    ...
    if source_module.pilot_manifest_sha256 != paired["source_manifest_sha256"]:
        raise ValueError("runtime source manifest does not match both terminal checkpoints")
    ...
    full = H1M4EBStrictTargetDataset(target_records, source_module.plan, "full")
    ...
    for name, dataset in variants.items():
        if dataset.window_indices_sha256 != full.window_indices_sha256 or dataset.support_and_carrier_hashes() != support_hashes:
            raise RuntimeError(f"{name} intervention changed support/query order")
    ...
    if len({base_result["query_window_indices_sha256"], *(row["query_window_indices_sha256"] for row in joint_results.values())}) != 1:
        raise RuntimeError("base/joint/intervention query order drift")
    ...
    receipt = {
        "schema": "h1_m4_eb_fold0_exploratory_terminal_gate_v1",
        "status": "PASS_H1_M4_EB_FOLD0_EXPANSION_AUTHORIZED_ONLY" if gate_pass else "STOP_H1_M4_EB_FOLD0_EXPLORATORY_GATE_FAILED",
        "claim_status": "exploratory expansion authorization only; never a decoding or EB attachment claim",
        ...
        "checkpoints": {
            ...
            "joint": {
                ...
                "sha256": sha256_file(joint_checkpoint_path),
                "config_sha256": sha256_file(joint_config_path),
                "metadata": joint_meta,
            },
            "paired": paired,
        },
        ...
        "data_scope": {
            "opened": "13 public held-in-calib NWBs only (11 source, then 2 target)",
