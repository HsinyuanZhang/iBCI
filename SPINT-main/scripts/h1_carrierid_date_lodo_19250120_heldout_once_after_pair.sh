#!/usr/bin/env bash
# Final one-shot 19250120 outer-date evaluator after its source-only pair.
# The protocol ends after this immutable evaluation receipt; no future queue is
...
fail() { printf '[%s] H1 %s final held-out handoff FAIL-CLOSED: %s\n' "$(date -Is)" "$OUTER_DATE" "$*" >&2; exit 1; }
...
printf '[%s] waiting for final %s source pair tmux session to disappear\n' "$(date -Is)" "$OUTER_DATE"
...
printf '[%s] H1 %s final canonical one-shot terminal evaluation completed; protocol stops here\n' "$(date -Is)" "$OUTER_DATE"
