"""Static contract for the 19250119-evaluation -> 19250120 source queue handoff."""
...
def _source() -> str:
    return (ROOT / "scripts/h1_carrierid_date_lodo_19250120_after_19250119_eval.sh").read_text(encoding="utf-8")


def test_requires_an_observed_active_or_clean_dead_predecessor_lifecycle() -> None:
    source = _source()
    assert "set -euo pipefail" in source
    assert "PREDECESSOR_HELDOUT_SESSION='h1_date_lodo_19250119_heldout_once_after_pair_v1'" in source
    assert "PREDECESSOR_PAIR_SESSION='h1_date_lodo_future_19250119_pair_s42_e49'" in source
    assert "NEXT_OUTER_DATE='19250120'" in source
...
def test_immutable_zero_update_receipt_narrow_cleanup_and_only_19250120_launch() -> None:
    source = _source()
    assert "terminal_evaluations/H1_CARRIERID_DATE_LODO_PHASE2_19250119_HS_HC_TERMINAL_EVALUATION_v1.json" in source
    assert "h1_carrierid_date_lodo_phase2_terminal_evaluation_v1" in source
    assert "PASS_H1_CARRIERID_DATE_LODO_PHASE2_19250119_HS_HC_EVALUATED" in source
    assert 'body.get("outer_date") != "19250119"' in source
    ...
    for forbidden_date in ("19250108", "19250113", "19250115"):
        assert forbidden_date not in source


def test_no_performance_or_target_data_gate_is_present() -> None:
    source = _source()
    for forbidden in (
