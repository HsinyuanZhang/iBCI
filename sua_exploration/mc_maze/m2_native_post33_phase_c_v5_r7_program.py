"""Append-only source closure for the v5/r7 device-repair rollover.

The r7 root is deliberately a new authorization lineage, not a modification of
...
and the r7 worker plumbing in one CPU-verifiable source map.
"""
...
ROLLOVER_ID = "PHASE_C_V5_R7_DEVICE_REPAIR"
PROGRAM_SCHEMA = "m2_post33_phase_c_v5_r7_program_receipt_v1"
ANCHOR_SCHEMA = "m2_post33_phase_c_v5_r7_public_anchor_v1"
R7_RECEIPT_ROOT = (
    ROOT / "sua_exploration/results/m2_native_post33_phase_c_v5_r7_launch_receipts_20260805"
)
R7_CELL_ROOT = ROOT / "sua_exploration/results/m2_native_post33_phase_c_v5_r7_cells_20260805"
PUBLIC_KEY = ROOT / "sua_exploration/configs/m2_native_post33_phase_c_v5_r7_root_ed25519_public.pem"
PUBLIC_ANCHOR = R7_RECEIPT_ROOT / "authority/public_anchor.json"
R6D_RETIREMENT = (
    ROOT
...
# The v4 closure supplies the complete frozen model/config/source graph.  r7
# removes every old executable authority path and substitutes only audited r7
...
R7_RUNTIME_SOURCES = frozenset(
    {
        "SPINT-main/src/train_post33_phase_c_v5_r7.py",
        "SPINT-main/src/evaluate_post33_phase_c_v5.py",
        "SPINT-main/src/evaluate_post33_phase_c_v5_r7.py",
        "streaming_calibration_exp/src/train_post33_phase_c_v5_r7.py",
        "streaming_calibration_exp/src/evaluate_post33_phase_c_v5.py",
        "streaming_calibration_exp/src/evaluate_post33_phase_c_v5_r7.py",
        "sua_exploration/mc_maze/m2_native_post33_deployment_device_prep_v5.py",
        "sua_exploration/mc_maze/m2_native_post33_phase_c_v5_r7_program.py",
        "sua_exploration/mc_maze/m2_native_post33_authorization_v5_r7.py",
        "sua_exploration/scripts/evaluate_m2_native_post33_phase_c_v5_r7.py",
        "sua_exploration/scripts/run_m2_native_post33_phase_c_v5_r7_cell_pipeline.py",
        "sua_exploration/scripts/run_m2_native_post33_phase_c_v5_r7_matrix.py",
        "sua_exploration/scripts/m2_native_post33_phase_c_v5_r7_prepare_stage_a.py",
        "sua_exploration/scripts/verify_m2_native_post33_phase_c_v5_r7.py",
    }
)
PROGRAM_SOURCE_PATHS = tuple(
    sorted((set(PROGRAM_EXPECTED_CLOSURE) - _REMOVE_FROM_V4_CLOSURE) | R7_RUNTIME_SOURCES)
)
...
def validate_r6d_retirement() -> dict[str, Any]:
    ...
    if (
        ...
        raise PermissionError("r6d retirement does not require a fresh r7 lineage")
...
def validate_public_anchor() -> dict[str, Any]:
    """Validate the fixed r7 public key path and append-only anchor receipt."""
    pem = require_canonical_regular_file(PUBLIC_KEY, within=ROOT)
    anchor_path, anchor = _read_json(PUBLIC_ANCHOR, "r7 public anchor")
    ...
    if set(anchor) != expected_keys or anchor.get("schema") != ANCHOR_SCHEMA:
        raise PermissionError("r7 public anchor schema mismatch")
    if (
        ...
        raise PermissionError("r7 public anchor identity/scope mismatch")
...
def build_r7_program_receipt() -> dict[str, Any]:
    """Build the unsigned, source-only r7 receipt after the public anchor exists."""
    ...
    return {
        ...
        "r7_receipt_root": str(R7_RECEIPT_ROOT.resolve()),
...
def validate_r7_program_receipt(path: str | Path) -> Mapping[str, Any]:
    """Fail closed on source, trust-anchor, or old-lineage drift."""
    receipt_path, receipt = _read_json(path, "r7 program receipt")
    required = {
        "schema", "rollover_id", "protocol_id", "phase_id", "absolute_workspace_root",
        "r7_receipt_root", "r7_cell_root", "public_anchor", "r6d_retirement",
    ...
    if set(receipt) != required or receipt.get("schema") != PROGRAM_SCHEMA:
        raise ValueError("r7 program receipt schema/exact-set mismatch")
    if (
        ...
        or receipt.get("r7_receipt_root") != str(R7_RECEIPT_ROOT.resolve())
        or receipt.get("r7_cell_root") != str(R7_CELL_ROOT.resolve())
        or any(receipt.get(field) is not False for field in ("gpu_used", "formal_data_accessed", "score_data_accessed"))
    ):
        raise ValueError("r7 program receipt identity/scope mismatch")
    if receipt.get("public_anchor") != validate_public_anchor():
        raise ValueError("r7 program receipt public-anchor drift")
    if receipt.get("r6d_retirement") != validate_r6d_retirement():
        raise ValueError("r7 program receipt retirement binding drift")
    eof, deep, phase_a = _validate_upstream_receipts()
    if (
        ...
        raise ValueError("r7 program receipt upstream closure drift")
    rows = source_rows()
    if (
        receipt.get("source_map") != rows
        or receipt.get("source_map_entry_count") != len(rows)
        or receipt.get("source_map_sha256") != sha256_json(rows)
    ):
        raise ValueError("r7 program receipt source-map drift/omission/substitution")
    del receipt_path
    return receipt
