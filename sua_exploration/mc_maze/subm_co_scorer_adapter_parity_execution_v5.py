"""Authorization-first execution support for corrected v5 schema-bridge parity.

Only standard-library and cryptography imports occur at module import. The
corrected v5 helper, v3 helper, Torch, NumPy, PyNWB, checkpoints, normalizers,
and NWB owners remain unreachable until a v5 signature, pin, runtime, output,
and nonce-claim gate has succeeded.
"""
...
AUTHORIZATION_ENVELOPE_SCHEMA = "dandi_000688_subc_parity_schema_bridge_execution_authorization_envelope_v5"
AUTHORIZATION_STATUS = "AUTHORIZED_FOR_ONE_CONSUMED_SUBC_SCHEMA_BRIDGE_PARITY_EXECUTION"
...
class ParityV5AuthorizationError(PermissionError):
    """Authorization is invalid before any data/model runtime may be imported."""


class ParityV5ExecutionError(RuntimeError):
    """A post-authorization parity or immutable output action failed."""
...
class PreAuthorizationAudit:
    ...
    corrected_helper_imports: int = 0
    v3_torch_owner_imports: int = 0
    ...
    def zero_data_access(self) -> bool:
        return (
            self.corrected_helper_imports == 0
            and self.v3_torch_owner_imports == 0
...
def _regular(path: Path, label: str) -> Path:
    try:
        ...
        raise ParityV5AuthorizationError(f"{label} missing") from exc
    if not resolved.is_file() or resolved.is_symlink():
        raise ParityV5AuthorizationError(f"{label} non-regular or symlinked")
    return resolved
...
def _parse_time(value: Any, label: str) -> datetime:
    if not isinstance(value, str):
        raise ParityV5AuthorizationError(f"{label} must be timezone-aware ISO-8601")
    try:
        parsed = datetime.fromisoformat(value.replace("Z", "+00:00"))
    except ValueError as exc:
        raise ParityV5AuthorizationError(f"{label} malformed") from exc
    if parsed.tzinfo is None:
        raise ParityV5AuthorizationError(f"{label} lacks timezone")
    return parsed.astimezone(timezone.utc)


def _assert_preimport_boundary() -> None:
    if os.environ.get("CUDA_VISIBLE_DEVICES", "") != "":
        raise ParityV5AuthorizationError("CUDA_VISIBLE_DEVICES must be empty before v5 authorization")
    ...
    if any(name == item or name.startswith(item + ".") for item in forbidden for name in loaded):
        raise ParityV5AuthorizationError("corrected helper, v3 helper, or Torch imported before authorization")
...
def _within(path: Path, parent: Path, label: str) -> Path:
    ...
    try:
        ...
        raise ParityV5AuthorizationError(f"{label} escapes v5 dedicated namespace") from exc
    return candidate
...
def _verify_sources(policy: Mapping[str, Any], audit: PreAuthorizationAudit) -> None:
    ...
    if not isinstance(pins, Mapping) or not pins:
        raise ParityV5AuthorizationError("v5 source pins missing")
    for raw_path, expected in pins.items():
        if not isinstance(raw_path, str) or not isinstance(expected, str):
            raise ParityV5AuthorizationError("v5 source-pin schema invalid")
        path = _regular(Path(raw_path), "source pin")
        if sha256_file(path) != expected:
            raise ParityV5AuthorizationError(f"source drift: {path}")
...
def _verify_key(policy: Mapping[str, Any], audit: PreAuthorizationAudit) -> Ed25519PublicKey:
    ...
    if not isinstance(pin, Mapping) or set(pin) != {"path", "sha256", "bytes"}:
        raise ParityV5AuthorizationError("public-key policy malformed")
    path = _regular(Path(str(pin["path"])), "dedicated v5 root public key")
    audit.public_key_files_read += 1
    if path.stat().st_size != pin["bytes"] or sha256_file(path) != pin["sha256"]:
        raise ParityV5AuthorizationError("dedicated root public-key pin drift")
    try:
        key = serialization.load_pem_public_key(path.read_bytes())
    except ValueError as exc:
        raise ParityV5AuthorizationError("public-key PEM malformed") from exc
    if not isinstance(key, Ed25519PublicKey):
        raise ParityV5AuthorizationError("dedicated root key is not Ed25519")
    return key
...
def _load_signed(
    ...
    try:
        ...
        raise ParityV5AuthorizationError("authorization canonical JSON malformed") from exc
    if not isinstance(envelope, dict) or _canonical_bytes(envelope) != raw:
        raise ParityV5AuthorizationError("authorization must be exact canonical JSON")
    if set(envelope) != {"schema", "authorization"} or envelope.get("schema") != AUTHORIZATION_ENVELOPE_SCHEMA:
        raise ParityV5AuthorizationError("authorization v5 envelope mismatch")
    body = envelope.get("authorization")
    if not isinstance(body, dict):
        raise ParityV5AuthorizationError("authorization body missing")
    try:
        signature = base64.b64decode(sig_file.read_bytes(), validate=True)
    except (OSError, ValueError) as exc:
        raise ParityV5AuthorizationError("detached signature malformed") from exc
    audit.signature_files_read += 1
    if len(signature) != 64:
        raise ParityV5AuthorizationError("detached signature length mismatch")
    try:
        key.verify(signature, raw)
    except InvalidSignature as exc:
        raise ParityV5AuthorizationError("invalid detached v5 signature") from exc
...
def _validate_body(
    ...
    *,
    ...
    if (
        ...
        raise ParityV5AuthorizationError("v5 forbidden capability flag enabled")
    if body.get("bindings") != policy.get("authorization_bindings"):
        raise ParityV5AuthorizationError("authorization v5 binding drift")
    if body.get("execution_policy_sha256") != policy.get("execution_policy_sha256"):
        raise ParityV5AuthorizationError("authorization v5 policy SHA drift")
    if body.get("output_root") != str(output_root.resolve()):
        raise ParityV5AuthorizationError("authorization output-root mismatch")
    ...
    if output_root.resolve() == output_parent or output_root.exists():
        raise ParityV5AuthorizationError("output root must be a new unique v5 child")
    if runtime_identity() != policy.get("runtime_identity"):
        raise ParityV5AuthorizationError("SPINT Python/host drift")
    if policy.get("cpu_policy") != CPU_POLICY:
        raise ParityV5AuthorizationError("v5 CPU policy malformed")
...
def _claim(
    *,
    ...
    payload = {
        "schema": "dandi_000688_subc_schema_bridge_parity_nonce_claim_v5",
    ...
    try:
        ...
        raise ParityV5AuthorizationError("v5 nonce already atomically claimed") from exc
    ...
    if stat.S_IMODE(destination.stat().st_mode) != 0o444:
        raise ParityV5AuthorizationError("v5 nonce-claim mode failure")
...
def _preauthorize_and_claim_with_policy(
    *,
    ...
    """Test seam; production uses execute_from_fixed_prelaunch only."""
...
def _set_cpu_environment() -> None:
    if os.environ.get("CUDA_VISIBLE_DEVICES", "") != "":
        raise ParityV5AuthorizationError("CUDA_VISIBLE_DEVICES drifted after authorization")
...
def _write_immutable(path: Path, value: Mapping[str, Any]) -> str:
    ...
    if stat.S_IMODE(path.stat().st_mode) != 0o444:
        raise ParityV5ExecutionError(f"immutable mode failed: {path.name}")
...
def _execute_after_authorization_with_policy(
    *,
    ...
    try:
        ...
        raise ParityV5ExecutionError("v5 output collision after nonce claim") from exc
    from sua_exploration.mc_maze.subm_co_scorer_adapter_parity_v5 import (
        concrete_parity_after_future_authorization_v5,
    )

    result = concrete_parity_after_future_authorization_v5(REPO_ROOT)
    ...
    environment = {
        "schema": "dandi_000688_subc_schema_bridge_parity_environment_v5",
        "runtime_identity": runtime_identity(),
        "cpu_policy": policy["cpu_policy"],
        "thread_environment": CPU_ENVIRONMENT,
    ...
    receipt = {
        "schema": "dandi_000688_subc_schema_bridge_parity_execution_receipt_v5",
        ...
        "runtime_imports": {
            "corrected_v5_helper_imported_after_authorization": True,
            "v3_torch_owner_imports_after_authorization": True,
        },
        "data_asset_access": {
    ...
    seal = {
        "schema": "dandi_000688_subc_schema_bridge_parity_execution_seal_v5",
...
def execute_from_fixed_prelaunch(
    *,
    ...
    """Production entrypoint: only a stored 0444 v5 prelaunch supplies policy."""
    if root.resolve() != REPO_ROOT.resolve():
        raise ParityV5AuthorizationError("v5 production runner requires fixed repository root")
    from sua_exploration.scripts.write_dandi688_subm_co_scorer_adapter_parity_prelaunch_v5 import (
        canonical_bytes,
    ...
    actual_policy_sha = hashlib.sha256(canonical_bytes(policy_without_sha)).hexdigest()
    if (
        policy.get("execution_policy_sha256") != stored.get("execution_policy_sha256")
        or stored.get("execution_policy_sha256") != actual_policy_sha
    ):
        raise ParityV5AuthorizationError("stored v5 policy hash mismatch")
