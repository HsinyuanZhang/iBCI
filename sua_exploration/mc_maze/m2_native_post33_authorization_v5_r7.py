"""Fresh, Stage-A-only detached-Ed25519 capability contract for v5/r7.
...
from sua_exploration.mc_maze.m2_native_post33_phase_c_v5_r7_program import (
    PUBLIC_KEY,
    R7_CELL_ROOT,
    R7_RECEIPT_ROOT,
    ROLLOVER_ID,
...
EVALUATOR = ROOT / "sua_exploration/scripts/evaluate_m2_native_post33_phase_c_v5_r7.py"
AUTH_SCHEMA = "m2_post33_phase_c_v5_r7_gpu_authorization_v1"
ENVELOPE_SCHEMA = "m2_post33_phase_c_v5_r7_signed_authorization_envelope_v1"
PORTABLE_SCHEMA = "m2_post33_phase_c_v5_r7_portable_manifest_v1"
SHARD_SCHEMA = "m2_post33_phase_c_v5_r7_shard_manifest_v1"
CLAIM_SCHEMA = "m2_post33_phase_c_v5_r7_authorization_nonce_claim_v1"
CAPABILITY_EVIDENCE_SCHEMA = "m2_post33_phase_c_v5_r7_execution_capability_evidence_v1"
SELECTOR_POLICY_SCHEMA = "m2_post33_phase_c_v5_r7_fresh_selector_policy_v1"
MAX_VALIDITY_SECONDS = 30 * 3600
CAPABILITY_SCOPE_CELL_EXECUTION = "cell_execution"
CAPABILITY_ENVIRONMENT = {
    "authorization_path": "M2_POST33_PHASE_C_V5_R7_AUTHORIZATION",
    "signature_path": "M2_POST33_PHASE_C_V5_R7_AUTHORIZATION_SIGNATURE",
    "phase_c_program_receipt_path": "M2_POST33_PHASE_C_V5_R7_PROGRAM_RECEIPT",
    "portable_manifest_path": "M2_POST33_PHASE_C_V5_R7_PORTABLE_MANIFEST",
    "shard_manifest_path": "M2_POST33_PHASE_C_V5_R7_SHARD_MANIFEST",
    "cost_supplement_path": "M2_POST33_PHASE_C_V5_R7_COST_SUPPLEMENT",
}
...
def _parse_utc(value: Any, label: str) -> datetime:
    if not isinstance(value, str):
        raise PermissionError(f"r7 authorization {label} must be an ISO-8601 string")
    parsed = datetime.fromisoformat(value.replace("Z", "+00:00"))
    if parsed.tzinfo is None:
        raise PermissionError(f"r7 authorization {label} must be timezone-aware")
...
def _read_json(path: str | Path, label: str) -> tuple[Path, Mapping[str, Any]]:
    ...
    try:
        ...
        raise PermissionError(f"r7 {label} is invalid JSON") from exc
    if not isinstance(payload, Mapping):
        raise PermissionError(f"r7 {label} must be a mapping")
...
def _verify_metadata(metadata: Any, expected: str | Path, label: str) -> Path:
    if not isinstance(metadata, Mapping) or set(metadata) != {
        ...
        raise PermissionError(f"r7 {label} metadata exact set mismatch")
    target = require_canonical_regular_file(expected)
    if dict(metadata) != file_metadata(target):
        raise PermissionError(f"r7 {label} substitution/hash drift")
    return target
...
def validate_observed_host(shard: Mapping[str, Any], *, observed_host: str | None = None) -> str:
    ...
    if not isinstance(observed, str) or not observed or shard.get("host_id") != observed:
        raise PermissionError("r7 signed shard host_id differs from observed hostname")
    return observed
...
def validate_observed_gpu(
    ...
    if not isinstance(raw, str) or not raw:
        raise PermissionError("r7 signed GPU requires exactly one visible physical device")
    visible = [part.strip() for part in raw.split(",")]
    if len(visible) != 1 or not visible[0] or raw != visible[0] or visible[0] != shard.get("gpu_id"):
        raise PermissionError("r7 CUDA_VISIBLE_DEVICES does not equal signed physical GPU")
    return {
...
def validate_r7_portable_transfer_manifest(
    ...
    *,
    ...
    if set(manifest) != required or manifest.get("schema") != PORTABLE_SCHEMA:
        raise PermissionError("r7 portable manifest schema/exact-set mismatch")
    if (
        ...
        raise PermissionError("r7 portable manifest identity/path/selector mismatch")
    program = _verify_metadata(manifest.get("program"), manifest.get("program", {}).get("canonical_path", ""), "program")
    validated_program = validate_r7_program_receipt(program)
    for field, program_field in (
        ...
        if metadata != validated_program.get(program_field):
            raise PermissionError(f"r7 portable {field} does not match program receipt")
    ...
    if manifest.get("r6d_retirement") != validated_program.get("r6d_retirement"):
        raise PermissionError("r7 portable r6d-retirement binding mismatch")
    validate_r6d_retirement()
    del path
    return manifest


def validate_r7_shard_manifest(
    shard_path: str | Path,
    *,
    ...
    if set(shard) != required or shard.get("schema") != SHARD_SCHEMA:
        raise PermissionError("r7 shard manifest schema/exact-set mismatch")
...
def _verify_signature(authorization_path: str | Path, signature_path: str | Path) -> Mapping[str, Any]:
    ...
    if anchor["public_key"] != file_metadata(pem):
        raise PermissionError("r7 public anchor/Pem mismatch")
    key = serialization.load_pem_public_key(pem.read_bytes())
    if not isinstance(key, Ed25519PublicKey):
        raise PermissionError("r7 public anchor is not Ed25519")
    try:
        key.verify(base64.b64decode(signature_file.read_bytes(), validate=True), authorization_file.read_bytes())
    except (InvalidSignature, ValueError) as exc:
        raise PermissionError("invalid r7 detached Ed25519 authorization") from exc
    ...
    if set(envelope) != {"schema", "authorization"} or envelope.get("schema") != ENVELOPE_SCHEMA:
        raise PermissionError("r7 authorization envelope mismatch")
    auth = envelope.get("authorization")
    if not isinstance(auth, Mapping):
        raise PermissionError("r7 authorization body must be a mapping")
    return auth
...
def verify_signed_authorization(
    ...
    *,
    ...
    if set(auth) != required or auth.get("schema") != AUTH_SCHEMA:
        raise PermissionError("r7 authorization schema/exact-set mismatch")
    if (
        ...
        raise PermissionError("r7 authorization scope/identity mismatch")
    if not isinstance(auth.get("authorization_id"), str) or not auth["authorization_id"]:
        raise PermissionError("r7 authorization id missing")
    nonce = auth.get("single_use_nonce")
    if not isinstance(nonce, str) or re.fullmatch(r"[0-9a-f]{64}", nonce) is None:
        raise PermissionError("r7 authorization nonce must be 256-bit lowercase hex")
    ...
    if not (issued <= current <= expires) or (expires - issued).total_seconds() > MAX_VALIDITY_SECONDS:
        raise PermissionError("r7 authorization expired/not-yet-valid/exceeds 30h")
    program = _verify_metadata(auth.get("phase_c_program_receipt"), phase_c_program_receipt_path, "program receipt")
    validate_r7_program_receipt(program)
    ...
    shard = validate_r7_shard_manifest(shard_file, portable_manifest_path=portable, cell_root=cell_root)
    if (
        ...
        raise PermissionError("r7 authorization/shard scope mismatch")
...
def _execution_capability_inputs_from_environment() -> dict[str, Path]:
    ...
    for field, variable in CAPABILITY_ENVIRONMENT.items():
        ...
        if not value:
            raise PermissionError(f"missing r7 capability environment: {variable}")
...
def claim_authorization_nonce(
    *,
    ...
    if Path(root).resolve() != R7_CELL_ROOT.resolve():
        raise PermissionError("r7 capability may only claim its fresh cell root")
    ...
    if not isinstance(host, str) or not re.fullmatch(r"[A-Za-z0-9_.-]+", host):
        raise PermissionError("r7 nonce-claim hostname is unsafe")
...
def verify_authorization_nonce_claim(
    *,
    ...
    if claim != expected:
        raise PermissionError("r7 nonce claim substitution or wrong bound files")
    return path
    ...
    *,
    ...
    if destination.exists():
        ...
        if existing != expected:
            raise PermissionError("r7 execution capability evidence substitution")
        return destination
...
def require_cell_execution_capability(
    *,
    ...
    if Path(root).resolve() != R7_CELL_ROOT.resolve():
        raise PermissionError("r7 execution may only use the fresh r7 cell root")
    ...
    if (
        ...
        raise PermissionError("cell is outside r7 signed shard allowlist")
