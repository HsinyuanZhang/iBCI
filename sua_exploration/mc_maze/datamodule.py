...
class MCMazeDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        with NWBHDF5IO(str(nwb_path), "r") as io:
            ...
            # Standardize behavior (zero mean, unit variance per dimension)
            self.behavior_mean = binned_vel.mean(axis=0)
            self.behavior_std = binned_vel.std(axis=0)
            self.behavior_std[self.behavior_std < 1e-8] = 1.0
            binned_vel = (binned_vel - self.behavior_mean) / self.behavior_std

            logger.info(f"Binned vel (standardized) range: [{binned_vel.min():.2f}, {binned_vel.max():.2f}]")
