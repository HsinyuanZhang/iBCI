...
def state_dim_for_kinematic_dim(kinematic_dim: int) -> int:
...
def velocity_slice(kinematic_dim: int) -> slice:
...
def build_state_matrix(
    ...
    *,
    ...
    state_dim = state_dim_for_kinematic_dim(kinematic_dim)
    states = np.empty((vel.shape[0], state_dim), dtype=np.float64)
    states[:, :kinematic_dim] = position
    states[:, velocity_slice(kinematic_dim)] = vel
...
def assert_constant_term(state: np.ndarray, *, kinematic_dim: int) -> None:
    array = np.asarray(state, dtype=np.float64)
    idx = constant_index(state_dim_for_kinematic_dim(kinematic_dim))
    value = float(array[..., idx])
    require(value == CONSTANT_VALUE, f"constant term drifted to {value}")


def fix_constant_dynamics(A: np.ndarray, *, kinematic_dim: int) -> np.ndarray:
...
class KalmanParameters:
    ...
    def state_dim(self) -> int:
        return state_dim_for_kinematic_dim(self.kinematic_dim)
...
def fit_kalman_parameters(
    ...
    *,
    ...
    for row in states:
        assert_constant_term(row, kinematic_dim=kinematic_dim)
    ...
    try:
        ...
        A = fix_constant_dynamics(X1 @ X0.T @ inv_x0, kinematic_dim=kinematic_dim)
...
def kalman_filter_query(
    ...
    *,
    ...
    assert_constant_term(x, kinematic_dim=kinematic_dim)
...
def evaluate_session(
    ...
    *,
    ...
    vel_slice = velocity_slice(blocks.kinematic_dim)
...
def build_rt_fold_blocks(
    *,
    ...
    kinematic_dim = 2
    calibration_states = build_state_matrix(cov_p[calibration_bins], kinematic_dim=kinematic_dim)
    query_states = build_state_matrix(cov_p[query_bins], kinematic_dim=kinematic_dim)
    return SessionBlocks(
...
def adapter_dry_run_falcon_h1(*, repo_root: Path, data_dir: Path) -> dict[str, Any]:
    ...
    from sua_exploration.comparators.core import h1_ridge_family as h1
...
def build_h1_session_blocks(record: Any) -> SessionBlocks:
    from sua_exploration.comparators.core import h1_ridge_family as h1
    ...
    kinematic_dim = H1_VELOCITY_DIM
    calibration_states = build_state_matrix(split.velocity[calibration_bins], kinematic_dim=kinematic_dim)
    query_states = build_state_matrix(split.velocity[query_bins], kinematic_dim=kinematic_dim)
...
def load_all_h1_blocks(*, data_dir: Path, repo_root: Path) -> list[SessionBlocks]:
    pilot = _import_h1_pilot_module(repo_root)
    H1_M4_FOLD0_TARGET = pilot.H1_M4_FOLD0_TARGET
    load_target_records = pilot.load_target_records

    records = load_target_records(data_dir)
    return [build_h1_session_blocks(records[name]) for name in H1_M4_FOLD0_TARGET]
