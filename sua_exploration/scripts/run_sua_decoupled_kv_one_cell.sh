#!/usr/bin/env bash
# One-cell validation-only runner for the fresh coupled/decoupled K/V screen.
#
# All arms use aligned real T4 for the B3S identity encoder.  e_ts4 shuffles
# only the decoder's direct T4 key component inside the Lightning module; E is
# therefore identical to e_t4 at a shared checkpoint initialization.
set -euo pipefail

ROOT="/home/xinyuan/Work_host/SPINT"
PY="${PYTHON_BIN:-/home/xinyuan/miniconda3/envs/spint/bin/python}"
SCREEN_ID="${SCREEN_ID:-sua_t4_decoupled_kv_v1}"
...
case "$ARM" in
  coupled_t4)
    DECODER_ARGS=(--decoder_mode coupled)
    ;;
  kv_e_t4)
    DECODER_ARGS=(--decoder_mode decoupled --decoupled_key_mode e_t4 --decoupled_key_dim 32 --decoupled_value_dim 32)
    ;;
  kv_e_ts4)
    DECODER_ARGS=(--decoder_mode decoupled --decoupled_key_mode e_ts4 --decoupled_key_dim 32 --decoupled_value_dim 32)
    ;;
  kv_e_only)
    DECODER_ARGS=(--decoder_mode decoupled --decoupled_key_mode e_only --decoupled_key_dim 32 --decoupled_value_dim 32)
    ;;
  kv_x_only)
    DECODER_ARGS=(--decoder_mode decoupled --decoupled_key_mode x_only --decoupled_key_dim 32 --decoupled_value_dim 32)
    ;;
  *)
...
mkdir -p "$LOGS"
{
  ...
  echo "protocol=M_activity=30; M_T4=50; evaluation=trials[50:]; strict 27/6; no formal test"
