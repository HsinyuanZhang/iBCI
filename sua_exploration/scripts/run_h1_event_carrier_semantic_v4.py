#!/usr/bin/env python3
"""Run the frozen CPU-only H1 sparse-event label-semantics V4 screen."""
...
def verify_hse5_reproduction(
    ...
    for budget in (3, 4):
        observed = body["budgets"][f"M{budget}"]["candidates"]["pca_delta_q4"]["sessions"]
    ...
    need(maximum <= tolerance, f"V4 failed exact H-SE5 reproduction: {maximum}")
    return {
        "passed": True,
...
def write_immutable(path: Path, value: Mapping[str, Any]) -> tuple[Path, str]:
    output = path.resolve()
    need(not output.exists(), f"refusing to overwrite V4 receipt: {output}")
    output.parent.mkdir(parents=True, exist_ok=True)
    encoded = (json.dumps(value, indent=2, sort_keys=True, allow_nan=False) + "\n").encode("utf-8")
    output.write_bytes(encoded)
    output.chmod(0o444)
    need(stat.S_IMODE(output.stat().st_mode) == 0o444, "V4 receipt mode drift")
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    need(reference.get("schema") == v2.SCHEMA, "V4 H-SE5 reference schema mismatch")
    sessions = design.load_context_sessions(args.data_root.resolve())
    body = semantic.run_screen(sessions)
    reproduction = verify_hse5_reproduction(body, reference)
    module_path = Path(semantic.__file__).resolve()
    ...
    body.update({
        ...
        "implementation_binding": {
            ...
            "shared_design_path": str(design_path),
            "shared_design_sha256": file_sha(design_path),
            "event_parser_path": str(parser_path),
            "event_parser_sha256": file_sha(parser_path),
        },
        "scope": {
            **body["scope"],
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "sua_exploration/results/h1_event_carrier_semantic_v4/source_screen.json",
    )
