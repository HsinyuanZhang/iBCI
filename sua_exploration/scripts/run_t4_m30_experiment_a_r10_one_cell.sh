#!/usr/bin/env bash
# r10 foreground-only cell entrypoint; scheduler must invoke this file via bash.
set -euo pipefail
...
SCREEN="sua_t4_m30_component_attribution_v10"

case "$ARM" in
    z4|ph4|ac4|mb4|b4|ls4) ;;
...
OUT="$ROOT/sua_exploration/checkpoints/${SCREEN}_${ARM}_dandi688_co_s${SEED}"
RESULT="$ROOT/sua_exploration/results/${SCREEN}/${ARM}_s${SEED}.json"
P3="$ROOT/sua_exploration/results/p3_${SCREEN}_${ARM}_dandi688_co_s${SEED}_seed${SEED}.json"
LOCK="${OUT}.lock"

if [[ -e "$OUT" || -e "$RESULT" || -e "$P3" || -e "$LOCK" ]]; then
    echo "refusing output collision for ${ARM}/seed ${SEED}" >&2
    exit 1
fi
mkdir "$LOCK"
...
CUDA_VISIBLE_DEVICES="$GPU" "$PY" -u \
    ...
    --out_name "${SCREEN}_${ARM}_dandi688_co_s${SEED}" \
    ...
    --batch_size 32 \
    --num_workers 4 \
    --cache_dir "$ROOT/sua_exploration/cache/t4_m30_experiment_a_v10" \
    --seed "$SEED" \
    --loss_mode task_only \
