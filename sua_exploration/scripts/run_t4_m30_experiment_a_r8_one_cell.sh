#!/usr/bin/env bash
# r8: launch only from the managed foreground scheduler session; never nohup/disown.
set -euo pipefail
ROOT="/home/xinyuan/Work_host/SPINT";PY="${PYTHON_BIN:-/home/xinyuan/miniconda3/envs/spint/bin/python}";ARM="${ARM:?}";SEED="${SEED:?}";GPU="${GPU:?}";SCREEN="sua_t4_m30_component_attribution_v8"
case "$ARM" in z4|ph4|ac4|mb4|b4|ls4) ;; *) exit 2;; esac;case "$SEED" in 42|43|44) ;; *) exit 2;; esac
"$PY" "$ROOT/sua_exploration/scripts/verify_t4_m30_experiment_a_r8_authorization.py" --require-claim
