#!/usr/bin/env python3
"""One-shot CPU-only r8 builder and in-memory Stage-A signer.

It writes a fresh public anchor, source program, manifests, and exactly two
seed-42 execution capabilities.  It does not create the r8 cell root, invoke
...
PROGRAM_PATH = R8_RECEIPT_ROOT / "program/phase_c_program_v5_r8.json"
PORTABLE_PATH = R8_RECEIPT_ROOT / "manifest/portable_v5_r8.json"
SHARD_PATHS = {
...
SUMMARY_PATH = R8_RECEIPT_ROOT / "launch/stage_a_ready_not_launched.json"
PREPARE_RECEIPT = R8_RECEIPT_ROOT / "prelaunch/r8_stage_a_prepare_receipt.json"
...
def _assert_fresh_targets() -> None:
    for path in (R8_RECEIPT_ROOT, R8_CELL_ROOT, PUBLIC_KEY):
        if path.exists() or path.is_symlink():
            raise FileExistsError(f"r8 fresh target already exists: {path}")
    ...
    if R8_CELL_ROOT.exists():
        raise PermissionError("r8 selector/cell root unexpectedly pre-exists")
...
def _historical_nonces() -> set[str]:
    ...
    if len(nonces) < 5:
        raise PermissionError("r8 nonce audit lacks the retired r6d/r7 authorization inventory")
    return nonces
...
def _r6d_gpu_assignments() -> dict[str, dict[str, Any]]:
    ...
    if sorted(result["gpu0"]["fold_allowlist"] + result["gpu1"]["fold_allowlist"]) != list(range(7)):
        raise PermissionError("r8 Stage-A physical assignment does not cover folds 0..6 exactly")
    if result["gpu0"]["host_id"] != result["gpu1"]["host_id"]:
        raise PermissionError("r8 Stage-A GPU assignment host mismatch")
    if socket.gethostname() != result["gpu0"]["host_id"]:
        raise PermissionError("r8 builder host differs from signed Stage-A host")
    return result
...
def _preflight() -> dict[str, Any]:
    ...
    # Hash the complete v5/r8 source closure before the first r8 artifact is
    # written.  This opens only local code/configuration files, not NWB data.
    rows = source_rows()
    if not rows:
        raise ValueError("r8 source closure is unexpectedly empty")
    ...
    return {
        ...
        "historical_r6d_r7_nonce_count": len(historical_nonces),
...
def _portable(program: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "absolute_cell_root": str(R8_CELL_ROOT.resolve()),
    ...
    *,
    ...
    return {
        ...
        "authorization": {
            ...
            "phase_id": PHASE_ID,
            "status": "GO",
            "authorization_id": f"m2-post33-phase-c-v5-r8-stage-a-{name}-20260805",
            "single_use_nonce": nonce,
            "issued_at": issued,
            "expires_at": expires,
            "stage": "stage_a",
            "capability_scope": "cell_execution",
            "host_id": _read_json(shard, "r8 shard")["host_id"],
            "gpu_id": _read_json(shard, "r8 shard")["gpu_id"],
            "absolute_cell_root": str(R8_CELL_ROOT.resolve()),
            "fold_allowlist": _read_json(shard, "r8 shard")["fold_allowlist"],
            "seed_allowlist": [42],
            "arms_in_order": list(ARMS),
...
def prepare() -> dict[str, Any]:
    ...
    try:
        ...
        program = build_r8_program_receipt()
        write_json_exclusive(PROGRAM_PATH, program)
        validate_r8_program_receipt(PROGRAM_PATH)
        ...
        if len(set(nonces.values())) != len(nonces) or set(nonces.values()) & historical_nonces:
            raise PermissionError("r8 nonce collision or prohibited r6d/r7 nonce reuse")
        ...
        stage_a_cells = sum(
            len(_read_json(path, "r8 shard")["fold_allowlist"]) * len(ARMS)
            for path in SHARD_PATHS.values()
        )
        if stage_a_cells != 14:
            raise PermissionError("r8 Stage-A capability does not cover exactly 14 paired cells")
        summary = {
            "schema": "m2_post33_phase_c_v5_r8_stage_a_ready_not_launched_v1",
            "rollover_id": ROLLOVER_ID,
            ...
            "stage_a_capabilities": {
                name: {
                    ...
                    "cell_count": len(_read_json(SHARD_PATHS[name], "r8 shard")["fold_allowlist"]) * 2,
                }
                for name, path in AUTH_PATHS.items()
            },
            "stage_a_total_cells": stage_a_cells,
            "cell_root_exists": R8_CELL_ROOT.exists(),
        ...
        if summary["cell_root_exists"]:
            raise PermissionError("r8 builder must not create a cell/selector root")
        write_json_exclusive(SUMMARY_PATH, summary)
        receipt = {
            "schema": "m2_post33_phase_c_v5_r8_stage_a_prepare_receipt_v1",
            ...
            "nonces": {
                ...
                "not_reused_from_r6d_or_r7": True,
                "values": nonces,
            },
...
def main() -> None:
    ...
    parser.add_argument("--prepare", action="store_true", help="write/sign the new r8 Stage-A capability")
