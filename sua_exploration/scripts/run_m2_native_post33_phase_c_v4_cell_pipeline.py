#!/usr/bin/env python3
"""Authorized single-cell pipeline used only by the Phase-C matrix launcher.
...
def _training_command(args: argparse.Namespace, key: CellKey, paths: dict[str, Path]) -> tuple[list[str], Path]:
    ...
    if key.arm == "spint":
        return [
            sys.executable,
            str(ROOT / "SPINT-main/src/train_post33_phase_c_v4.py"),
    ...
    return [
        sys.executable,
        str(ROOT / "streaming_calibration_exp/src/train_post33_phase_c_v4.py"),
