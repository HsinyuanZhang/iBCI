    ...
    *,
    ...
    for fold in range(core.RT_EXPECTED_FOLDS):
        ...
        blocks = core.build_rt_fold_blocks(
            ...
            state_parameterisation=state_parameterisation,
        )
        results.append(core.evaluate_session(blocks, q_floor=q_floor, w_floor=w_floor))
    per_fold_hashes = []
    for item in results:
        sealed = item.query_identity.get("sealed_stage2") or {}
        computed = item.query_identity.get("computed") or {}
        sealed_hash = sealed.get("ordered_query_identity_sha256")
        computed_hash = computed.get("ordered_query_identity_sha256")
        require(sealed_hash is not None and sealed_hash == computed_hash, f"RT query identity mismatch: {item.session_name}")
        per_fold_hashes.append(str(sealed_hash))
    require(len(per_fold_hashes) == core.RT_EXPECTED_FOLDS, "RT query-hash roster is not 15 folds")
    dataset_query_hash = core.sha256_bytes("".join(per_fold_hashes).encode("ascii"))
    return {
        "dataset": "rt",
        "state_parameterisation": state_parameterisation,
        "query_identity": {
            "all_folds_bound_to_sealed_stage2": True,
            "dataset_query_identity_sha256": dataset_query_hash,
        ...
        "per_session": [
            {
                ...
                "fit": {
                    "q_floor_applied": item.fit.q_floor_applied,
                    "w_floor_applied": item.fit.w_floor_applied,
                    "w_cond": core.json_safe_condition_number(item.fit.w_cond),
                    "q_cond": core.json_safe_condition_number(item.fit.q_cond),
                    "w_cond_raw": core.json_safe_condition_number(item.fit.w_cond_raw),
                    "q_cond_raw": core.json_safe_condition_number(item.fit.q_cond_raw),
                    "parameters": item.parameters.as_dict() if item.parameters is not None else None,
                },
            }
            for item in results
...
def run_h1(
    *,
    ...
    query_binding = core.bind_h1_query_manifest(data_dir=data_dir, repo_root=REPO_ROOT)
    require(query_binding["matched"], f"H1 query manifest SHA drift: {query_binding}")
    blocks = core.load_all_h1_blocks(
        data_dir=data_dir, repo_root=REPO_ROOT, state_parameterisation=state_parameterisation
    )
    results = [core.evaluate_session(block, q_floor=q_floor, w_floor=w_floor) for block in blocks]
    pooled_r2 = None
    if all(item.success and item.parameters is not None for item in results):
        ...
        for block, item in zip(blocks, results):
            ...
            predictions.append(
                filtered[:, core.velocity_slice(block.kinematic_dim, block.state_parameterisation)]
            )
        pooled_prediction = np.concatenate(predictions, axis=0)
        pooled_r2 = float(core.r2_pooled(pooled_truth, pooled_prediction))
    return {
        "dataset": "falcon_h1",
        "state_parameterisation": state_parameterisation,
        "query_identity": query_binding,
        "summary": core.summarize_sessions(results),
        "pooled_r2": pooled_r2,
        "per_session": [
            {
...
def run_experiment(
    *,
    ...
    if dataset == "subject_m":
        ...
        receipt_name = f"kalman_comparator_rt_{state_parameterisation}_receipt.json"
    elif dataset == "falcon_h1":
        results = run_h1(
            data_dir=h1_data_dir,
            q_floor=q_floor,
            w_floor=w_floor,
            state_parameterisation=state_parameterisation,
        )
        receipt_name = f"kalman_comparator_falcon_h1_{state_parameterisation}_receipt.json"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        choices=core.STATE_PARAMETERISATIONS,
        default=core.STATE_POSITION_VELOCITY,
    )
    ...
    data_dir = args.rt_data_dir.resolve() if args.dataset == "rt" else args.data_dir.resolve()
    summary = run_experiment(
        ...
        state_parameterisation=str(args.state_parameterisation),
    )
