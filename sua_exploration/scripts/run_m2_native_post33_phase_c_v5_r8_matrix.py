#!/usr/bin/env python3
"""Deterministic Stage-A-only r8 matrix launcher for repaired v5 workers."""
...
PIPELINE = ROOT / "sua_exploration/scripts/run_m2_native_post33_phase_c_v5_r8_cell_pipeline.py"
R8_RETIRED = ROOT / "sua_exploration/results/m2_native_post33_phase_c_v5_r9_device_recovery_20260805/r8_pre_hydra_abi_engineering_failure_retirement.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="required only for --execute; no r8 cell may start without an external review receipt",
    )
...
def dry_run_payload(shard: Mapping[str, Any]) -> dict[str, Any]:
    """Render the non-executing plan from the r8 (not v4) shard schema."""
    pairs = [(fold, seed) for fold in shard["fold_allowlist"] for seed in shard["seed_allowlist"]]
    return {
        "schema": "m2_post33_phase_c_v5_r8_shard_dry_run_v1",
        ...
        "portable_manifest_sha256": shard["portable_manifest_sha256"],
        "execution_started": False,
    }


def main() -> None:
    if R8_RETIRED.exists():
        raise PermissionError("r8 is append-only retired after a pre-Hydra authorization ABI failure")
    args = parse_args()
    ...
    portable = validate_r8_portable_transfer_manifest(
    ...
    pairs = [(fold, seed) for fold in shard["fold_allowlist"] for seed in shard["seed_allowlist"]]
    dry = dry_run_payload(shard)
    if not args.execute:
        print(json.dumps(dry, sort_keys=True, indent=2))
        return
    if args.independent_review is None:
        raise PermissionError("r8 --execute is blocked until an independent review receipt is supplied")
    ...
    if not isinstance(review, dict) or any(review.get(key) != value for key, value in expected_review.items()):
        raise PermissionError("r8 independent review receipt does not approve this exact program/shard")
