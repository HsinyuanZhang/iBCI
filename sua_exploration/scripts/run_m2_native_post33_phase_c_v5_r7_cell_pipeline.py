#!/usr/bin/env python3
"""Authorized r7 single-cell pipeline bound to the repaired v5 workers.
...
def _training_command(args: argparse.Namespace, key: CellKey, paths: dict[str, Path]) -> tuple[list[str], Path]:
    ...
    if key.arm == "spint":
        return [
            sys.executable,
            str(ROOT / "SPINT-main/src/train_post33_phase_c_v5_r7.py"),
    ...
    return [
        sys.executable,
        str(ROOT / "streaming_calibration_exp/src/train_post33_phase_c_v5_r7.py"),
...
def main() -> None:
    ...
    if paths["selector_records"].exists():
        raise PermissionError("r7 requires a fresh selector; selector records pre-exist before training")
