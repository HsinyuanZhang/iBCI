#!/usr/bin/env python3
"""CPU-only r7 source/capability verifier; it cannot launch a worker."""
...
PORTABLE = R7_RECEIPT_ROOT / "manifest/portable_v5_r7.json"
SHARDS = {
...
PREPARE = R7_RECEIPT_ROOT / "prelaunch/r7_stage_a_prepare_receipt.json"
...
def _assert_v5_worker_equivalence() -> dict[str, Any]:
    pairs = {
        "spint_evaluator": (
            ...
            ROOT / "SPINT-main/src/evaluate_post33_phase_c_v5_r7.py",
        ),
        "t4_evaluator": (
            ROOT / "streaming_calibration_exp/src/evaluate_post33_phase_c_v5.py",
            ROOT / "streaming_calibration_exp/src/evaluate_post33_phase_c_v5_r7.py",
        ),
        "spint_trainer": (
            ROOT / "SPINT-main/src/train_post33_phase_c_v4.py",
            ROOT / "SPINT-main/src/train_post33_phase_c_v5_r7.py",
        ),
        "t4_trainer": (
            ROOT / "streaming_calibration_exp/src/train_post33_phase_c_v4.py",
            ROOT / "streaming_calibration_exp/src/train_post33_phase_c_v5_r7.py",
        ),
    }
    evidence: dict[str, Any] = {}
    for label, (baseline, r7) in pairs.items():
        require_canonical_regular_file(baseline, within=ROOT)
        require_canonical_regular_file(r7, within=ROOT)
        if _normalized(r7, old="m2_native_post33_authorization_v4", new="m2_native_post33_authorization_v5_r7") != _normalized(
            baseline, old="m2_native_post33_authorization_v4", new="m2_native_post33_authorization_v4"
        ):
            raise ValueError(f"r7 {label} differs from its reviewed baseline beyond auth import")
        evidence[label] = {"baseline": file_metadata(baseline), "r7": file_metadata(r7)}
    return evidence


def verify() -> dict[str, Any]:
    if R7_CELL_ROOT.exists():
        raise PermissionError("r7 verification refuses a root after any cell/selector materialization")
    anchor = validate_public_anchor()
    retirement = validate_r6d_retirement()
    program = validate_r7_program_receipt(PROGRAM)
    validate_r7_portable_transfer_manifest(
        PORTABLE,
        workspace_root=ROOT,
    ...
    for name in ("gpu0", "gpu1"):
        shard = validate_r7_shard_manifest(
            SHARDS[name], portable_manifest_path=PORTABLE, cell_root=R7_CELL_ROOT
        )
    ...
    if set(nonces) & {""} or len(nonces) != len(set(nonces)) or coverage != expected:
        raise PermissionError("r7 Stage-A capability coverage/nonce integrity failed")
    forbidden = list((R7_RECEIPT_ROOT / "auth").glob("*stage_b*")) + list(
        (R7_RECEIPT_ROOT / "auth").glob("*opening*"))
    if forbidden:
        raise PermissionError("r7 must not contain Stage-B or score-opening authority")
    ...
    if (
        ...
        raise PermissionError("r7 prepared-state receipt violates no-launch boundary")
    workers = _assert_v5_worker_equivalence()
    return {
        "schema": "m2_post33_phase_c_v5_r7_cpu_verifier_result_v1",
...
def main() -> None:
    ...
    if args.json_out is not None:
        ...
        # explicit output is allowed only under the fresh r7 receipt root.
        output = args.json_out.resolve()
        try:
            output.relative_to(R7_RECEIPT_ROOT.resolve())
        except ValueError as exc:
            raise PermissionError("verifier output must remain inside r7 receipt root") from exc
        if output.exists():
