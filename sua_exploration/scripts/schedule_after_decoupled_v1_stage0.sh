...
# of blindly starting M15 shrinkage.  A positive v2 seed-42 diagnostic expands
# only after its strict same-seed coupled/content gate passes.  This scheduler
# never opens formal data, launches INT8, or starts M15.
set -euo pipefail
...
V1_THREE_SEED_AGGREGATE="$V1_RESULTS/aggregate_seeds42_43_44.json"
...
V2_AGGREGATOR="$ROOT/sua_exploration/scripts/aggregate_sua_decoupled_kv_v2.py"
V2_AGGREGATE="$V2_RESULTS/aggregate_seed42.json"
V2_THREE_SEED_AGGREGATE="$V2_RESULTS/aggregate_seeds42_43_44.json"
...
run_v1_coupled_seed() {
  local seed="$1"
  local gpu="$2"
  local result="$V1_RESULTS/coupled_t4_m50_s${seed}.json"
  if [[ -f "$result" ]]; then
    echo "[$(date -Is)] skip existing v1 coupled baseline $result"
    return
  fi
  echo "[$(date -Is)] launch paired v1 coupled baseline seed=$seed gpu=$gpu"
  env ARM=coupled_t4 SEED="$seed" GPU="$gpu" "$V1_RUNNER" --launch
}
...
run_v2_replication_seed() {
  local seed="$1"
  local gpu="$2"
  # The v2 aggregate uses a fresh, same-seed v1 coupled baseline.  Do not
  # substitute seed 42 or the historical ordinary-T4 artifact.
  run_v1_coupled_seed "$seed" "$gpu"
  for arm in "${V2_ARMS[@]}"; do
    local result="$V2_RESULTS/${arm}_m50_s${seed}.json"
    if [[ -f "$result" ]]; then
      echo "[$(date -Is)] skip existing v2 result $result"
      continue
    fi
    echo "[$(date -Is)] launch v2 arm=$arm seed=$seed gpu=$gpu"
    env ARM="$arm" SEED="$seed" GPU="$gpu" "$V2_RUNNER" --launch
  done
}
...
if jq -e '.stage0_descriptive_mechanism_pass == true' \
  ...
  "$PY" "$V1_AGGREGATOR" \
    --result-dir "$V1_RESULTS" \
    --seeds 42,43,44 \
    --out "$V1_THREE_SEED_AGGREGATE"
  echo "[$(date -Is)] v1 three-seed aggregate complete; stop before formal/INT8"
else
  ...
  "$PY" "$V2_AGGREGATOR" \
    ...
    --seeds 42 \
    --out "$V2_AGGREGATE"
  echo "[$(date -Is)] wrote strict v2 seed-42 aggregate"

  if jq -e \
    '.stage0_descriptive_candidate_pass | to_entries | any(.value == true)' \
    "$V2_AGGREGATE" >/dev/null; then
    echo "[$(date -Is)] v2 Stage-0 passed; expand paired seeds 43/44"
    run_v2_replication_seed 43 0 &
    lane0=$!
    run_v2_replication_seed 44 1 &
    lane1=$!
    wait "$lane0"
    wait "$lane1"
    "$PY" "$V2_AGGREGATOR" \
      ...
      --v1-result-dir "$V1_RESULTS" \
      --seeds 42,43,44 \
      --out "$V2_THREE_SEED_AGGREGATE"
    echo "[$(date -Is)] v2 three-seed aggregate complete; stop before formal/INT8"
  else
    echo "[$(date -Is)] v2 Stage-0 failed; stop for diagnosis before another FP32 design"
  fi
fi
