#!/usr/bin/env python3
"""One-shot CPU-only r9 builder and in-memory Stage-A signer.

It writes a fresh public anchor, source program, manifests, and exactly two
seed-42 execution capabilities.  It does not create the r9 cell root, invoke
...
R7 = ROOT / "sua_exploration/results/m2_native_post33_phase_c_v5_r7_launch_receipts_20260805"
SOURCE = Path(__file__).resolve()
PROGRAM_PATH = R9_RECEIPT_ROOT / "program/phase_c_program_v5_r9.json"
PORTABLE_PATH = R9_RECEIPT_ROOT / "manifest/portable_v5_r9.json"
SHARD_PATHS = {
...
SUMMARY_PATH = R9_RECEIPT_ROOT / "launch/stage_a_ready_not_launched.json"
PREPARE_RECEIPT = R9_RECEIPT_ROOT / "prelaunch/r9_stage_a_prepare_receipt.json"
...
def _assert_fresh_targets() -> None:
    for path in (R9_RECEIPT_ROOT, R9_CELL_ROOT, PUBLIC_KEY):
        if path.exists() or path.is_symlink():
            raise FileExistsError(f"r9 fresh target already exists: {path}")
    ...
    if R9_CELL_ROOT.exists():
        raise PermissionError("r9 selector/cell root unexpectedly pre-exists")


def _historical_nonces() -> set[str]:
    nonces: set[str] = set()
    for label, root in (("r6d", R6D), ("r7", R7)):
        for path in sorted((root / "auth").glob("*.json")):
            payload = _read_json(path, f"historical {label} authorization")
    ...
    if len(nonces) < 5:
        raise PermissionError("r9 nonce audit lacks the retired r6d/r7 authorization inventory")
    # r8's two already-claimed nonce values are deliberately recovered only
    # from the sealed retirement receipt's claim-path metadata.  No r8 claim,
    # status, selector, checkpoint, or score payload is opened here.
    r8_retirement = _read_json(R8_PRE_HYDRA_RETIREMENT, "r8 retirement")
    claims = r8_retirement.get("nonce_claims")
    metadata = claims.get("metadata") if isinstance(claims, Mapping) else None
    if not isinstance(metadata, list) or len(metadata) != 2:
        raise PermissionError("r9 nonce audit lacks the sealed r8 claim inventory")
    for record in metadata:
        path_value = record.get("canonical_path") if isinstance(record, Mapping) else None
        nonce = Path(str(path_value)).stem
        if not isinstance(path_value, str) or len(nonce) != 64 or any(ch not in "0123456789abcdef" for ch in nonce):
            raise PermissionError("r8 retirement contains an invalid claimed nonce path")
        nonces.add(nonce)
    if len(nonces) < 7:
        raise PermissionError("r9 nonce audit lacks the retired r6d/r7/r8 nonce inventory")
    return nonces
...
def _r6d_gpu_assignments() -> dict[str, dict[str, Any]]:
    ...
    if sorted(result["gpu0"]["fold_allowlist"] + result["gpu1"]["fold_allowlist"]) != list(range(7)):
        raise PermissionError("r9 Stage-A physical assignment does not cover folds 0..6 exactly")
    if result["gpu0"]["host_id"] != result["gpu1"]["host_id"]:
        raise PermissionError("r9 Stage-A GPU assignment host mismatch")
    if socket.gethostname() != result["gpu0"]["host_id"]:
        raise PermissionError("r9 builder host differs from signed Stage-A host")
    return result
...
def _preflight() -> dict[str, Any]:
    ...
    # Hash the complete v5/r9 source closure before the first r9 artifact is
    # written.  This opens only local code/configuration files, not NWB data.
    rows = source_rows()
    if not rows:
        raise ValueError("r9 source closure is unexpectedly empty")
    ...
    historical_nonces = _historical_nonces()
    ...
    return {
        ...
        "historical_r6d_r7_r8_nonce_count": len(historical_nonces),
...
def _portable(program: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "absolute_cell_root": str(R9_CELL_ROOT.resolve()),
        ...
        "r7_prelaunch_retirement": dict(program["r7_prelaunch_retirement"]),
        "r8_pre_hydra_retirement": dict(program["r8_pre_hydra_retirement"]),
    ...
    *,
    ...
    return {
        ...
        "authorization": {
            ...
            "phase_id": PHASE_ID,
            "status": "GO",
            "authorization_id": f"m2-post33-phase-c-v5-r9-stage-a-{name}-20260805",
            "single_use_nonce": nonce,
            "issued_at": issued,
            "expires_at": expires,
            "stage": "stage_a",
            "capability_scope": "cell_execution",
            "host_id": _read_json(shard, "r9 shard")["host_id"],
            "gpu_id": _read_json(shard, "r9 shard")["gpu_id"],
            "absolute_cell_root": str(R9_CELL_ROOT.resolve()),
            "fold_allowlist": _read_json(shard, "r9 shard")["fold_allowlist"],
            "seed_allowlist": [42],
            "arms_in_order": list(ARMS),
...
def prepare() -> dict[str, Any]:
    ...
    try:
        ...
        program = build_r9_program_receipt()
        write_json_exclusive(PROGRAM_PATH, program)
        validate_r9_program_receipt(PROGRAM_PATH)
        ...
        if len(set(nonces.values())) != len(nonces) or set(nonces.values()) & historical_nonces:
            raise PermissionError("r9 nonce collision or prohibited r6d/r7/r8 nonce reuse")
        ...
        stage_a_cells = sum(
            len(_read_json(path, "r9 shard")["fold_allowlist"]) * len(ARMS)
            for path in SHARD_PATHS.values()
        )
        if stage_a_cells != 14:
            raise PermissionError("r9 Stage-A capability does not cover exactly 14 paired cells")
        summary = {
            "schema": "m2_post33_phase_c_v5_r9_stage_a_ready_not_launched_v1",
            "rollover_id": ROLLOVER_ID,
            ...
            "stage_a_capabilities": {
                name: {
                    ...
                    "cell_count": len(_read_json(SHARD_PATHS[name], "r9 shard")["fold_allowlist"]) * 2,
                }
                for name, path in AUTH_PATHS.items()
            },
            "stage_a_total_cells": stage_a_cells,
            "cell_root_exists": R9_CELL_ROOT.exists(),
        ...
        if summary["cell_root_exists"]:
            raise PermissionError("r9 builder must not create a cell/selector root")
        write_json_exclusive(SUMMARY_PATH, summary)
        receipt = {
            "schema": "m2_post33_phase_c_v5_r9_stage_a_prepare_receipt_v1",
            ...
            "nonces": {
                ...
                "not_reused_from_r6d_r7_or_r8": True,
                "values": nonces,
            },
...
def main() -> None:
    ...
    parser.add_argument("--prepare", action="store_true", help="write/sign the new r9 Stage-A capability")
