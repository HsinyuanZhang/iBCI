"""Standalone validation epoch-window evaluator for the exact-head oracle."""
...
from eval_epoch_window_decoupled_v2_dandi688 import (  # noqa: E402
    compute_protocol_epochs,
    compute_variant_score,
    select_epoch_window_checkpoints,
    sha256_file,
)
from select_head_oracle_protocol_dandi688 import (  # noqa: E402
    evaluate_fixed_oracle_protocol_over_validation_sessions,
)
...
def _validate_metadata(
    ...
    *,
    ...
    if metadata.get("split_counts") != [27, 6, 6]:
        raise ValueError(
            "oracle metadata must record strict 27/6/6"
        )
    ...
    if sha256_file(manifest_path) != metadata.get(
        ...
        raise ValueError(
            "strict manifest SHA differs from oracle metadata"
        )
    if sha256_file(teacher_ckpt) != metadata.get("teacher_sha256"):
        raise ValueError(
            "teacher checkpoint SHA differs from oracle metadata"
        )
    ...
    if (
        ...
        raise ValueError(
            "oracle training protocol receipt is inconsistent"
        )
    ...
    if (
        ...
        raise ValueError("oracle evaluator requires aligned T4@50")
    ...
    if decoder.get("key_mode") not in {"e_t4", "e_ts4"}:
        raise ValueError("oracle key mode is invalid")
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if (
        ...
        raise ValueError(
            "oracle score fixes epochs=12, burn-in=4, "
            "M_activity=30 and evaluation start=50"
        )
    epochs = compute_protocol_epochs(
        args.total_epochs, args.burn_in
    )
    ...
    for path, description in (
        (metadata_path, "oracle run metadata"),
    ...
    for epoch in epochs:
        ...
        if session_splits is None:
            ...
            raise ValueError(
                f"session split drift at oracle epoch {epoch}"
            )
        elif result["session_unit_counts"] != session_unit_counts:
            raise ValueError(
                f"unit-count drift at oracle epoch {epoch}"
            )
        if result.get("formal_test_files_opened") != 0:
            raise ValueError(
                "oracle evaluator opened a formal test file"
            )
        per_epoch[str(epoch)] = {
    ...
    per_epoch_mean = {
        epoch: per_epoch[str(epoch)]["mean_r2"]
        for epoch in epochs
    }
    variant_score = compute_variant_score(
        per_epoch_mean, epochs
    )
    decoder = metadata["decoder_architecture"]
    payload = {
        "schema_version": 2,
        "purpose": (
            "teacher_head_preserving_kv_oracle_validation_"
            "epoch_window"
        ),
        "generated_by": (
            "eval_epoch_window_head_oracle_dandi688.py"
        ),
        "protocol_metric_source": (
            "select_head_oracle_protocol_dandi688."
            "evaluate_fixed_oracle_protocol_over_validation_sessions"
        ),
        ...
        "decoder_architecture": {
            ...
            "key_permutation_seed": decoder[
                "key_permutation_seed"
            ],
            "headwise_softmax_preserved": decoder[
        ...
        "variant_score_definition": (
            "unweighted mean of six-session validation mean R2 "
            "over protocol epochs 5..12"
        ),
    ...
    print(f"Wrote oracle validation result: {out_path}")
