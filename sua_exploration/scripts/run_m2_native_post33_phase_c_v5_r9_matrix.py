#!/usr/bin/env python3
"""Deterministic Stage-A-only r9 matrix launcher for repaired v5 workers."""
...
PIPELINE = ROOT / "sua_exploration/scripts/run_m2_native_post33_phase_c_v5_r9_cell_pipeline.py"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="required only for --execute; no r9 cell may start without an external review receipt",
    )
...
def _run_cell(args: argparse.Namespace, key: CellKey, *, host_id: str, gpu_id: str) -> None:
    ...
    if paths["completed"].exists():
        print(f"SKIP completed cell: {key.arm} fold-{key.fold} seed-{key.seed}", flush=True)
        return
...
def dry_run_payload(shard: Mapping[str, Any]) -> dict[str, Any]:
    """Render the non-executing plan from the r9 (not v4) shard schema."""
    pairs = [(fold, seed) for fold in shard["fold_allowlist"] for seed in shard["seed_allowlist"]]
    return {
        "schema": "m2_post33_phase_c_v5_r9_shard_dry_run_v1",
...
def main() -> None:
    ...
    portable = validate_r9_portable_transfer_manifest(
    ...
    if args.independent_review is None:
        raise PermissionError("r9 --execute is blocked until an independent review receipt is supplied")
    review = json.loads(args.independent_review.resolve(strict=True).read_text(encoding="utf-8"))
    expected_review = {
        "schema": "m2_post33_phase_c_v5_r9_independent_launch_review_v1",
    ...
    if not isinstance(review, dict) or any(review.get(key) != value for key, value in expected_review.items()):
        raise PermissionError("r9 independent review receipt does not approve this exact program/shard")
