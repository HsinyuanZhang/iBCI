#!/usr/bin/env python3
"""No-output fixed-policy Stage-A validator for the r6c live coordinator.

The process deliberately exports only an exit status.  It validates the
already signed fixed-policy decision and emits exactly one of three codes:
``EXIT_CONTINUE``, ``EXIT_VALIDATED_STOP``, or ``EXIT_INVALID``.  It never
prints the decision or any endpoint value.
"""
...
def main() -> None:
    ...
    if decision.get("decision") == "seed42_severe_negative_futility_stop":
        raise SystemExit(EXIT_VALIDATED_STOP)
    raise SystemExit(EXIT_INVALID)
