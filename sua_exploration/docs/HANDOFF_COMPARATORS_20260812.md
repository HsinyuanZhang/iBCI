...
|---|---|---|---|---|---|
| subject-M SUA | done | none | done, 0.4179 | done, 0.1154 | skeleton, not run |
| subject-M pseudo-MUA | done | none | done, 0.4102 | done, 0.1042 | skeleton, not run |
| FALCON M2 | partial | done, matched | done, 0.1139 | audit done, `PV_DEFINABLE` 6/6; scoring arm built, not run | skeleton, blocked (see 7.5.2) |
| RT | done | done, B2-D1024 | done, 0.2002 | proven inapplicable | skeleton, not run |
| FALCON H1 | same-checkpoint only | done, matched | done, 0.2582 / tuned 0.2889 | audit **quarantined**, must be re-derived | skeleton, not run |

Status as of the 7.5 review. No Kalman arm and no PV scoring arm has been executed.
...
| # | Item | Compute | Why |
|---|---|---|---|
| 1 | **Kalman filter**, all four datasets — **skeleton built, NOT run** | CPU | The most conspicuous absence. The velocity Kalman filter is the standard clinical iBCI decoder and H1 comes from that lineage. Comparing against PV and ridge but not Kalman reads as incomplete. Resolve the three blockers in 7.5.2 first, especially the synthesised position. |
...
| 3 | PV **scoring arm** on M2 — audit already done, `PV_DEFINABLE` 6/6 | CPU | The applicability audit is complete and the scoring arm is implemented; only execution remains |
| 3b | Re-derive the H1 PV audit on held-in data | CPU | The existing audit is quarantined for a scope violation; route through `index_heldin_calib` per 7.5.3 |
...
| 7 | Resolve the three Kalman blockers in 7.5.2, then run Kalman on all four datasets | CPU |
| 8 | Verify the SPR correspondence finding, then decide on Part B | CPU |
| 9 | Run the M2 PV scoring arm; re-derive the H1 PV audit on held-in data | CPU |
