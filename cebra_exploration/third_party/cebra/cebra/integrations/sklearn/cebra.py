...
class CEBRA(TransformerMixin, BaseEstimator):
    ...
    def _prepare_fit(
        ...
        *y,
        freeze_sessions: Optional[List[int]] = None,
        init_from: Optional["CEBRA"] = None,
        ...
        Args:
            ...
            freeze_sessions: Optional session indices whose encoders are frozen
                (``requires_grad=False``).  Only valid for multi-session fits.  The
                optimizer then sees only the remaining session encoders plus the
                criterion.  Local modification 2026-08-13: required so a new session
                can join a joint multi-session fit without moving the source encoders.
            init_from: Optional fitted multi-session estimator whose first
                ``init_from.num_sessions`` encoders initialise the corresponding
                sessions of this fit.  The remaining sessions (typically one new
                target) stay randomly initialised.
        ...
        frozen = list(freeze_sessions) if freeze_sessions is not None else []
        if frozen or init_from is not None:
            if not is_multisession or not isinstance(model, nn.ModuleList):
                raise ValueError(
                    "freeze_sessions and init_from require a multi-session fit."
                )
        if init_from is not None:
            sklearn_utils_validation.check_is_fitted(init_from, "n_features_")
            n_src = init_from.num_sessions
            if n_src is None or not isinstance(init_from.model_, nn.ModuleList):
                raise ValueError(
                    "init_from must be a fitted multi-session CEBRA estimator."
                )
            if len(model) <= n_src:
                raise ValueError(
                    "init_from must have strictly fewer sessions than the new fit "
                    f"(got {n_src} vs {len(model)})."
                )
            for session_id in range(n_src):
                src_model = init_from.model_[session_id]
                dst_model = model[session_id]
                if int(src_model.num_input) != int(dst_model.num_input):
                    raise ValueError(
                        f"init_from session {session_id} input dim "
                        f"{src_model.num_input} != {dst_model.num_input}."
                    )
                dst_model.load_state_dict(src_model.state_dict())
        for session_id in frozen:
            if session_id < 0 or session_id >= len(model):
                raise ValueError(
                    f"freeze_sessions index {session_id} out of range for "
                    f"{len(model)} sessions."
                )
            for parameter in model[session_id].parameters():
        ...
        if not trainable:
            raise ValueError("freeze_sessions left no trainable encoder parameters.")
        self.frozen_sessions_ = tuple(frozen)
        self.n_trainable_parameters_ = int(
            sum(parameter.numel() for parameter in trainable)
        )
        optimizer = torch.optim.Adam(
            itertools.chain(trainable, criterion.parameters()),
    ...
    def partial_fit(
        ...
        *y,
        ...
        if not hasattr(self, "state_") or self.state_ is None:
            self.state_ = self._prepare_fit(
                X,
                *y,
                freeze_sessions=freeze_sessions,
                init_from=init_from,
            )
    ...
    def fit(
        ...
        *y,
        ...
        Args:
            ...
            freeze_sessions: Session indices to freeze in a multi-session fit.  Local
                modification 2026-08-13.  Cannot be combined with ``adapt=True``.
            init_from: Fitted multi-session estimator used to initialise the first
                sessions of this fit.  Local modification 2026-08-13.
            callback: If a function is passed here with signature ``callback(num_steps, solver)``,
                the function will be regularly called at the specified ``callback_frequency``.
            callback_frequency: Specify the number of iterations that need to pass before triggering
                the specified ``callback``,
        ...
        Example:
            ...
            >>> tmp_file.unlink()
        """
        if adapt and (freeze_sessions is not None or init_from is not None):
            raise ValueError(
                "freeze_sessions and init_from cannot be combined with adapt=True."
            )
