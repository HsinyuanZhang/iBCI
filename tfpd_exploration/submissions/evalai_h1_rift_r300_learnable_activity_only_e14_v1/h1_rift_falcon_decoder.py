"""Falcon runtime for H1 RIFT R300 ACTIVITY_ONLY + per-layer learned_slope.

Architecture is RIFT, identity is C2 activity E0 with literal zero T,
recency is per-layer learned_slope on the unscaled default ladder.
Backend is cached PyTorch FP32 via CpuLearnableRecencyRuntime.
Not BT-EORT, not ORT, not SPINT, not 582218, not 582241.
"""
...
PAYLOAD_SCHEMA = "h1_rift_r300_learnable_activity_only_e14_falcon_payload_v1"
CHANNELS = 176
BEHAVIOR_SCALE = 20.0
...
def build_decoder(recency_cfg: LearnableRecencyConfig) -> RiftDecoder:
    """Rebuild the trained H1 R300 learned_slope stack without wrap.py."""
...
def _task_bank(tag: str, row: dict) -> TaskBank:
    ...
    return TaskBank(
        ...
        calibration_meta={
            ...
            "trial_count": 3,
            "estimator": "ACTIVITY_ONLY C2 activity rematerialized E0 + literal zero T (sealed 27-tag)",
...
class H1RiftLearnableFalconDecoder(BCIDecoder):
    def __init__(self, task_config, model_path: str, batch_size: int = 1):
        ...
        if payload.get("bias_mode") != "learnable_learned_slope":
            raise ValueError("payload bias_mode must be learnable_learned_slope")
        if payload.get("identity_interface") != "activity_only":
            raise ValueError("payload identity_interface must be activity_only")
        if int(payload.get("proj_dim", -1)) != PROJ_DIM:
            raise ValueError(f"payload proj_dim must be {PROJ_DIM}")
        ...
        self._n_predicts = 0
        self.kind = "rift_learnable_cached"
