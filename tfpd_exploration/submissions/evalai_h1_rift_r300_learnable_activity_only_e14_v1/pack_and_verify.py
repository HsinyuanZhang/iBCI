#!/usr/bin/env python3
"""Build the H1 RIFT R300 learned_slope ACTIVITY_ONLY e14 cached payload, host-verify, and docker-pack.

Seals official 27-tag ACT banks (C2 activity E0, literal zero T) + learned_slope temporal.
Does not EvalAI-submit. Does not overwrite 582218 / 582241 / 582277.
"""
...
CKPT = ROOT / "btransform_unified_v2/learnable_recency_v1/results/h1_activity_only_s42/epoch_014.pt"
CKPT_SHA = "27dedcbfb16e8c7baba2af4fd15e67dba38451db84d44b26ac7a21cde27b083b"
SCORE = ROOT / "btransform_unified_v2/learnable_recency_v1/results/h1_activity_only_s42/ho_m3_selection.json"
RUN_META = ROOT / "btransform_unified_v2/learnable_recency_v1/results/h1_activity_only_s42/run_meta.json"
BANKS_DIR = ROOT / "btransform_unified_v2/results/final_ablation_official_v1/h1_activity_only_banks_v1"
BANKS_NPZ = BANKS_DIR / "banks_27.npz"
BANKS_SHA = "3b273ca47a1c798ce1c1f3ca079f508ac6825c4624bd96e322e6df90c7df335a"
...
IMAGE_TAG = "h1-rift-r300-learnable-activity-only-e14:v1"
METHOD_LABEL = (
    "H1 RIFT R300 learned_slope ACTIVITY_ONLY e14 EMA cached CPU; architecture=RIFT backend=cached "
    "identity=activity_only recency=learned_slope_default; not BT-EORT; not ORT; not SPINT; not 582218; not 582241"
)
METHOD_NAME = "H1 RIFT R300 learned_slope ACT e14 cached"
METHOD_DESCRIPTION = (
    "H1 RIFT R300 P16 ACTIVITY_ONLY control with per-layer learned_slope, epoch 14 EMA. "
    "C2 rematerialized E0, literal zero T. HO-M3 grouped-seven pick e14 r2_mean 0.2659 worst 0.1046. "
    "Cached PyTorch FP32 CPU backend. Not BT-EORT, not ORT, not SPINT, not 582218 frozen ACT, not 582241 Full."
)
N_STEPS = 80
GATE = 1.0e-5
...
def _build_payload() -> tuple[Path, str, dict]:
    ...
    if sha256(CKPT) != CKPT_SHA:
        raise RuntimeError(f"epoch_014.pt SHA drift: {sha256(CKPT)} != {CKPT_SHA}")
    if sha256(BANKS_NPZ) != BANKS_SHA:
        raise RuntimeError("ACT banks_27.npz SHA drift")
    score = json.loads(SCORE.read_text())
    selected = score["selected"]
    if int(selected["epoch"]) != 14 or score.get("identity_ablation") != "activity_only":
        raise RuntimeError(f"HO-M3 selection drifted: {selected}")
    run_meta = json.loads(RUN_META.read_text())
    if run_meta.get("identity") != "activity_only":
        raise RuntimeError(f"run_meta identity drifted: {run_meta.get('identity')}")
    recency_cfg = config_from_run_meta(run_meta, "h1")
    receipt = json.loads((BANKS_DIR / "receipt.json").read_text())
    if (
        receipt.get("schema") != "h1_signed_state_r300_ablation_banks_v1"
        or receipt.get("arm") != "ACTIVITY_ONLY"
        or receipt.get("tag_count") != 27
        or receipt.get("status") != "BUILT"
    ):
        raise RuntimeError("ACTIVITY_ONLY bank receipt drift")
    with open(MASK_PAYLOAD, "rb") as handle:
        official = CPUUnpickler(handle).load()["bank_by_dataset_tag"]
    if set(official) != set(receipt["tags"]):
        raise RuntimeError(f"official 27-tag set drifted vs ACTIVITY_ONLY banks: {sorted(set(official) ^ set(receipt['tags']))}")
    ckpt = torch.load(CKPT, map_location="cpu", weights_only=False)
    if ckpt.get("variant") != "learnable_learned_slope_activity_only" or int(ckpt.get("context_bins", 0)) != 300:
        raise RuntimeError("checkpoint is not formal H1 R300 learnable_learned_slope_activity_only")
    if int(ckpt.get("epoch", -1)) != 14:
        raise RuntimeError("checkpoint is not e14")
    ...
    with np.load(BANKS_NPZ) as handle:
        for tag, row in official.items():
            ...
            if not np.array_equal(t, np.zeros_like(t)):
                raise RuntimeError(f"{tag}: ACTIVITY_ONLY T is not literal zero")
            if np.array_equal(e0, np.zeros_like(e0)):
                raise RuntimeError(f"{tag}: ACTIVITY_ONLY E0 collapsed to NONE")
            if np.array_equal(t, old_t):
                raise RuntimeError(f"{tag}: ACTIVITY_ONLY T collapsed to H-C T")
            sealed[tag] = {
                "E0": e0,
                "T": t,
    ...
    payload = {
        ...
        "recency_tier": "learned_slope",
        "recency_ladder": "default",
        "identity_interface": "activity_only",
        "proj_dim": 16,
    ...
    dest = DEST / "artifacts" / "h1_rift_r300_learnable_activity_only_e14.pkl"
...
"""
    ...
    candidate = {
        "arm": "h1_rift_r300_learnable_activity_only_e14_cached",
        "task": "h1",
        "image_tag": IMAGE_TAG,
        "image_id": image_id,
