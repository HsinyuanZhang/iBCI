"""Guarded EvalAI submission helper for the D-opt-4 activity-30 static M2 image.
...
METHOD_LABEL = (
    "D-opt-4 activity-30 static M2 cached identity (ridge T4 k=4 from D-opt "
    "first-30 selection; B3S activity pool = first 30 label-free calibration trials)"
)
...
CANDIDATE: dict[str, Any] = {
    ...
    "method_name": "M2 D-opt-4 static ridge T4 seed42, activity-30 pool",
    "method_description": (
        ...
        "the selected four; B3S activity pool = the full label-free first-30 "
        "calibration block, mean-pooled by the encoder). Static four-label "
    ...
    "budget_disclosure": (
        ...
        "trials (label-free neural activity only); zero online updates"
    ),
}
...
def state_path() -> Path:
    return Path(__file__).resolve().parent / "artifacts" / "evalai_push_state_dopt4_act30_v1.json"
...
def _push_image(client: Any, image: Any, runtime: tuple[Any, Any, Any, str, str]) -> dict[str, object]:
    ...
    state = {
        "schema_version": "m2_dopt4_act30_evalai_push_state_v1",
        "arm": CANDIDATE["arm"],
        "image_tag": CANDIDATE["image_tag"],
        "image_id": CANDIDATE["image_id"],
        "payload_sha256": CANDIDATE["payload_sha256"],
...
def _register(state: dict[str, object], runtime: tuple[Any, Any, Any, str, str]) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="m2_dopt4_act30_evalai_") as tmp:
