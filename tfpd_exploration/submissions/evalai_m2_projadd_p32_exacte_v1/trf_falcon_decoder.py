"""Self-contained Falcon runtime for M2 proj_add P32 + exact-E.

Identity: tokens = token_mlp([local+P_1 ; local+P_2] | carrier4),
token_in=36, P = Linear(50->32, bias=False). Each 16-d group of P(E0)
is added onto a copy of the local conv. Exact-E is causal-k5 frontend
reuse plus last-layer last-query. Temporal layers 1-3 stay full. No
cross-window temporal KV. Does not import tfpd_exploration.
"""
...
P32_CFG = TrfConfig(kind="proj_add", token_in=36, proj_out=32, temporal_width=256, ffn=512, readout_hidden=128)
...
def build_decoder(kind: str) -> CausalTransformerDecoder:
    if kind in {"proj_add", "proj_add_p32"}:
        return CausalTransformerDecoder(P32_CFG)
...
class TrfFalconDecoder(BCIDecoder):
    ...
    def __init__(self, task_config, model_path: str, batch_size: int = 1):
        ...
        proj_dim = int(meta.get("proj_dim", 32))
        token_in = int(meta.get("token_in", 36))
        if proj_dim != 32 or token_in != 36:
            raise ValueError(f"P32 decoder expects proj_dim=32 token_in=36, got {proj_dim}/{token_in}")
        self.kind = kind
