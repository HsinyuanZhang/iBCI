"""AOF-S V3 production-local admission using the sole V1 lifecycle."""
...
from .profile import AofsV3Profile, V3_PROFILE
...
def closure_map(repo_root: Path = plan.REPO_ROOT) -> dict[str, str]:
    ...
    for relative in plan.STATIC_CLOSURE_RELATIVES:
        ...
        _need(path.is_file() and not path.is_symlink(), f"AOF-S V3 closure missing/symlink leaf: {relative}")
...
def _reviewed(repo_root: Path, mapping: Mapping[str, str], digest: str) -> dict[str, str]:
    ...
    _need(dict(mapping) == measured and closure_sha256(measured) == digest,
          "AOF-S V3 externally reviewed closure map/digest mismatch")
    return measured
...
def _issue(*, repo_root: Path, result_root: Path, reviewed: Mapping[str, str], digest: str) -> _Capability:
    ...
    _need(not os.path.lexists(result_root) and not os.path.lexists(artifact),
          "AOF-S V3 fresh result/artifact root required")
    parent = result_root.parent
    _need(parent.is_dir() and not parent.is_symlink(), "AOF-S V3 result parent invalid")
    info = parent.stat(follow_symlinks=False)
    return _Capability(_TOKEN, result_root, info.st_dev, info.st_ino, measured, digest,
                       binding.validate_v2_recovery_graph(repo_root), binding.validate_v2_sealed_artifact(repo_root))


def _consume(capability: _Capability, repo_root: Path) -> None:
    _need(isinstance(capability, _Capability) and capability.token is _TOKEN, "AOF-S V3 opaque capability required")
    key = (capability.parent_device, capability.parent_inode, str(capability.result_root))
    _need(key not in _CONSUMED, "AOF-S V3 capability already consumed")
    ...
    _need((info.st_dev, info.st_ino) == (capability.parent_device, capability.parent_inode)
          and not os.path.lexists(capability.result_root), "AOF-S V3 result root/parent drift")
    _need(_reviewed(repo_root, capability.closure, capability.closure_sha256) == capability.closure,
          "AOF-S V3 closure drift before attempt")
    _need(binding.validate_v2_recovery_graph(repo_root) == capability.v2_failure,
          "AOF-S V3 V2 failure predecessor drift")
    _need(binding.validate_v2_sealed_artifact(repo_root) == capability.v2_artifact,
          "AOF-S V3 V2 sealed artifact drift")
    _CONSUMED.add(key)


def _final(capability: _Capability, repo_root: Path) -> None:
    _need(_reviewed(repo_root, capability.closure, capability.closure_sha256) == capability.closure,
          "AOF-S V3 closure drift before terminal")
    _need(binding.validate_v2_recovery_graph(repo_root) == capability.v2_failure,
          "AOF-S V3 V2 failure predecessor drift before terminal")
    _need(binding.validate_v2_sealed_artifact(repo_root) == capability.v2_artifact,
          "AOF-S V3 V2 sealed artifact drift before terminal")


def _profile() -> _lifecycle.LifecycleProfile:
    return _lifecycle.LifecycleProfile(
        attempt_schema="m2_aof_scalar_static_package_v3_attempt",
        attempt_status="LOCAL_BUILD_V3_RESERVED_NO_NETWORK",
        terminal_schema="m2_aof_scalar_static_package_v3_terminal",
        terminal_status="LOCAL_BUILD_V3_VALIDATED_NOT_SUBMITTED",
        failure_schema="m2_aof_scalar_static_package_v3_failure",
        ...
        attempt_authority=lambda cap: {"closure": cap.closure, "closure_sha256": cap.closure_sha256,
                                       "recovery_profile": V3_PROFILE.name, "v2_failure_predecessor": cap.v2_failure},
        predecessor_authority=lambda cap: {"v2_failure": cap.v2_failure, "v2_sealed_artifact": {
...
def _reuse_build(*, repo_root: Path, capability: _Capability) -> Callable[[Path], Mapping[str, Any]]:
    def build(path: Path) -> Mapping[str, Any]:
        _need(path == repo_root / plan.V2_ARTIFACT_ROOT_RELATIVE / plan.PAYLOAD_NAME,
              "AOF-S V3 only reuses exact V2 payload path")
        artifact = repo_root / plan.ARTIFACT_ROOT_RELATIVE
        artifact.mkdir(mode=0o755)
        receipt = capability.v2_artifact["receipt"]
        body = {"schema": "m2_aof_scalar_static_package_v3_reused_v2_payload_authority",
                "v2_artifact_root": capability.v2_artifact["root_relative"],
                "payload": capability.v2_artifact["bodies"][plan.PAYLOAD_NAME],
                "receipt": capability.v2_artifact["bodies"][plan.RECEIPT_NAME],
                "scientific_rebuild": False}
        ...
        return {"payload_sha256": capability.v2_artifact["bodies"][plan.PAYLOAD_NAME],
                "receipt_sha256": capability.v2_artifact["bodies"][plan.RECEIPT_NAME],
                "payload": descriptor, "session_records": receipt["session_records"],
...
def execute_production_local(*, reviewed_closure: Mapping[str, str], reviewed_closure_sha256: str,
    ...
    """Only production path: V2 reuse + host validation + local CLI Docker.

    There are intentionally no callback arguments and no Docker SDK import.
    The inert public CLI cannot issue this opaque capability.
    """
...
def _execute_synthetic_for_test(*, repo_root: Path, result_root: Path,
    ...
    """Private typed test seam covering the same shared lifecycle."""
