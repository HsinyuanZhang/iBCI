#!/usr/bin/env python3
"""Pack M1 proj_add + muscle flat (zero-slope) after a completed HO3 all24 scan.
...
CELL = "M1-MUSCLE-R100-D4-P16-PROJADD-LEARNABLE-FIXED-V1"
N_STEPS = 80
GATE = 1.0e-5
...
def _build_payload(run_root: Path) -> tuple[Path, str, dict[str, Any], dict[str, np.ndarray]]:
    ...
    if meta.get("cell") != CELL or meta.get("identity_interface") != "proj_add":
        raise RuntimeError("run_meta is not muscle proj_add flat P16")
    ...
    if recency_cfg.ladder != "default" or recency_cfg.tier != "fixed" or not recency_cfg.per_layer:
    ...
    if ckpt.get("schema") != CKPT_SCHEMA or int(ckpt.get("epoch", -1)) != epoch or ckpt.get("tier") != "fixed":
    ...
    dest = DEST / "artifacts" / "m1_rift_r100_projadd_muscle_flat.pkl"
...
def main() -> None:
    ...
    image_tag = f"m1-rift-r100-projadd-muscle-flat-s{selection['seed']}-e{selection['epoch']}:v1"
    method_label = (
        f"M1 RIFT R100 proj_add P16 flat muscle official4 s{selection['seed']} e{selection['epoch']} "
    ...
    method_name = f"M1 RIFT muscle proj_add flat s{selection['seed']} e{selection['epoch']}"
    method_description = (
        "M1 RIFT R100 D4/width256 proj_add P16, per-layer fixed zero slopes (flat control), "
    ...
    candidate = {
        ...
        "arm": "m1_rift_r100_projadd_muscle_flat_cached",
        "image_tag": image_tag,
