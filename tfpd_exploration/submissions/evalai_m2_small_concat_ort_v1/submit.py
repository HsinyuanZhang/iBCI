#!/usr/bin/env python3
"""Guarded EvalAI push/register for M2 SMALL concat ORT exact-E.

Default is pack-only: candidate register is false and --execute is refused.
Does nothing unless --execute is passed AND the candidate flips register:true
AND immutable image/payload IDs are confirmed. Do not call --execute from
the pack session.
"""
...
def _already_registered(candidate: dict[str, Any], rows: list[dict[str, Any]]) -> dict[str, Any] | None:
    for item in rows:
        ...
        if method == candidate["method_name"]:
            return item
    return None
...
def _register(candidate: dict[str, Any], state: dict[str, object], runtime: tuple[Any, Any, Any, str, str]) -> dict[str, object]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="m2_concat_ort_evalai_") as tmp:
...
def main() -> None:
    ...
    if candidate.get("evalai_opened") is True:
        raise RuntimeError("candidate evalai_opened is true; refuse")
    if candidate.get("register") is not True:
        if args.execute:
            raise RuntimeError("refusing execute: candidate register is false (pack-only; authorize later)")
        print(
            json.dumps(
                {
                    "mode": "pack_only_blocked",
