"""Falcon runtime for M2 RIFT R50 concat + cached CPU backend.

Architecture is RIFT, backend is cached PyTorch FP32. Not BT-EORT, not ORT,
not SPINT. Banks are the sealed official 13-tag M2 payload (same E0/T as
the RIFT concat train cache on overlapping sessions).
"""
...
PAYLOAD_SCHEMA = "m2_rift_r50_concat_cached_falcon_payload_v1"
CHANNELS = 96
BEHAVIOR_SCALE = 5.0
...
class M2RiftCachedFalconDecoder(BCIDecoder):
    def __init__(self, task_config, model_path: str, batch_size: int = 1):
        ...
        if payload.get("identity_interface") != "concat":
            raise ValueError("payload identity_interface must be concat")
