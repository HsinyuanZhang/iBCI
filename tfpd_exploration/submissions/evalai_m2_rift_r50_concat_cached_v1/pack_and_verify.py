#!/usr/bin/env python3
"""Build the M2 RIFT R50 concat cached payload, host-verify, and docker-pack.

Does not EvalAI-submit. Does not overwrite 582047 or H1 582073 artifacts.
"""
...
CKPT = ROOT / "btransform_unified_v2/results/rift_v1/m2_r50_concat_s42_formal_v1/epoch_007.pt"
SCORE = ROOT / "btransform_unified_v2/results/rift_v1/m2_r50_concat_s42_formal_v1/score_receipt.json"
...
IMAGE_TAG = "m2-rift-r50-concat-cached-e7:v1"
METHOD_LABEL = (
    "M2 RIFT R50 concat e7 EMA cached CPU; architecture=RIFT backend=cached identity=concat; "
    "not BT-EORT; not ORT; not SPINT"
)
METHOD_NAME = "M2 RIFT R50 concat e7 cached"
N_STEPS = 80
GATE = 1.0e-5
SKIP_PKG = {"joint_m2_model.py", "m2_mechanism_model.py"}
...
def _build_payload() -> tuple[Path, str, dict]:
    ...
    if ckpt.get("identity_interface") != "concat" or int(ckpt.get("epoch", -1)) != 7:
        raise RuntimeError("checkpoint is not formal M2 concat e7")
    ...
    dest = DEST / "artifacts" / "m2_rift_r50_concat_e7.pkl"
...
def main() -> int:
    ...
    candidate = {
        "arm": "m2_rift_r50_concat_e7_cached",
        "budget_disclosure": (
            "Official 13-tag M2 banks (33-trial native E0 + MOVE-T4, same sealed banks as 582047). "
            "RIFT R50 concat seed42 EMA e7 after 24-epoch source-seven train. ext4 equal-session "
            f"{selection['equal_session_mean']:.4f} is development selection, not official HO. "
        ...
        "method_description": (
            ...
            f"Same official 13-tag banks as 582047. ext4 pick e7 mean {selection['equal_session_mean']:.4f}. "
            "Not BT-EORT, not ORT, not SPINT."
        ),
