"""Falcon runtime for M2 RIFT R50 proj_add P16 activity_only (carrier T=0) + learned_slope e9.

Architecture is RIFT, identity is proj_add P16 with carrier T=0 (activity_only),
recency is per-layer learned_slope on the unscaled (default) half-life ladder.
Backend is cached PyTorch FP32 via CpuLearnableRecencyRuntime. Not BT-EORT,
not ORT, not SPINT, not 582189 concat, not 582009 P32.
"""
...
PAYLOAD_SCHEMA = "m2_rift_r50_projadd_activity_only_e9_cached_v1"
CHANNELS = 96
BEHAVIOR_SCALE = 5.0
...
def _task_bank(tag: str, row: dict) -> TaskBank:
    ...
    if np.any(carrier != 0.0):
        raise ValueError(f"{tag}: activity_only bank carrier T must be zero")
    return TaskBank(
        ...
        calibration_meta={
            ...
            "trial_count": 33,
            "estimator": "official M2 13-tag native E0 + zero-carrier T (activity_only)",
...
class M2RiftCachedFalconDecoder(BCIDecoder):
    def __init__(self, task_config, model_path: str, batch_size: int = 1):
        ...
        if payload.get("identity") != "activity_only":
            raise ValueError("payload identity must be activity_only")
        ...
        for tag, row in self.bank_by_dataset_tag.items():
            t = np.asarray(row["T"])
            if np.any(t != 0.0):
                raise ValueError(f"{tag}: bank carrier T must be all zeros in activity_only")
        ...
        self._n_predicts = 0
        self.kind = "rift_cached_projadd_activity_only"
