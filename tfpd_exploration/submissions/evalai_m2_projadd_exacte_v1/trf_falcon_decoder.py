"""Self-contained Falcon runtime for M2 proj_add + exact-E.

Identity: tokens = token_mlp([local16 + P(E0)] | carrier4), token_in=20,
P = Linear(50->16, bias=False). Exact-E is causal-k5 frontend reuse plus
last-layer last-query. Temporal layers 1-3 stay full. No cross-window
temporal KV. Does not import tfpd_exploration.
"""
...
SMALL_CFG = TrfConfig(kind="proj_add", token_in=20, temporal_width=256, ffn=512, readout_hidden=128)
...
class SharedSetFrontend(nn.Module):
    ...
    def forward(self, x: torch.Tensor, bank: SessionBank, unit_keep: torch.Tensor) -> torch.Tensor:
        ...
        if e0.dim() == 2:
            proj = self.e0_proj(e0).view(1, 1, n_units, cfg.proj_out)
        else:
            proj = self.e0_proj(e0).unsqueeze(1)
        fused_local = local + proj
        if t4.dim() == 2:
