#!/usr/bin/env python3
"""Guarded EvalAI push/register for packed M2 proj_add P16 exact-E."""
...
def list_submissions(make_request, urls):
...
def load_candidate(manifest: Path) -> dict[str, Any]:
    ...
    required = (
        "arm", "image_tag", "image_id", "payload_sha256",
        "method_label", "method_name", "method_description",
        "budget_disclosure", "state_path",
    )
...
def preflight(candidate):
    ...
    expected = {
        ...
        "ai.eval.exact_e": "true",
        "ai.eval.dataloader_workers": "0",
        "ai.eval.label_budget": "33",
        "ai.eval.old_spint_decoder": "false",
    }
...
def _push_image(candidate, client, image, runtime):
    ...
    client.login(username=username, password=password, registry=auth["authorizationData"][0]["proxyEndpoint"])
    tag = str(uuid.uuid4())
    ...
    state = {
        "schema_version": "m2_projadd_evalai_push_state",
        "arm": candidate["arm"],
...
def _already_registered(candidate, rows):
    for item in rows:
...
def _register(candidate, state, runtime):
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="m2_projadd_evalai_") as tmp:
        ...
        if recovered is not None:
            state.update({"registered": True, "submission_id": recovered["id"], "recovered_after_timeout": True, "post_error": str(exc)})
            _save_state(Path(candidate["state_path"]), state)
            return state
        raise
...
def main() -> None:
    ...
    print(json.dumps({"mode": "execute" if args.execute else "preflight", "arm": candidate["arm"], "quota": limits, "image_id": image.id}, indent=2, sort_keys=True), flush=True)
    if not args.execute:
        return
    ...
    print(json.dumps({"arm": completed["arm"], "submission_id": completed["submission_id"], "image_id": completed["image_id"]}, indent=2, sort_keys=True))
