#!/usr/bin/env python3
"""Guarded EvalAI push/register for the packed M1 RIFT mean-rate4 R100 cached image.
...
POST only when the phase's current daily and concurrent limits permit it.
...
def _quota(phase: Mapping[str, Any], rows: list[dict[str, Any]]) -> dict[str, Any]:
    """Calculate the live phase limits from the phase document, never constants."""
    ...
    return {"today": today, "active": active, "active_ids": active_ids, "total": len(rows),
...
def _require_quota(limits: Mapping[str, Any]) -> None:
    if int(limits["today"]) >= int(limits["max_per_day"]):
...
def _register(candidate: dict[str, Any], state: dict[str, object], runtime: tuple[Any, Any, Any, str, str]) -> dict[str, object]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="m1_muscle_evalai_") as tmp:
