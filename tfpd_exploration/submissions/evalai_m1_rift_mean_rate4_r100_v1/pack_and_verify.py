#!/usr/bin/env python3
"""Pack the selected M1 mean-rate4 R100 EMA as an EvalAI image.

Binds formal_mean_rate4_s42 epoch 4 EMA to carrier_mean_rate4_v1.  Epoch
selection is re-derived from the completed 24-epoch HO3 scan: channel-centered
variance-weighted R², equal-session mean, earliest-max ties.  This program only
builds/verifies a local payload and image; it never submits.
"""
...
RUNNER = V2 / "scripts" / "m1_muscle_r100_multiseed_v1"
...
M1_CELL = "M1-MUSCLE-R100-D4-JOINT-B3S-CONCAT-MULTISEED-V1"
CARRIER_VARIANT = "muscle_response_svd3_mean_rate4_matched_scale"
IDENTITY = "muscle_response_svd3_mean_rate4_matched_scale_sealed"
EXPECTED_SELECTION_EPOCH = 4
EXPECTED_SELECTION_MEAN = 0.5876280069351196
HO_WINDOWS = {"20121004": 1305, "20121017": 1295, "20121024": 1281}
IMAGE_TAG = "m1-rift-mean-rate4-r100-e4:v1"
METHOD_LABEL = (
    "M1 RIFT mean-rate4 R100 concat selected EMA cached CPU; "
    "architecture=RIFT backend=cached identity=muscle_response_svd3_mean_rate4_matched_scale_sealed"
)
METHOD_NAME = "M1 RIFT mean-rate4 R100 selected cached"
PAYLOAD_NAME = "m1_rift_mean_rate4_r100.pkl"
...
'''
for path in (ROOT, DEST, V1_SCRIPTS, V1_SRC, V2_SRC, RUNNER):
...
def selected_epoch(run_root: Path) -> tuple[int, dict[str, Any], dict[str, Any], Path]:
    ...
    if score.get("schema") != "m1_muscle_r100_multiseed_ho_calib_epoch_scan_v1" or score.get("status") != "COMPLETED" or score.get("cell") != M1_CELL:
    ...
    if expected != EXPECTED_SELECTION_EPOCH:
        raise RuntimeError(f"handoff requires selected epoch {EXPECTED_SELECTION_EPOCH}, got {expected}")
    if not np.isclose(values[expected], EXPECTED_SELECTION_MEAN, rtol=0.0, atol=1e-12):
        raise RuntimeError(f"selected HO3 mean {values[expected]} is not the handed-off {EXPECTED_SELECTION_MEAN}")
...
def validate_training_bindings(runner: Any, run_root: Path, carrier_pack: Path, meta: Mapping[str, Any]) -> None:
    ...
    if meta.get("schema") != "m1_muscle_r100_multiseed_train_v1" or meta.get("status") != "FORMAL" or meta.get("cell") != M1_CELL:
        raise RuntimeError("run root is not a formal M1 mean-rate4 multiseed run")
    ...
    if not isinstance(binding, dict) or binding.get("carrier_variant") != CARRIER_VARIANT:
        raise RuntimeError("run metadata is not bound to the mean-rate4 carrier variant")
    ...
    required = {
        "schema": "m1_muscle_r100_multiseed_train_receipt_v1", "status": "COMPLETED", "cell": M1_CELL,
        "arm": meta.get("arm"), "seed": meta.get("seed"), "sampler_seed": meta.get("sampler_seed"),
        "seed_scope": meta.get("seed_scope"), "seed42_original_equivalence": meta.get("seed42_original_equivalence"),
        "epochs": 24, "steps": 159960, "source_hashes": meta.get("source_hashes"),
        "source_contract": meta.get("source_contract"), "b3s": meta.get("b3s"),
        "carrier_binding": binding, "fit_sha256": binding.get("fit_sha256"),
        "baseline_contract": meta.get("baseline_contract"),
...
def build_payload(run_root: Path, carrier_pack: Path) -> tuple[Path, str, dict[str, Any], dict[str, np.ndarray]]:
    ...
    if "ema" not in state:
        raise RuntimeError("selected checkpoint is missing the EMA state")
    ...
    for session in ALL:
        ...
        if not np.array_equal(t_np, expected_t):
            raise RuntimeError(f"sealed mean-rate4 carrier drift for {session}")
    ...
    payload = {"schema": PAYLOAD_SCHEMA, "task": "m1", "context_bins": 100, "bias_mode": "recency",
               "identity_interface": IDENTITY, "behavior_scaling_factor": 1.0,
               ...
               "carrier_fit_sha256": binding["fit_sha256"], "bank_by_dataset_tag": sealed, "seal_validation": {
                   "status": "PASSED", "comparison": "live_joint_mean_rate4_T_raw_M10_vs_sealed_concat", "per_session": seal_parity},
               "ema_state_dict": {key: value.detach().cpu().float().clone() for key, value in concat.state_dict().items()}, "selection": selection}
    dest = DEST / "artifacts" / PAYLOAD_NAME
...
def main() -> int:
    ...
    method_description = (
        "M1 RIFT R100/D4 with sealed mean-rate4 carrier and B3S EMA E0. "
    ...
    budget_disclosure = (
        "Official seven-tag M10 banks; visible held-out calibration used only for "
        "post-training EMA selection; no official test, no TTA. IsHeldOutZeroShot=false."
    )
    docker = None if args.skip_docker else docker_build(payload_sha, method_label)
    candidate = {"arm": "m1_rift_mean_rate4_r100_cached", "evalai_opened": False, "register": False, "payload_sha256": payload_sha,
