"""Falcon runtime for M1 RIFT mean-rate4 carrier R100 concat + cached CPU backend.

Live B3S E0 and the sealed mean-rate4 carrier are written into the seven
M10 banks at pack time. The container loads RiftConcatDecoder + cached KV only.
"""
...
PAYLOAD_SCHEMA = "m1_rift_muscle_r100_cached_falcon_payload_v1"
CHANNELS = 64
BEHAVIOR_SCALE = 1.0
...
def _task_bank(tag: str, row: dict) -> TaskBank:
    ...
    return TaskBank(
        ...
        calibration_meta={
            ...
            "trial_count": 10,
            "estimator": "live B3S E0 on official M10 + sealed muscle_response_svd3_mean_rate4_matched_scale T",
...
class M1RiftCachedFalconDecoder(BCIDecoder):
    def __init__(self, task_config, model_path: str, batch_size: int = 1):
        ...
        if payload.get("identity_interface") != "muscle_response_svd3_mean_rate4_matched_scale_sealed":
            raise ValueError("payload identity_interface must be muscle_response_svd3_mean_rate4_matched_scale_sealed")
