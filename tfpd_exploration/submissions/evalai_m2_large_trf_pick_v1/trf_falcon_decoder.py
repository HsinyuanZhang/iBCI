"""Self-contained Falcon runtime for the M2 causal Transformer picks.

This module is copied into the EvalAI image. It reconstructs the training
frontend + temporal stack + readout from a state_dict and runs a 50-bin
full-window forward on every predict. It does not import tfpd_exploration,
does not instantiate T4CachedIdentityDecoder, and does not reuse KV cache
across windows.
"""
...
PAYLOAD_SCHEMA = "m2_trf_falcon_payload_v1"
WINDOW = 50
CHANNELS = 96
BEHAVIOR_SCALE = 5.0
...
SMALL_CFG = TrfConfig(kind="small", temporal_width=256, ffn=512, readout_hidden=128)
LARGE_CFG = TrfConfig(kind="large", temporal_width=512, ffn=1024, readout_hidden=128)
...
class SharedSetFrontend(nn.Module):
    ...
    def forward(self, x: torch.Tensor, bank: SessionBank, unit_keep: torch.Tensor) -> torch.Tensor:
        ...
        tokens = self.token_norm(self.token_mlp(torch.cat([local, e0, t4], dim=-1)))
...
def build_decoder(kind: str) -> CausalTransformerDecoder:
    if kind == "small":
        return CausalTransformerDecoder(SMALL_CFG)
    if kind == "large":
        return CausalTransformerDecoder(LARGE_CFG)
...
class TrfFalconDecoder(BCIDecoder):
    """BCIDecoder that actually runs the new Transformer, one full window per step."""
    ...
    def __init__(self, task_config, model_path: str, batch_size: int = 1):
        ...
        if int(payload.get("window_size", -1)) != WINDOW:
            raise ValueError("payload window_size must be 50")
        ...
        if len(self.bank_by_dataset_tag) != EXPECTED_SESSION_COUNT:
            raise ValueError("M2 payload must cover exactly 13 calibration sessions")
