"""Route-owned local Docker CLI argv construction; no Docker SDK or network."""
...
def _run(argv: Sequence[str], *, runner: Runner = subprocess.run) -> subprocess.CompletedProcess[str]:
    command = list(argv)
    _need(command and command[0] == plan.DOCKER_EXECUTABLE, "AOF-S V5 requires exact local docker executable")
    _need(all(isinstance(item, str) and item for item in command), "AOF-S V5 malformed Docker argv")
    _need(not {"login", "push", "pull"}.intersection(command[1:]), "AOF-S V5 registry command prohibited")
    result = runner(command, shell=False, check=False, capture_output=True, text=True)
    _need(result.returncode == 0, f"AOF-S V5 Docker command failed: {result.stderr[:300]}")
    return result
...
def build_and_validate_container(*, repo_root: Path, artifact_root: Path, runner: Runner = subprocess.run) -> dict[str, Any]:
    ...
    _need(data_root.is_dir() and not data_root.is_symlink(), "AOF-S V5 local minival data root invalid")
    output = artifact_root / "container_minival"
    _need(not os.path.lexists(output), "AOF-S V5 container minival output must be fresh")
    ...
    _need(executable.is_file() and not executable.is_symlink() and os.access(executable, os.X_OK),
          "AOF-S V5 local docker executable unavailable")
    base = _run([plan.DOCKER_EXECUTABLE, "image", "inspect", "--format", "{{.Id}}", plan.LOCAL_DOCKER_BASE_TAG], runner=runner)
    _need(base.stdout.strip() == plan.LOCAL_DOCKER_BASE_ID, "AOF-S V5 local base image identity drift")
    context, dockerfile = repo_root / "tfpd_exploration/submissions", repo_root / plan.PACKAGE_RELATIVE / "Dockerfile"
    ...
    _need(image_id.startswith("sha256:") and len(image_id) == 71, "AOF-S V5 built image identity malformed")
    ...
    _need(prediction.is_file() and target.is_file(), "AOF-S V5 container minival outputs missing")
