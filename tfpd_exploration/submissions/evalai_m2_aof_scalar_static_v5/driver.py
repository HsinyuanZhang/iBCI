"""AOF-S V5 local admission: shared lifecycle with parent-layout-only repair."""
...
from .profile import V5_PROFILE
...
def closure_map(repo_root: Path = plan.REPO_ROOT) -> dict[str, str]:
    ...
    for relative in plan.STATIC_CLOSURE_RELATIVES:
        ...
        _need(path.is_file() and not path.is_symlink(), f"AOF-S V5 closure missing/symlink leaf: {relative}")
...
def _reviewed(repo_root: Path, mapping: Mapping[str, str], digest: str) -> dict[str, str]:
    ...
    _need(dict(mapping) == measured and closure_sha256(measured) == digest,
          "AOF-S V5 externally reviewed closure map/digest mismatch")
    return measured
...
def _issue(*, repo_root: Path, result_root: Path, reviewed: Mapping[str, str], digest: str) -> _Capability:
    ...
    _need(not os.path.lexists(result_root) and not os.path.lexists(artifact) and not os.path.lexists(artifact.parent),
          "AOF-S V5 fresh result/artifact parent/root required")
    parent = result_root.parent
    _need(parent.is_dir() and not parent.is_symlink(), "AOF-S V5 result parent invalid")
    info = parent.stat(follow_symlinks=False)
    return _Capability(_TOKEN, result_root, info.st_dev, info.st_ino, measured, digest,
                       binding.validate_v4_failure_graph(repo_root), binding.validate_v2_sealed_artifact(repo_root))


def _consume(capability: _Capability, repo_root: Path) -> None:
    _need(isinstance(capability, _Capability) and capability.token is _TOKEN, "AOF-S V5 opaque capability required")
    key = (capability.parent_device, capability.parent_inode, str(capability.result_root))
    _need(key not in _CONSUMED, "AOF-S V5 capability already consumed")
    ...
    _need((info.st_dev, info.st_ino) == (capability.parent_device, capability.parent_inode)
          and not os.path.lexists(capability.result_root), "AOF-S V5 result root/parent drift")
    _need(_reviewed(repo_root, capability.closure, capability.closure_sha256) == capability.closure,
          "AOF-S V5 closure drift before attempt")
    _need(binding.validate_v4_failure_graph(repo_root) == capability.v4_failure, "AOF-S V5 V4 failure predecessor drift")
    _need(binding.validate_v2_sealed_artifact(repo_root) == capability.v2_artifact, "AOF-S V5 V2 sealed artifact drift")
    _CONSUMED.add(key)


def _final(capability: _Capability, repo_root: Path) -> None:
    _need(_reviewed(repo_root, capability.closure, capability.closure_sha256) == capability.closure,
          "AOF-S V5 closure drift before terminal")
    _need(binding.validate_v4_failure_graph(repo_root) == capability.v4_failure,
          "AOF-S V5 V4 failure predecessor drift before terminal")
    _need(binding.validate_v2_sealed_artifact(repo_root) == capability.v2_artifact,
          "AOF-S V5 V2 sealed artifact drift before terminal")


def _create_route_artifact_root(*, repo_root: Path, result_root: Path) -> Path:
    """Create only V5's missing artifacts parent/root after the two-leaf prefix."""
    _need(result_root.is_dir() and not result_root.is_symlink() and set(os.listdir(result_root)) == _PRE_BUILD_PREFIX,
          "AOF-S V5 artifact parent may be created only after attempt/predecessor prefix")
    package = repo_root / plan.PACKAGE_RELATIVE
    artifact = repo_root / plan.ARTIFACT_ROOT_RELATIVE
    artifact_parent = artifact.parent
    _need(package.is_dir() and not package.is_symlink() and artifact_parent.parent == package,
          "AOF-S V5 route-owned artifact package path invalid")
    _need(not os.path.lexists(artifact_parent) and not os.path.lexists(artifact),
          "AOF-S V5 artifact parent/root must remain fresh")
    artifact_parent.mkdir(mode=0o755)
    artifact.mkdir(mode=0o755)
    _need(artifact_parent.is_dir() and not artifact_parent.is_symlink()
          and artifact.is_dir() and not artifact.is_symlink() and set(os.listdir(artifact_parent)) == {artifact.name},
          "AOF-S V5 artifact parent/root topology drift after creation")
    return artifact


def _profile() -> _lifecycle.LifecycleProfile:
    return _lifecycle.LifecycleProfile(
        attempt_schema="m2_aof_scalar_static_package_v5_attempt", attempt_status="LOCAL_BUILD_V5_RESERVED_NO_NETWORK",
        terminal_schema="m2_aof_scalar_static_package_v5_terminal", terminal_status="LOCAL_BUILD_V5_VALIDATED_NOT_SUBMITTED",
        failure_schema="m2_aof_scalar_static_package_v5_failure", artifact_root_relative=plan.ARTIFACT_ROOT_RELATIVE,
        ...
        attempt_authority=lambda cap: {"closure": cap.closure, "closure_sha256": cap.closure_sha256,
                                       "recovery_profile": V5_PROFILE.name, "v4_failure_predecessor": cap.v4_failure},
        predecessor_authority=lambda cap: {"v4_failure": cap.v4_failure, "v2_sealed_artifact": {
            "root_relative": cap.v2_artifact["root_relative"], "root": cap.v2_artifact["root"], "bodies": cap.v2_artifact["bodies"]}},
...
def _reuse_build(*, repo_root: Path, capability: _Capability) -> Callable[[Path], Mapping[str, Any]]:
    def build(path: Path) -> Mapping[str, Any]:
        _need(path == repo_root / plan.V2_ARTIFACT_ROOT_RELATIVE / plan.PAYLOAD_NAME,
              "AOF-S V5 only reuses exact V2 payload path")
        artifact = _create_route_artifact_root(repo_root=repo_root, result_root=capability.result_root)
        receipt = capability.v2_artifact["receipt"]
        body = {"schema": "m2_aof_scalar_static_package_v5_reused_v2_payload_authority",
