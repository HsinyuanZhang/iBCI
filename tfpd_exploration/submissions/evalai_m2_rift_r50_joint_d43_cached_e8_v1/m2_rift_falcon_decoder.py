"""Falcon runtime for M2 RIFT R50 joint D43 + cached CPU backend.

Architecture is RIFT proj_add with sealed live FiLM E0. Backend is cached
PyTorch FP32. Not BT-EORT, not ORT, not SPINT. Carriers are the sealed
official 13-tag MOVE-T4 banks; E0 is the D43 EMA encoder on public M33.
"""
...
PAYLOAD_SCHEMA = "m2_rift_r50_joint_d43_cached_falcon_payload_v1"
CHANNELS = 96
BEHAVIOR_SCALE = 5.0
...
def _task_bank(tag: str, row: dict) -> TaskBank:
    ...
    return TaskBank(
        ...
        calibration_meta={
            ...
            "trial_count": 33,
            "estimator": "joint D43 FiLM live E0 sealed + official MOVE-T4",
...
class M2RiftCachedFalconDecoder(BCIDecoder):
    def __init__(self, task_config, model_path: str, batch_size: int = 1):
        ...
        if payload.get("identity_interface") != "live_film_e0_sealed":
            raise ValueError("payload identity_interface must be live_film_e0_sealed")
        ...
        self.decoder = RiftDecoder("m2", context_bins=CONTEXT_BINS, bias_mode="recency", seed=MODEL_SEED)
