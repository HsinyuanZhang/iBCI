...
class SharedSetFrontend(nn.Module):
    ...
    def forward(self, x: torch.Tensor, bank: SessionBank, unit_keep: torch.Tensor) -> torch.Tensor:
        ...
        return self._slots_from_tokens(tokens, unit_keep)

    def prepare_static(self, bank: SessionBank, batch: int, n_units: int) -> tuple[torch.Tensor, torch.Tensor]:
        cfg = self.cfg
        e0 = bank.E0
        t4 = bank.T
        n_groups = cfg.proj_out // cfg.conv_channels
        if e0.dim() == 2:
            ...
            proj = self.e0_proj(e0).view(e0.shape[0], 1, n_units, n_groups, cfg.conv_channels)
        if t4.dim() == 2:
            t4 = t4.view(1, 1, n_units, cfg.t4_dim)
        else:
            t4 = t4.unsqueeze(1)
        return proj, t4
    ...
    def forward_static(
        ...
        t4 = t4.expand(batch, width, n_units, cfg.t4_dim)
        tokens = self.token_norm(self.token_mlp(torch.cat([fused_local, t4], dim=-1)))
        return self._slots_from_tokens(tokens, unit_keep)

    def _slots_from_tokens(self, tokens: torch.Tensor, unit_keep: torch.Tensor) -> torch.Tensor:
        cfg = self.cfg
...
class _FrontendWindowCache:
    """Boundary-correct k=5 frontend reuse. No temporal KV. In-place buffers."""
    ...
    def __init__(self, frontend, *, window: int, kernel: int = 5) -> None:
        ...
        self._raw_scratch: torch.Tensor | None = None
        self._z_scratch: torch.Tensor | None = None

    def reset(self) -> None:
        self.raw = self.z = self._raw_scratch = self._z_scratch = None
    ...
    def rebuild(self, raw_window: torch.Tensor) -> torch.Tensor:
        ...
        if self.raw is None or self.raw.shape != raw_window.shape:
            self.raw = raw_window.detach().clone()
            self.z = z.detach().clone()
            self._raw_scratch = torch.empty_like(self.raw[:, 1:])
            self._z_scratch = torch.empty_like(self.z[:, 1:])
        else:
            self.raw.copy_(raw_window)
            self.z.copy_(z)
        return self.z

    def advance(self, next_bin: torch.Tensor) -> torch.Tensor:
        if self.raw is None or self.z is None:
            raise RuntimeError("advance requires rebuild after reset")
        if next_bin.ndim != 3 or next_bin.shape[1:] != (1, self.raw.size(2)):
            raise ValueError("next_bin must be [B,1,N] with the compiled unit roster")
        self._raw_scratch.copy_(self.raw[:, 1:])
        self.raw[:, :-1].copy_(self._raw_scratch)
        self.raw[:, -1:].copy_(next_bin)
        pair = getattr(self, "frontend_pair", None)
        if pair is not None:
            left, right = pair(self.raw[:, : self.kernel - 1], self.raw[:, -self.kernel :])
        else:
            left = self.frontend(self.raw[:, : self.kernel - 1])
            right = self.frontend(self.raw[:, -self.kernel :])[:, -1:]
        self._z_scratch.copy_(self.z[:, 1:])
        self.z[:, :-1].copy_(self._z_scratch)
        self.z[:, : self.kernel - 1].copy_(left)
        self.z[:, -1:].copy_(right)
        return self.z
...
class _ExactEEngine:
    ...
    def __init__(self, model: CausalTransformerDecoder, banks: list[SessionBank]) -> None:
        ...
        self.keep = keep.contiguous()
        n_units = int(self.bank.E0.shape[-2] if self.bank.E0.dim() == 3 else self.bank.E0.shape[0])
        self.proj, self.t4 = self.model.frontend.prepare_static(self.bank, self.keep.size(0), n_units)
        self.cache = _FrontendWindowCache(self._frontend, window=WINDOW, kernel=5)
        self.cache.frontend_pair = self._frontend_pair

    def _frontend(self, x: torch.Tensor) -> torch.Tensor:
        keep = self.keep
        if keep.size(0) == 1 and x.size(0) > 1:
            keep = keep.expand(x.size(0), -1)
        elif keep.size(0) != x.size(0):
            keep = keep[: x.size(0)]
        return self.model.frontend.forward_static(x, self.proj, self.t4, keep.contiguous())

    def _frontend_pair(self, left_raw: torch.Tensor, right_raw: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        # Pad left T=4 to T=5 so both sides share one frontend GEMM. Dummy last
        # bin cannot change the first 4 causal conv outputs.
        pad = torch.zeros(
            left_raw.size(0),
            right_raw.size(1) - left_raw.size(1),
            left_raw.size(2),
            device=left_raw.device,
            dtype=left_raw.dtype,
        )
        stacked = torch.cat((torch.cat((left_raw, pad), dim=1), right_raw), dim=0)
        keep = torch.cat((self.keep, self.keep), dim=0)
        if self.proj.size(0) == 1:
            proj, t4 = self.proj, self.t4
        else:
            proj = torch.cat((self.proj, self.proj), dim=0)
            t4 = torch.cat((self.t4, self.t4), dim=0)
        fused = self.model.frontend.forward_static(stacked, proj, t4, keep)
        left_w = left_raw.size(1)
        return fused[: left_raw.size(0), :left_w], fused[left_raw.size(0) :, -1:]
