...
Vendored into the EvalAI image. Reconstructs Conv1→16 k5, concat(local,E0,H-C),
8-slot set attention d=256, 4-layer current-query stack, 256→128→16 last-bin,
native scale 1. Finite W=100/k=5 V3 stream. Does not import tfpd_exploration.
on_done is a no-op.
"""
...
class M1TemporalFalconDecoder(BCIDecoder):
    """BCIDecoder that advances the V3 current-query stream one public bin per step."""
    ...
    def reset(self, dataset_tags: Iterable[Path] = (Path(""),)):
        ...
        if missing:
            raise ValueError(f"Dataset tags {missing} are absent from the M1 V3 bank payload")
        self.device = torch.device("cpu")
    ...
    def observe(self, neural_observations: np.ndarray):
        ...
        # Official M1 bins are uint8 from falcon_challenge.bin_units; the
        # evaluator also yields float64 and non-contiguous slices.  Coerce
        # here so V3 still sees finite C-contiguous float32.
        observations = np.ascontiguousarray(observations, dtype=np.float32)
        if not np.isfinite(observations).all():
            raise ValueError("V3 requires finite observations")
