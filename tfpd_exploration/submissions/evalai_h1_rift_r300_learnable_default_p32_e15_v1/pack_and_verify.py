#!/usr/bin/env python3
"""Build the H1 RIFT R300 learned_slope default P32 e15 cached payload, host-verify, and docker-pack.

Seals signed_state14 T + rematerialized C2 E0 + learned_slope temporal.
Does not EvalAI-submit. Does not overwrite 582241 (P16 e16) or 582196.
"""
...
CKPT = ROOT / "btransform_unified_v2/learnable_recency_v1/results/h1_learned_slope_default_p32_s42/epoch_015.pt"
CKPT_SHA = "4676c437078373ecd50580374f1468da0fe712954b9d3d863b68c8fa8b144879"
SCORE = ROOT / "btransform_unified_v2/learnable_recency_v1/results/h1_learned_slope_default_p32_s42/ho_m3_selection.json"
RUN_META = ROOT / "btransform_unified_v2/learnable_recency_v1/results/h1_learned_slope_default_p32_s42/run_meta.json"
...
IMAGE_TAG = "h1-rift-r300-learned-slope-default-p32-e15:v1"
METHOD_LABEL = (
    "H1 RIFT R300 learned_slope default-ladder P32 e15 EMA cached CPU; architecture=RIFT backend=cached "
    "identity=signed_state14 recency=learned_slope_default; not BT-EORT; not ORT; not SPINT; not 582241; not 582196"
)
METHOD_NAME = "H1 RIFT R300 learned_slope default P32 e15 cached"
METHOD_DESCRIPTION = (
    "H1 RIFT R300 P32 with per-layer learned_slope on default unscaled half-life ladder, epoch 15 EMA. "
    "HO-M3 grouped-seven pick e15 r2_mean 0.4797 worst 0.2984. Cached PyTorch FP32 CPU backend. "
    "Not BT-EORT, not ORT, not SPINT, not 582241 P16 e16, not 582196."
)
N_STEPS = 80
GATE = 1.0e-5
...
def _build_payload() -> tuple[Path, str, dict]:
    ...
    if int(run_meta.get("proj_dim", -1)) != 32:
        raise RuntimeError(f"run_meta proj_dim drifted: {run_meta.get('proj_dim')}")
    ...
    if ckpt.get("variant") != "learnable_learned_slope" or int(ckpt.get("context_bins", 0)) != 300:
        raise RuntimeError("checkpoint is not formal H1 R300 learnable_learned_slope")
    ...
    dest = DEST / "artifacts" / "h1_rift_r300_learned_slope_default_p32_e15.pkl"
...
"""
    ...
    candidate = {
        "arm": "h1_rift_r300_learned_slope_default_p32_e15_cached",
        "task": "h1",
        "image_tag": IMAGE_TAG,
        "image_id": image_id,
