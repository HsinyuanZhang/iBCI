"""Guarded EvalAI submission helper for the full-block m30 activity-30 static M2 image.
...
the single network step (the coordinator push).
"""
...
METHOD_LABEL = (
    "Full-block m30 activity-30 static M2 cached identity (chronological "
    "first-30 ridge T4 fit; B3S activity pool = first 30 label-free calibration trials)"
)
...
CANDIDATE: dict[str, Any] = {
    ...
    "method_name": "M2 full-block m30 static ridge T4 seed42, activity-30 pool",
    "method_description": (
        "Frozen SPINT M2 decoder with per-session cached identity calibrated "
        "offline from the chronological first-30 public calibration block "
        "(ridge lambda=0.1 T4 fit consuming the block's finite-direction "
        "trials; B3S activity pool = the same full label-free first-30 "
        "calibration block, mean-pooled by the encoder). Static full-block "
        "deployment: no adaptive memory, no online update. The EvalAI runtime "
        "serves cached identities only and performs no hidden-query label "
        "read, optimizer step, parameter update, or backpropagation."
    ),
    "budget_disclosure": (
        "30-trial chronological calibration block: the ridge T4 fit consumes "
        "the finite-direction (labelled) subset of the block; activity pool = "
        "the first 30 calibration trials (label-free neural activity only); "
        "zero online updates"
    ),
}
...
def state_path() -> Path:
    return Path(__file__).resolve().parent / "artifacts" / "evalai_push_state_m30_act30_v1.json"
...
def _push_image(client: Any, image: Any, runtime: tuple[Any, Any, Any, str, str]) -> dict[str, object]:
    ...
    state = {
        "schema_version": "m2_m30_act30_evalai_push_state_v1",
        "arm": CANDIDATE["arm"],
        "image_tag": CANDIDATE["image_tag"],
        "image_id": CANDIDATE["image_id"],
        "payload_sha256": CANDIDATE["payload_sha256"],
...
def _register(state: dict[str, object], runtime: tuple[Any, Any, Any, str, str]) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="m2_m30_act30_evalai_") as tmp:
