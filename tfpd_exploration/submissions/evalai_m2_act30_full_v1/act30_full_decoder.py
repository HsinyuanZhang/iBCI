"""FALCON runtime for the frozen full-block m30 activity-30 M2 cached-identity submission.
...
identities were produced offline (build time): the chronological first-30
calibration block serves as the carrier support (ridge lambda=0.1 T4 fit on
its finite-direction trials) and as the label-free B3S activity pool (the B3S
encoder mean-pools it internally).  Nothing about that calibration phase
exists inside this runtime module: it imports no selection law, reads no trial
metadata, and holds no optimizer state.  All state carried across ``predict``
calls consists of frozen weights plus the cached identity tensors and the
fixed neural history buffers.
"""
...
class Act30FullM2Decoder(BCIDecoder):
    ...
    states produced offline from each session's chronological first-30
    calibration block: the ridge T4 carrier fit consumes the block's
    finite-direction (labelled) trials, and the B3S activity pool is the full
    label-free block.  No calibration data, selection law, optimizer state,
    ...
    def __init__(self, task_config, model_path: str, batch_size: int = 1):
        ...
        # Bind the runtime to the full-block m30 activity-30 package so a
        # payload from any other arm cannot be silently served by this image.
        ...
        if int(metadata.get("label_budget", -1)) != PAYLOAD_LABEL_BUDGET:
            raise ValueError("payload label budget is not the chronological first-30 block")
