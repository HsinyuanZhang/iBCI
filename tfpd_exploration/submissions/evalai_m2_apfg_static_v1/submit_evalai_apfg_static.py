"""Guarded EvalAI submission helper for the static-pool APFG M2 image (exploratory probe).
...
METHOD_LABEL = (
    "Static-pool APFG M2 cached identity (anchored post-fusion scalar gate "
    "frozen at build time; D-opt k=4 from D-opt first-30 selection; ridge T4 "
    "k=4; B3S activity pool = first 30 label-free calibration trials; "
    "exploratory probe, no online state)"
)

METHOD_NAME = "M2 static-pool APFG (anchored post-fusion gate, exploratory probe)"

METHOD_DESCRIPTION = (
    "Frozen SPINT M2 decoder with per-session cached identity calibrated "
    "offline exactly like the officially scored D-opt-4 activity-30 arm, plus "
    "one anchored post-fusion scalar gate applied at build time: E = native + "
    "tanh(alpha) * (post - native), alpha frozen at the source-refit value "
    "bound to the immutable same-surface result graph. Static deployment: no "
    "adaptive memory, no online update, no query-label read. The EvalAI "
    "runtime serves cached identities only and performs no optimizer step, "
    "parameter update, or backpropagation. Exploratory probe: the local "
    "external point estimate did not pass the pre-registered promotion gate."
)
...
BUDGET_DISCLOSURE = (
    ...
    "trials (label-free neural activity only); one scalar gate weight "
    "transferred from source-session-only training; zero online updates"
)
...
RECEIPT_PATH = HERE / "artifacts" / "t4_m2_seed42_dopt4_act30_apfg_identity.receipt.json"
...
def _candidate() -> dict[str, Any]:
    ...
    return {
        "arm": ARM,
        "image_tag": f"spint-t4-m2:apfg-static-s42-{payload_sha256[:8]}",
        "image_id": "",
...
def state_path() -> Path:
    return HERE / "artifacts" / "evalai_push_state_apfg_static_v1.json"
...
def plan_report(candidate: dict[str, Any]) -> dict[str, object]:
    return {
        ...
        "frozen_alpha": candidate["frozen_alpha"],
...
def _validate_local_image(docker_module: Any, candidate: dict[str, Any]) -> tuple[Any, Any]:
    ...
    if float(labels.get("ai.eval.apfg.alpha", "nan")) != FROZEN_ALPHA:
        raise RuntimeError("image label ai.eval.apfg.alpha drift")
    return client, image
...
def _push_image(client: Any, image: Any, candidate: dict[str, Any], runtime: tuple[Any, Any, Any, str, str]) -> dict[str, object]:
    ...
    state = {
        "schema_version": "m2_apfg_static_evalai_push_state_v1",
        "arm": candidate["arm"],
        "image_tag": candidate["image_tag"],
...
def _register(state: dict[str, object], candidate: dict[str, Any], runtime: tuple[Any, Any, Any, str, str]) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="m2_apfg_static_evalai_") as tmp:
