"""Falcon runtime for M2 RIFT R50 static_identity + learned_slope e15.

No TaskBank, no E0, no e0_proj, no carrier. Shared static_identity[96,16]
...
PAYLOAD_SCHEMA = "m2_rift_r50_static_e15_cached_v1"
CHANNELS = 96
BEHAVIOR_SCALE = 5.0
...
def build_decoder(recency_cfg: LearnableRecencyConfig) -> StaticLearnableRiftDecoder:
    ...
    return StaticLearnableRiftDecoder("m2", recency_cfg, seed=SEED)
...
class M2RiftCachedFalconDecoder(BCIDecoder):
    def __init__(self, task_config, model_path: str, batch_size: int = 1):
        ...
        if self.official_tags != OFFICIAL_TAGS or len(self.official_tags) != EXPECTED_SESSION_COUNT:
            raise ValueError("M2 payload must cover exactly the 13 official dataset tags")
    ...
    def reset(self, dataset_tags: Iterable[Path] = (Path(""),)):
        ...
        if missing:
            raise ValueError(f"Dataset tags {missing} are absent from the M2 static payload")
    ...
    def predict(self, neural_observations: np.ndarray) -> np.ndarray:
        ...
        if not np.isfinite(native).all():
            raise RuntimeError("non-finite M2 static prediction")
        self._n_predicts += 1
