#!/usr/bin/env python3
"""Local contract validation + reference scoring for the D-opt-4 static M2 image.
...
2. ``external``  - replay the LOCAL continual evaluator semantics
                   ...
                   and compare with the sealed static_m4 screen rows
...
def load_sealed_rows() -> dict[tuple[str, str], dict[str, Any]]:
    ...
    require(len(rows) == 13, "sealed static_m4 row coverage drift")
    return rows
...
def audit_payload(payload_path: Path) -> dict[str, Any]:
    ...
    from dopt_static_decoder import CPUUnpickler, PAYLOAD_ARM, PAYLOAD_SCHEMA
    ...
    require(metadata["arm"] == PAYLOAD_ARM, "payload arm drift")
    require(metadata["label_budget"] == 4 and metadata["activity_budget"] == 4, "budget drift")
...
class ImportBlocker(importlib.abc.MetaPathFinder):
    """Poison the frozen selection/carrier laws so any runtime import fails."""
        ...
        for surface_name, dataset, expected_count in surfaces:
            ...
            for session in sessions:
                ...
                require(
                    ...
                    f"{session} ({surface_name}) r2 drift vs sealed static_m4: "
            ...
            sealed_values = np.asarray(
                [row["sealed_screen_static_m4_r2"] for row in per_session.values()]
            )
...
def main() -> None:
    ...
    parser.add_argument("--payload", type=Path, default=ARTIFACTS / "t4_m2_seed42_dopt4_static_identity.pkl")
    parser.add_argument("--output", type=Path, default=ARTIFACTS / "local_validation_receipt.json")
    args = parser.parse_args()
    if not args.execute:
        print(json.dumps({
            "schema": "m2_dopt4_static_local_validation_dry_v1",
    ...
    receipt: dict[str, Any] = {
        "schema_version": "m2_dopt4_static_local_validation_v1",
