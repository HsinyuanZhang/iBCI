"""FALCON runtime for the frozen D-opt-4 static M2 cached-identity submission.
...
exactly the four selected trials (B3S mean-pools them internally).  Nothing
about that calibration phase exists inside this runtime module: it imports no
selection law, reads no trial metadata, and holds no optimizer state.  All
state carried across ``predict`` calls consists of frozen weights plus the
cached identity tensors and the fixed neural history buffers.
"""
...
class DoptStaticM2Decoder(BCIDecoder):
    ...
    trials (greedy D-opt k=4 within the first-30 finite-angle candidates) and
    their target labels.  No calibration data, selection law, optimizer state,
    ...
    def __init__(self, task_config, model_path: str, batch_size: int = 1):
        ...
        # Bind the runtime to the D-opt-4 static package so a payload from any
        # other arm cannot be silently served by this image.
        ...
        if int(metadata.get("activity_budget", -1)) != 4:
            raise ValueError("payload activity budget is not the static four-trial pool")
