#!/usr/bin/env python3
"""Build the M2 RIFT R50 concat e9 cached payload, host-verify, and docker-pack.

Does not EvalAI-submit. Does not overwrite 582047, 582128, or H1 582073.
"""
...
CKPT = ROOT / "btransform_unified_v2/results/rift_v1/m2_r50_concat_s42_formal_v1/epoch_009.pt"
CKPT_SHA = "78435b8e01b0beaa78057c918ec195558d9c100a692ab7a8c066766f675ac88b"
SELECTED_EMA = ROOT / "btransform_unified_v2/results/rift_v1/m2_r50_concat_s42_ext6_pick_v1/selected_ema.pt"
SELECTED_EMA_SHA = "8d156b28a9d144ebcd39f1597c7eb1af4a4e186a3af397b2442c911b5076418f"
SCORE = ROOT / "btransform_unified_v2/results/rift_v1/m2_r50_concat_s42_ext6_pick_v1/score_receipt.json"
...
IMAGE_TAG = "m2-rift-r50-concat-cached-e9:v1"
METHOD_LABEL = (
    "M2 RIFT R50 concat e9 EMA cached CPU; architecture=RIFT backend=cached identity=concat; "
    "ext6 pick; not BT-EORT; not ORT; not SPINT"
)
METHOD_NAME = "M2 RIFT R50 concat e9 cached"
N_STEPS = 80
GATE = 1.0e-5
...
def _build_payload() -> tuple[Path, str, dict]:
    ...
    if ckpt.get("schema") != "m2_rift_concat_epoch_checkpoint_v1":
        raise RuntimeError("checkpoint schema is not M2 RIFT concat")
    if ckpt.get("identity_interface") != "concat" or int(ckpt.get("epoch", -1)) != 9:
        raise RuntimeError("checkpoint is not formal M2 concat e9")
    if int(ckpt.get("context_bins", 0)) != 50 or ckpt.get("bias_mode") != "recency":
        raise RuntimeError("checkpoint geometry is not R50 recency")
    ...
    ema_state = {k: v.detach().cpu().float().clone() for k, v in current.items()}
    payload = {
    ...
    dest = DEST / "artifacts" / "m2_rift_r50_concat_e9.pkl"
...
def main() -> int:
    ...
    candidate = {
        "arm": "m2_rift_r50_concat_e9_cached",
        "budget_disclosure": (
            "Official 13-tag M2 banks (33-trial native E0 + MOVE-T4, same sealed banks as 582047). "
            "RIFT R50 concat seed42 EMA e9 after 24-epoch source-seven train. ext6 equal-session "
        ...
        "method_description": (
            ...
            "D4/width256, concat identity [local16|E0_50|T4], recency half-lives, cached CPU KV. "
            f"Same official 13-tag banks as 582047. ext6 pick e9 mean {selection['equal_session_mean']:.6f}. "
            "Not BT-EORT, not ORT, not SPINT. Not 582128 e7."
        ),
