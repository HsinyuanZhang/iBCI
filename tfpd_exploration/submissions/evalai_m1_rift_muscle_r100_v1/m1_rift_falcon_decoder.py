"""Falcon runtime for M1 RIFT muscle-carrier R100 concat + cached CPU backend.

Live B3S E0 and the official muscle-response carrier are sealed into the seven
...
def _task_bank(tag: str, row: dict) -> TaskBank:
    ...
    return TaskBank(
        ...
        calibration_meta={
            ...
            "trial_count": 10,
            "estimator": "live B3S E0 on official M10 + sealed muscle_response16_svd4/global_rms T",
...
class M1RiftCachedFalconDecoder(BCIDecoder):
    def __init__(self, task_config, model_path: str, batch_size: int = 1):
        ...
        if payload.get("identity_interface") != "muscle_response16_svd4/global_rms_sealed":
            raise ValueError("payload identity_interface must be muscle_response16_svd4/global_rms_sealed")
