"""Falcon runtime for M2 RIFT R50 proj_add P16 + per-layer learned_slope.

Architecture is RIFT, identity is proj_add P16, recency is per-layer
learned_slope on the unscaled (default) half-life ladder. Backend is
cached PyTorch FP32 via CpuLearnableRecencyRuntime. Not BT-EORT, not ORT,
not SPINT, not 582189 concat, not 582009 P32.
"""
...
PAYLOAD_SCHEMA = "m2_rift_r50_projadd_learnable_default_e10_cached_v1"
CHANNELS = 96
BEHAVIOR_SCALE = 5.0
...
def _task_bank(tag: str, row: dict) -> TaskBank:
    ...
    return TaskBank(
        ...
        calibration_meta={
            ...
            "trial_count": 33,
            "estimator": "official M2 13-tag native E0 + MOVE-T4 (sealed 582047 banks)",
...
class M2RiftCachedFalconDecoder(BCIDecoder):
    def __init__(self, task_config, model_path: str, batch_size: int = 1):
        ...
        self._n_predicts = 0
        self.kind = "rift_cached_learnable_projadd"
