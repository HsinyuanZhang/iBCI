#!/usr/bin/env python3
"""Build the M2 RIFT R50 proj_add P16 learned_slope default-ladder e10 payload.

Host-verify against CpuLearnableRecencyRuntime, then docker-pack.
Does not EvalAI-submit. Does not overwrite 582189, 582009, 582047, or concat e9.
"""
...
RUN = ROOT / "btransform_unified_v2/learnable_recency_v1/results/m2_projadd_learned_slope_default_s43"
CKPT = RUN / "epoch_011.pt"
CKPT_SHA = "b2570e47d4c97f5e2228eb76b3f91ccdafbc98399c9d1d9a57bb64f0246b4a2e"
SCORE = ROOT / "btransform_unified_v2/learnable_recency_v1/results/selection_m2_projadd_learned_slope_default_s43_ext6/score_receipt.json"
...
IMAGE_TAG = "m2-rift-r50-projadd-learnable-default-s43-e11:v1"
METHOD_LABEL = (
    "M2 RIFT R50 proj_add P16 learned_slope default-ladder s43 e11 EMA cached CPU; "
    "architecture=RIFT backend=cached identity=proj_add; ext6 pick; "
    "not BT-EORT; not ORT; not SPINT; not 582189 concat; not 582009 P32"
)
METHOD_NAME = "M2 RIFT R50 proj_add learned default s43 e11"
N_STEPS = 80
GATE = 1.0e-5
...
def _build_payload() -> tuple[Path, str, dict]:
    ...
    if int(selected["epoch"]) != 11:
        raise RuntimeError(f"ext6 score receipt selection drifted: {selected}")
    if abs(float(selected["equal_session_mean"]) - 0.3668221300454653) > 1.0e-12:
        raise RuntimeError(f"ext6 e11 mean drifted: {selected}")
    ...
    if int(ckpt.get("epoch", -1)) != 11 or ckpt.get("tier") != "learned_slope":
        raise RuntimeError("checkpoint is not learned_slope e11")
    ...
    payload = {
        ...
        "selection": {
            ...
            "pooled_r2": float(score["ema_by_epoch"]["11"]["pooled_r2"]),
            "n_windows": 15403,
            "official_test_used": False,
        },
    }
    dest = DEST / "artifacts" / "m2_rift_r50_projadd_learnable_default_s43_e11.pkl"
...
def main() -> int:
    ...
    candidate = {
        "arm": "m2_rift_r50_projadd_learnable_default_s43_e11_cached",
        "budget_disclosure": (
            "Official 13-tag M2 banks (33-trial native E0 + MOVE-T4, same sealed banks as 582047). "
            "RIFT R50 proj_add P16 seed43 EMA e11 after 24-epoch source-seven train, "
        ...
        "method_description": (
            ...
            "D4/width256, proj_add P16 identity, per-layer learned recency slopes on the "
            "default (H1-scale) half-life ladder, cached CPU KV. "
            f"Same official 13-tag banks as 582047. ext6 pick e11 mean {selection['equal_session_mean']:.6f}. "
            "Not BT-EORT, not ORT, not SPINT. Not 582189 concat e9, not 582009 P32."
        ),
