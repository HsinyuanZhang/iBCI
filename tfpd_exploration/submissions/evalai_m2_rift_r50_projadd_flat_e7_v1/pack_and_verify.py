#!/usr/bin/env python3
"""Build the M2 RIFT R50 proj_add P16 flat (zero-slope) e7 payload.

Host-verify against CpuLearnableRecencyRuntime, then docker-pack.
Does not EvalAI-submit. Does not overwrite 582240 learned or 582189 concat.
"""
...
RUN = ROOT / "btransform_unified_v2/learnable_recency_v1/results/m2_projadd_flat_p16_s42"
CKPT = RUN / "epoch_007.pt"
CKPT_SHA = "87fa94187887e5c74b7288104bf00b9e0534894bc3f11b6251fa24ede14bf650"
SCORE = ROOT / "btransform_unified_v2/learnable_recency_v1/results/selection_m2_projadd_flat_p16_s42_ext6/score_receipt.json"
...
IMAGE_TAG = "m2-rift-r50-projadd-flat-e7:v1"
METHOD_LABEL = (
    "M2 RIFT R50 proj_add P16 flat zero-slope e7 EMA cached CPU; "
    "architecture=RIFT backend=cached identity=proj_add; ext6 all24 pick; "
    "not BT-EORT; not ORT; not SPINT; not 582240 learned; not 582189 concat"
)
METHOD_NAME = "M2 RIFT R50 proj_add flat e7"
N_STEPS = 80
GATE = 1.0e-5
...
def _build_payload() -> tuple[Path, str, dict]:
    ...
    if sha256(CKPT) != CKPT_SHA:
        raise RuntimeError("epoch_007.pt SHA drift")
    with open(BANK_PAYLOAD, "rb") as handle:
        bt = CPUUnpickler(handle).load()
    banks = bt["bank_by_dataset_tag"]
    if len(banks) != 13:
        raise RuntimeError(f"expected 13 official tags, got {len(banks)}")
    score = json.loads(SCORE.read_text())
    if set(score.get("ema_by_epoch", {})) != {str(epoch) for epoch in range(1, 25)}:
        raise RuntimeError("ext6 receipt is not a complete 24-epoch scan")
    selected = score["selection"]
    if (
        int(selected["epoch"]) != 7
        or selected.get("rule") != "earliest maximum finite unweighted equal_session_mean"
    ):
        raise RuntimeError(f"ext6 score receipt selection drifted: {selected}")
    if abs(float(selected["equal_session_mean"]) - 0.36109424441104815) > 1.0e-12:
        raise RuntimeError(f"ext6 e7 mean drifted: {selected}")
    meta = json.loads((RUN / "run_meta.json").read_text())
    recency_cfg = config_from_run_meta(meta, "m2")
    if (
        recency_cfg.ladder != "default"
        or recency_cfg.tier != "fixed"
        or not recency_cfg.per_layer
        or any(half is not None for half in recency_cfg.half_life_seconds)
    ):
        raise RuntimeError(f"run_meta learnable_config drifted: {recency_cfg}")
    ckpt = torch.load(CKPT, map_location="cpu", weights_only=False)
    if ckpt.get("schema") != "m2_rift_projadd_learnable_epoch_checkpoint_v1":
        raise RuntimeError("checkpoint schema is not M2 proj_add learnable")
    if ckpt.get("identity_interface") != "proj_add" or int(ckpt.get("proj_dim", -1)) != 16:
        raise RuntimeError("checkpoint is not proj_add P16")
    if int(ckpt.get("epoch", -1)) != 7 or ckpt.get("tier") != "fixed":
        raise RuntimeError("checkpoint is not flat/fixed e7")
    ...
    if slopes.shape != (8,) or float(slopes.abs().max()) > 0.0:
        raise RuntimeError(f"flat EMA slopes are not exact zeros: {slopes.tolist()}")
    ema_state = {k: v.detach().cpu().float().clone() for k, v in model.state_dict().items()}
    payload = {
        "schema": PAYLOAD_SCHEMA,
        "task": "m2",
        "context_bins": 50,
        "bias_mode": "learnable_fixed",
        ...
        "selection": {
            ...
            "pooled_r2": float(score["ema_by_epoch"]["7"]["pooled_r2"]),
            "n_windows": 15403,
            "official_test_used": False,
        },
    }
    dest = DEST / "artifacts" / "m2_rift_r50_projadd_flat_e7.pkl"
...
def main() -> int:
    ...
    candidate = {
        "arm": "m2_rift_r50_projadd_flat_e7_cached",
        "budget_disclosure": (
            "Official 13-tag M2 banks (33-trial native E0 + MOVE-T4, same sealed banks as 582047). "
            "RIFT R50 proj_add P16 seed42 EMA e7 after 24-epoch source-seven train, "
            "flat control: per-layer fixed zero recency slopes. "
            f"ext6 all24 earliest-max equal-session {selection['equal_session_mean']:.6f} is development selection, "
        ...
        "method_description": (
            ...
            "D4/width256, proj_add P16 identity, flat control with per-layer zero recency slopes, "
            "cached CPU KV. "
            f"Same official 13-tag banks as 582047. ext6 all24 pick e7 mean {selection['equal_session_mean']:.6f}. "
            "Not BT-EORT, not ORT, not SPINT. Not 582240 learned, not 582189 concat."
        ),
