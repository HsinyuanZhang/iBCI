"""Falcon runtime for M2 RIFT R50 proj_add P16 + flat (zero-slope) recency.

Architecture is RIFT, identity is proj_add P16, recency is per-layer
fixed zero slopes (flat control). Backend is cached PyTorch FP32 via
CpuLearnableRecencyRuntime. Not BT-EORT, not ORT, not SPINT, not 582240
learned, not 582189 concat.
"""
...
PAYLOAD_SCHEMA = "m2_rift_r50_projadd_flat_e7_cached_v1"
CHANNELS = 96
BEHAVIOR_SCALE = 5.0
...
def build_decoder(recency_cfg: LearnableRecencyConfig) -> RiftDecoder:
    """Rebuild the trained proj_add + per-layer flat (fixed zero-slope) stack."""
