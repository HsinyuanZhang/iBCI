"""Falcon runtime for M1 RIFT R100 proj_add P16 + muscle learned_slope.

Identity is proj_add P16. Recency is per-layer learned_slope on the default
ladder. The seven sealed banks keep legacy fullsession E0 and replace only
...
PAYLOAD_SCHEMA = "m1_rift_r100_projadd_muscle_learnable_cached_v1"
...
def build_decoder(recency_cfg: LearnableRecencyConfig, seed: int) -> RiftDecoder:
    """Rebuild proj_add + per-layer learned_slope without wrap.py."""
...
class M1RiftCachedFalconDecoder(BCIDecoder):
    def __init__(self, task_config, model_path: str, batch_size: int = 1):
        ...
        self._n_predicts = 0
        self.kind = "rift_cached_learnable_projadd_muscle"
