#!/usr/bin/env python3
"""Guarded EvalAI push/register for the packed H1 proj_add stage-2 exact-E image.
...
def _push_image(candidate: dict[str, Any], client: Any, image: Any, runtime: tuple[Any, Any, Any, str, str]) -> dict[str, object]:
    ...
    state = {
        "schema_version": "h1_projadd_evalai_push_state",
        "arm": candidate["arm"],
...
def _register(candidate: dict[str, Any], state: dict[str, object], runtime: tuple[Any, Any, Any, str, str]) -> dict[str, object]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="h1_projadd_evalai_") as tmp:
