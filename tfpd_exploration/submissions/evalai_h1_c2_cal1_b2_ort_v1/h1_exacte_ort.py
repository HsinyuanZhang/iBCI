"""ONNX Runtime CPU backend for the fast exact-E H1 proj_add runtime.

Port of ``m1_projadd_runtime_v1/20260907T020150Z/scripts/m1_exacte_ort.py``
onto H1 geometry (submission 582025, P16 L=200 e24): the per-bin advance
path (set-attention over the 5 changed positions -> z window assembly ->
...
semantics (SDPA last-query, windowed sinusoidal PE, LayerNorm, k=5 boundary
recompute); no cross-window temporal KV reuse.
...
for all 27 official H1 dataset tags); otherwise the engine refuses and the
caller falls back to the eager fast path.
"""
...
import h1_exacte_fast as fast  # noqa: E402
...
class _OrtAdvanceCore(nn.Module):
    ...
    def forward(
        ...
        tokens = self.token_norm(
            self.token_mlp(torch.cat([fused, t4_static.expand(batch, P, n_units, t4_static.shape[-1])], dim=-1))
        )
...
class _OrtRebuildCore(nn.Module):
    ...
    def forward(self, window: torch.Tensor, proj_static: torch.Tensor, t4_static: torch.Tensor) -> torch.Tensor:
        ...
        tokens = self.token_norm(
            self.token_mlp(torch.cat([fused, t4_static.expand(batch, width, n_units, t4_static.shape[-1])], dim=-1))
        )
...
def export_ort_graphs(model, out_dir: Path, batches=(1, 2, 3, 4), n_units: int = 176, window: int = 200) -> dict:
    ...
    for B in batches:
        adv = _OrtAdvanceCore(model, window).eval()
        reb = _OrtRebuildCore(model, window).eval()
        local_sel = torch.randn(B, 5, n_units, conv_channels)
        z_prev = torch.randn(B, window, set_dim)
        proj_static = torch.randn(B, 1, n_units, 1, conv_channels)
        t4_static = torch.randn(B, 1, n_units, cfg.t4_dim)
        win = torch.randn(B, window, n_units)
        ...
        torch.onnx.export(
            reb, (win, proj_static, t4_static), str(p_reb),
...
class OrtExactEEngine(fast.FastExactEEngine):
    ...
    def __init__(self, model, banks, window: int, graph_dir: Path, intra_op: int = 2, inter_op: int = 1) -> None:
        super().__init__(model, banks, window)
        import onnxruntime as ort

        if not self.keep_all_true:
            raise ValueError("ORT path requires all-true bank unit masks (public H1 banks satisfy this)")
        self._ort = ort
...
class OrtH1ProjAddFalconDecoder(fast.FastH1ProjAddFalconDecoder):
    ...
    def predict(self, neural_observations: np.ndarray) -> np.ndarray:
        ...
        if self._engine is None:
            self._engine = OrtExactEEngine(
                self.decoder, self.local_banks, self.window_size, self._graph_dir,
        ...
        if self._n_predicts % 2000 == 0:
            print(f"h1-ort-e predict_steps={self._n_predicts} n_active={n_active}", flush=True)
