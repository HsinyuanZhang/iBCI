"""Falcon runtime for M2 RIFT R50 proj_add P16 activity_only_empty_side + learned_slope e10.

Architecture is RIFT, identity is proj_add P16 with EMPTY-head E0 and literal-zero T
(activity_only_empty_side), recency is per-layer learned_slope on the unscaled
(default) half-life ladder. Backend is cached PyTorch FP32 via
CpuLearnableRecencyRuntime. Not BT-EORT, not ORT, not SPINT, not 582217 frozen ACT,
not leaked activity_only e9.
"""
...
PAYLOAD_SCHEMA = "m2_rift_r50_projadd_activity_only_empty_side_e10_cached_v1"
CHANNELS = 96
BEHAVIOR_SCALE = 5.0
...
def recency_config_from_payload(payload: dict) -> LearnableRecencyConfig:
    ...
    return config_from_run_meta({"learnable_config": raw}, "m2")
...
def build_decoder(recency_cfg: LearnableRecencyConfig) -> RiftDecoder:
    """Rebuild the trained proj_add + per-layer learned_slope stack without wrap.py."""
    ...
    model = RiftDecoder("m2", context_bins=CONTEXT_BINS, bias_mode="recency", seed=SEED, proj_dim=PROJ_DIM)
    rebuilt = _adopt_temporal(model.temporal, recency_cfg.temporal_config)
    model.temporal = LearnableRecencyTemporal.from_initialized(rebuilt, recency_cfg)
    model.temporal_config = recency_cfg.temporal_config
    model.learnable_cfg = recency_cfg
    model.bias_mode = f"learnable_{recency_cfg.tier}"
    return model


class CpuLearnableRiftRuntime:
    """CpuRiftRuntime frontend with CpuLearnableRecencyRuntime (per-layer slopes)."""
...
def _task_bank(tag: str, row: dict) -> TaskBank:
    ...
    if np.any(carrier != 0.0):
        raise ValueError(f"{tag}: activity_only_empty_side bank carrier T must be zero")
    return TaskBank(
        ...
        calibration_meta={
            ...
            "trial_count": 33,
            "estimator": "official M2 13-tag EMPTY-head E0 + literal-zero T (activity_only_empty_side)",
...
class M2RiftCachedFalconDecoder(BCIDecoder):
    def __init__(self, task_config, model_path: str, batch_size: int = 1):
        ...
        if payload.get("identity") != "activity_only_empty_side":
            raise ValueError("payload identity must be activity_only_empty_side")
        if payload.get("ladder") != "default" or payload.get("tier") != "learned_slope":
        ...
        for tag, row in self.bank_by_dataset_tag.items():
            t = np.asarray(row["T"])
            if np.any(t != 0.0):
                raise ValueError(f"{tag}: bank carrier T must be all zeros in activity_only_empty_side")
        ...
        self._n_predicts = 0
        self.kind = "rift_cached_projadd_activity_only_empty_side"
