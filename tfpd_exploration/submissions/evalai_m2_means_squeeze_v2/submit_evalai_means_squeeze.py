"""Guarded EvalAI submission helper for native-M33 mean-rate FiLM cached identity.

Default mode is a read-only authenticated preflight. Formal private
submission needs ``--execute`` plus the complete immutable image ID and
payload SHA-256. A durable state file prevents an accidental repeat.
"""
...
METHOD_LABEL = (
    "Native first-33 T4 plus hold-vs-reach mean-rate FiLM cached identity "
    "(activity pool = first 33; FiLM offline on held-in only; online cached "
    "E[N,50]; no TTA)"
)

CANDIDATE: dict[str, Any] = {
    "arm": "means_ep12_lr3e4_s42",
    "image_tag": "spint-t4-m2:means-m33-ep12-lr3e4-0cc4ab76",
    "image_id": "sha256:a8a5305863dd70324d9c2ff14f3efc5f08f833a4d0eb8f6533cc29fabbe0ea53",
    "payload_sha256": "0cc4ab769c8752191c87b6d8947b32768e15c51a49a097c3b6335b416fa81ca5",
    "method_label": METHOD_LABEL,
    "method_name": "M2 native M33 T4 + hold/reach mean FiLM cached identity",
    "method_description": (
        "Frozen SPINT M2 decoder with per-session cached identity calibrated "
        "offline from the chronological first-33 public calibration trials: "
        "native T4 from finite-direction trials in that block, plus a hold-vs-reach "
        "mean-rate contrast (delta and log-ratio, robust-z) that modulates pooled "
        "B3S features through a 1224-parameter FiLM trained only on held-in days. "
        "The EvalAI runtime serves cached E[N,50] identities only and performs no "
        "hidden-query label read, optimizer step, parameter update, or backpropagation."
    ),
    "budget_disclosure": (
        "33-trial chronological calibration block: native T4 consumes the "
        "finite-direction subset; activity pool = the first 33 calibration trials; "
        "hold/reach contrast from the same block; FiLM applied offline; zero online updates"
    ),
}
...
def state_path() -> Path:
    return Path(__file__).resolve().parent / "artifacts" / "evalai_push_state_means_squeeze_v2.json"
...
def _validate_local_image(docker_module: Any) -> tuple[Any, Any]:
    ...
    expected = {
        ...
        "ai.eval.label_budget": "33",
        "ai.eval.activity_budget": "33",
...
def preflight() -> tuple[dict[str, object], Any, Any, dict[str, object], dict[str, object], tuple[Any, Any, Any, str, str]]:
    runtime = _runtime()
...
def _push_image(client: Any, image: Any, runtime: tuple[Any, Any, Any, str, str]) -> dict[str, object]:
    ...
    state = {
        "schema_version": "m2_means_squeeze_v2_evalai_push_state",
        "arm": CANDIDATE["arm"],
        "image_tag": CANDIDATE["image_tag"],
        "image_id": CANDIDATE["image_id"],
        "payload_sha256": CANDIDATE["payload_sha256"],
...
def _register(state: dict[str, object], runtime: tuple[Any, Any, Any, str, str]) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="m2_means_squeeze_v2_evalai_") as tmp:
