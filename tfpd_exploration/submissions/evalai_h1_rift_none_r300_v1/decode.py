"""EvalAI entry for H1 RIFT R300 ACTIVITY_ONLY/NONE ablation cached CPU."""
import argparse
import os
import sys
...
from h1_rift_falcon_decoder import H1RiftCachedFalconDecoder
...
def main() -> None:
    ...
    decoder = H1RiftCachedFalconDecoder(
        task_config=config,
    ...
    pred_path = os.environ.get("PREDICTION_PATH") or os.environ.get("PREDICTION_PATH_LOCAL")
    if pred_path:
        with open(pred_path, "rb+") as handle:
            handle.flush()
            os.fsync(handle.fileno())
        print(f"prediction pickle fsync {pred_path} bytes={os.path.getsize(pred_path)}", flush=True)
