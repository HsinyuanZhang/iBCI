"""V8 receipt and lifecycle composition for the PhysicalHelper successor.

V8 deliberately delegates all score science, cell codecs, gates, and the
shared immutable lifecycle to V5/V7/V1.  This module owns only fresh receipt
schemas and descriptor-held proof that the exact V7 authority plus its honest
post-attempt PhysicalHelper failure preceded any V8 authority or score root.
"""
...
class V8ScoreError(v1score.ScoreError):
    """Fail closed for V8 lineage, durable authority, or receipt drift."""


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise V8ScoreError(message)
...
def _sha(value: object, label: str) -> str:
    try:
        ...
        raise V8ScoreError(str(error)) from error


def _identity_payload(identity: plan.ScoreIdentity) -> dict[str, object]:
    if not isinstance(identity, plan.ScoreIdentity):
        raise V8ScoreError("V8 score requires its exact typed identity")
    return identity.payload()


def _v7_failure_payload() -> dict[str, object]:
    return plan.V7_PHYSICAL_HELPER_FAILURE.payload()
...
def _v7_identity_from_preflight(preflight: Mapping[str, object]) -> v7plan.ScoreIdentity:
    ...
    if not isinstance(closure, Mapping):
        raise V8ScoreError("V8 V7 predecessor identity closure is absent")
    try:
        identity = v7plan.ScoreIdentity(closure=closure)
    except (TypeError, v7plan.V7PlanError) as error:
        raise V8ScoreError("V8 V7 predecessor identity reconstruction failed") from error
    if identity.payload() != identity_payload:
        raise V8ScoreError("V8 V7 predecessor identity canonical drift")
    return identity


def _read_v7_physical_helper_failure(
    root: Path,
    *,
    contract: plan.V7PhysicalHelperFailureContract,
    require_literal_contract: bool,
) -> dict[str, object]:
    """Hold and validate V7 authority plus its exact six-leaf failure graph.

    The generic contract form is test-only.  All public V8 issuer/reservation
    entry points below pass the one literal :data:`V7_PHYSICAL_HELPER_FAILURE`
    object, so neither an environment variable nor a caller mapping can point
    a V8 score at an alternative predecessor.
    """
    if require_literal_contract and contract is not plan.V7_PHYSICAL_HELPER_FAILURE:
        raise V8ScoreError("V8 V7 PhysicalHelper predecessor contract identity drift")
    ...
    try:
        ...
        if not stat.S_ISDIR(held.st_mode) or (int(held.st_dev), int(held.st_ino)) != authority_identity:
            raise V8ScoreError("V8 V7 authority directory identity drift before read")
        expected = set(AUTHORITY_TOPOLOGY) | {f"{name}.sha256" for name in AUTHORITY_TOPOLOGY}
        if set(os.listdir(authority_fd)) != expected:
            raise V8ScoreError("V8 V7 authority pair topology drift")
        ...
        if pre_sha != contract.preflight_sha256 or auth_sha != contract.authorization_sha256:
            raise V8ScoreError("V8 V7 authority body SHA drift")
        identity = _v7_identity_from_preflight(preflight)
        try:
            checked_preflight = v7score.validate_target_free_preflight(preflight, identity)
            checked_auth = v7score.validate_root_authorization(
                authorization, official_preflight_sha256=pre_sha, preflight=checked_preflight, identity=identity,
            )
        except v7score.V7ScoreError as error:
            raise V8ScoreError(str(error)) from error
        ...
        if (
            ...
            raise V8ScoreError("V8 V7 authority semantic lineage drift")
        if v1score._directory_identity(authority_directory) != authority_identity:
            raise V8ScoreError("V8 V7 authority directory identity drift during read")
    except V8ScoreError:
        raise
    except OSError as error:
        raise V8ScoreError("V8 V7 authority descriptor read failed") from error
    finally:
        if authority_fd >= 0:
    ...
    try:
        ...
        raise V8ScoreError("V8 V7 failed score root cannot be inspected") from error
    ...
    if (
        ...
        raise V8ScoreError("V8 V7 failed score-root identity/mode drift")
    failed_fd = -1
    try:
        ...
        if not stat.S_ISDIR(held.st_mode) or (int(held.st_dev), int(held.st_ino)) != expected_identity:
            raise V8ScoreError("V8 V7 failed score-root identity drift before read")
        expected_names = {
            "attempt.json", "input_authority.json", "failure.json",
            "attempt.json.sha256", "input_authority.json.sha256", "failure.json.sha256",
        }
        if set(os.listdir(failed_fd)) != expected_names:
            raise V8ScoreError("V8 V7 failed score-root exact six-leaf topology drift")
        _attempt_body, attempt, attempt_sha = _read_exact_pair(failed_fd, "attempt.json")
        _input_body, input_payload, input_sha = _read_exact_pair(failed_fd, "input_authority.json")
        _failure_body, failure, failure_sha = _read_exact_pair(failed_fd, "failure.json")
        if (
            attempt_sha != contract.attempt_sha256
            or input_sha != contract.input_authority_sha256
            or failure_sha != contract.failure_sha256
        ):
            raise V8ScoreError("V8 V7 failed score body SHA drift")
        expected_attempt = v7score._attempt_payload(identity, contract.preflight_sha256, contract.authorization_sha256)
        if attempt != expected_attempt:
            raise V8ScoreError("V8 V7 failed score attempt semantic drift")
        try:
            fixed = v1score._fixed_authority_from_payload(checked_preflight["evaluation_authority"])
            checked_input = v1score.validate_input_authority_payload(
                input_payload, identity=identity, evaluation_authority=fixed,
            )
        except v1score.ScoreError as error:
            raise V8ScoreError("V8 V7 failed score input-authority semantic drift") from error
        if checked_input != input_payload:
            raise V8ScoreError("V8 V7 failed score input-authority canonical drift")
        try:
            checked_failure = v7score.validate_failure_payload(failure, identity, attempt_sha, input_sha)
        except v7score.V7ScoreError as error:
            raise V8ScoreError(str(error)) from error
        ...
        if (
            ...
            or checked_failure.get("within_assets_opened") is not facts["within_external_opened"]
            or checked_failure.get("external_assets_opened") is not facts["within_external_opened"]
            ...
            or checked_failure.get("input_authority_sha256") != input_sha
            or checked_failure.get("terminal_published") is not facts["terminal_published"]
            or checked_failure.get("target_paths_resolved_or_opened") is not True
            or any(checked_failure.get(key) != 0 for key in (
                "target_optimizer_steps", "target_backward_calls", "target_update_calls",
            ))
        ):
            raise V8ScoreError("V8 V7 failed score access/boundary facts drift")
        after = os.lstat(failed_directory)
        if (int(after.st_dev), int(after.st_ino)) != expected_identity:
            raise V8ScoreError("V8 V7 failed score-root identity drift during read")
    except V8ScoreError:
        raise
    except OSError as error:
        raise V8ScoreError("V8 V7 failed score descriptor read failed") from error
    finally:
        if failed_fd >= 0:
            os.close(failed_fd)
    return payload
...
def validate_v7_physical_helper_predecessor(root: Path) -> dict[str, object]:
    """Public production validator for the literal V7 failed execution graph."""
    observed = _read_v7_physical_helper_failure(
        Path(root), contract=plan.V7_PHYSICAL_HELPER_FAILURE, require_literal_contract=True,
    )
    if observed != _v7_failure_payload():
        raise V8ScoreError("V8 V7 PhysicalHelper predecessor canonical binding drift")
    # V7 itself was a successor.  Retain its two immutable transitive proofs;
    # neither one is caller-configurable and both are descriptor-held by V7.
    try:
        v7score.validate_v6_runtime_flags_predecessor(Path(root))
        v5score.validate_completed_source_gate(Path(root))
    except (v7score.V7ScoreError, v5score.V5ScoreError) as error:
        raise V8ScoreError(str(error)) from error
    return observed
...
def build_target_free_preflight(
    *, root: Path, identity: plan.ScoreIdentity, source_gate: v1score.SourceGateBinding,
    ...
    """Adapt the V7 target-free payload without changing science authority."""
    _identity_payload(identity)
    try:
        base = v7score.build_target_free_preflight(
            root=Path(root), identity=identity, source_gate=source_gate, fixed_authority=fixed_authority,
        )
    except v7score.V7ScoreError as error:
        raise V8ScoreError(str(error)) from error
    result = dict(base)
    result["schema"] = "causal_dual_memory_cell_d_score_target_free_preflight_v8"
    result["authority_root_relative"] = plan.AUTHORITY_ROOT_RELATIVE
    result["score_root_relative"] = plan.SCORE_ROOT_RELATIVE
    result["v7_physical_helper_failure"] = _v7_failure_payload()
    return result


def _v7_preflight_body(value: Mapping[str, object]) -> dict[str, object]:
    body = dict(value)
    predecessor = body.pop("v7_physical_helper_failure", None)
    if predecessor != _v7_failure_payload():
        raise V8ScoreError("V8 preflight V7 PhysicalHelper predecessor binding drift")
    body["schema"] = "causal_dual_memory_cell_d_score_target_free_preflight_v7"
    body["authority_root_relative"] = v7plan.AUTHORITY_ROOT_RELATIVE
    body["score_root_relative"] = v7plan.SCORE_ROOT_RELATIVE
    return body
...
def validate_target_free_preflight(value: Mapping[str, object], identity: plan.ScoreIdentity) -> dict[str, object]:
    required = {
        ...
        "v6_runtime_flags_failure", "v7_physical_helper_failure",
    }
    if (
        not isinstance(value, Mapping) or set(value) != required
        or value.get("schema") != "causal_dual_memory_cell_d_score_target_free_preflight_v8"
        ...
        raise V8ScoreError("V8 target-free preflight schema/root drift")
    try:
        checked_v7 = v7score.validate_target_free_preflight(_v7_preflight_body(value), identity)
    except v7score.V7ScoreError as error:
        raise V8ScoreError(str(error)) from error
    expected = dict(checked_v7)
    expected["schema"] = "causal_dual_memory_cell_d_score_target_free_preflight_v8"
    expected["authority_root_relative"] = plan.AUTHORITY_ROOT_RELATIVE
    expected["score_root_relative"] = plan.SCORE_ROOT_RELATIVE
    expected["v7_physical_helper_failure"] = _v7_failure_payload()
    if dict(value) != expected:
        raise V8ScoreError("V8 target-free preflight canonical predecessor drift")
    return expected
...
def validate_preflight_against_fixed_authorities(
    ...
    if checked["evaluation_authority"] != observed:
        raise V8ScoreError("V8 durable preflight fixed evaluation authority drift")
    return checked


def build_root_authorization(*, official_preflight_sha256: str, preflight: Mapping[str, object]) -> dict[str, object]:
    digest = _sha(official_preflight_sha256, "V8 official preflight SHA")
    if not isinstance(preflight.get("source_gate"), Mapping):
        raise V8ScoreError("V8 root authorization source-gate binding absent")
    return {
        "schema": "causal_dual_memory_cell_d_score_root_authorization_v8",
        ...
        "v7_physical_helper_failure": _v7_failure_payload(),
    }


def _v7_authorization_body(value: Mapping[str, object]) -> dict[str, object]:
    body = dict(value)
    predecessor = body.pop("v7_physical_helper_failure", None)
    if predecessor != _v7_failure_payload():
        raise V8ScoreError("V8 authorization V7 PhysicalHelper predecessor binding drift")
    body["schema"] = "causal_dual_memory_cell_d_score_root_authorization_v7"
    body["authority_root_relative"] = v7plan.AUTHORITY_ROOT_RELATIVE
    body["score_root_relative"] = v7plan.SCORE_ROOT_RELATIVE
    return body
...
def validate_root_authorization(
    ...
    required = {
        ...
        "v7_physical_helper_failure",
    }
    if not isinstance(value, Mapping) or set(value) != required:
        raise V8ScoreError("V8 root authorization schema drift")
    ...
    if dict(value) != expected or _digest(_identity_payload(identity)) != expected["identity_sha256"]:
        raise V8ScoreError("V8 root authorization canonical identity/predecessor drift")
    return expected
...
def reserve_authority_artifact(root: Path, capability: object, *, identity: plan.ScoreIdentity) -> v1score.ArtifactRoot:
    ...
    if plan.implementation_closure(base).payload() != _identity_payload(identity)["closure"]:
        raise V8ScoreError("V8 authority reservation implementation closure drift")
    validate_v7_physical_helper_predecessor(base)
    ...
    if observed.payload()["body_sha256s"].get("terminal.json") != _identity_payload(identity)["source_gate"]["terminal_sha256"]:
        raise V8ScoreError("V8 authority reservation completed source-gate terminal drift")
...
def publish_target_free_preflight(
    ...
    if checked["source_gate"] != v5score.validate_completed_source_gate(Path(root)).payload():
        raise V8ScoreError("V8 durable preflight completed source-gate binding drift")
    if checked["v7_physical_helper_failure"] != validate_v7_physical_helper_predecessor(Path(root)):
        raise V8ScoreError("V8 durable preflight V7 PhysicalHelper predecessor drift")
...
def publish_root_authorization(
    ...
    try:
        ...
        raise V8ScoreError("V8 durable official preflight malformed") from error
    if not isinstance(preflight, Mapping):
        raise V8ScoreError("V8 durable official preflight root drift")
    checked = validate_preflight_against_fixed_authorities(Path(root), preflight, identity=identity)
    if checked["source_gate"] != v5score.validate_completed_source_gate(Path(root)).payload():
        raise V8ScoreError("V8 root authorization completed source-gate binding drift")
    if checked["v7_physical_helper_failure"] != validate_v7_physical_helper_predecessor(Path(root)):
        raise V8ScoreError("V8 root authorization V7 PhysicalHelper predecessor drift")
...
def _read_durable_authority_pair(
    ...
    try:
        ...
        if not stat.S_ISDIR(held.st_mode) or (int(held.st_dev), int(held.st_ino)) != named:
            raise V8ScoreError("V8 durable authority root identity drift before read")
        expected = set(AUTHORITY_TOPOLOGY) | {f"{name}.sha256" for name in AUTHORITY_TOPOLOGY}
        if set(os.listdir(fd)) != expected:
            raise V8ScoreError("V8 durable authority pair topology drift")
        ...
        if v1score._directory_identity(directory) != named:
            raise V8ScoreError("V8 durable authority root identity drift during read")
        ...
        raise V8ScoreError("V8 durable authority descriptor read failed") from error
    finally:
        if fd >= 0:
            os.close(fd)
...
def validate_selected_launch_environment(
    ...
    try:
        return v7score.validate_selected_launch_environment(identity, environ)
    except v7score.V7ScoreError as error:
        raise V8ScoreError(str(error)) from error
...
def issue_durable_execution_capability(
    ...
    base = Path(root)
    if plan.implementation_closure(base).payload() != _identity_payload(identity)["closure"]:
        raise V8ScoreError("V8 execution issuer implementation closure drift")
    validate_v7_physical_helper_predecessor(base)
    ...
    if v5score.validate_completed_source_gate(base).payload() != preflight["source_gate"]:
        raise V8ScoreError("V8 execution issuer completed source-gate binding drift")
...
def reserve_score_artifact(
    ...
    base = Path(root)
    if plan.implementation_closure(base).payload() != _identity_payload(identity)["closure"]:
        raise V8ScoreError("V8 score-root reservation implementation closure drift")
    validate_v7_physical_helper_predecessor(base)
    preflight, _authorization, pre_sha, auth_sha = load_durable_authority(base, identity=identity)
    ...
    if approved.official_preflight_sha256 != pre_sha or approved.root_authorization_sha256 != auth_sha:
        raise V8ScoreError("V8 score-root reservation durable capability SHA drift")
    validate_preflight_against_fixed_authorities(base, preflight, identity=identity)
    if v5score.validate_completed_source_gate(base).payload() != preflight["source_gate"]:
        raise V8ScoreError("V8 score-root reservation completed source-gate drift")
    plan.assert_fresh_prospective_root(base, plan.SCORE_ROOT_RELATIVE)
...
def _validate_reserved_score_artifact(root: Path, artifact: v1score.ArtifactRoot, identity: Any) -> None:
    if not isinstance(identity, plan.ScoreIdentity):
        raise V8ScoreError("V8 reserved artifact identity type drift")
    try:
...
def _v7_score_body(value: Mapping[str, object]) -> dict[str, object]:
    body = dict(value)
    predecessor = body.pop("v7_physical_helper_failure", None)
    if predecessor != _v7_failure_payload():
        raise V8ScoreError("V8 score V7 PhysicalHelper predecessor binding drift")
    body["schema"] = "causal_dual_memory_cell_d_matched_score_v7"
    return body
...
def _score_payload(
    ...
    try:
        base = v7score._score_payload(identity, input_sha, summaries, gates)
    except v7score.V7ScoreError as error:
        raise V8ScoreError(str(error)) from error
    result = dict(base)
    result["schema"] = "causal_dual_memory_cell_d_matched_score_v8"
    result["v7_physical_helper_failure"] = _v7_failure_payload()
    return result
...
def validate_score_payload(
    ...
    if not isinstance(value, Mapping) or set(value) != required or value.get("schema") != "causal_dual_memory_cell_d_matched_score_v8":
        raise V8ScoreError("V8 score receipt schema drift")
    try:
        checked_v7 = v7score.validate_score_payload(
            _v7_score_body(value), identity, input_payload, input_authority_sha256, evaluation_authority,
        )
    except (v7score.V7ScoreError, v1score.ScoreError) as error:
        raise V8ScoreError(str(error)) from error
    expected = dict(checked_v7)
    expected["schema"] = "causal_dual_memory_cell_d_matched_score_v8"
    expected["v7_physical_helper_failure"] = _v7_failure_payload()
    if dict(value) != expected:
        raise V8ScoreError("V8 score receipt canonical predecessor drift")
    return expected


def _v7_terminal_body(value: Mapping[str, object]) -> dict[str, object]:
    body = dict(value)
    predecessor = body.pop("v7_physical_helper_failure", None)
    if predecessor != _v7_failure_payload():
        raise V8ScoreError("V8 terminal V7 PhysicalHelper predecessor binding drift")
    body["schema"] = "causal_dual_memory_cell_d_score_terminal_v7"
    return body
...
def _terminal_payload(
    ...
    try:
        base = v7score._terminal_payload(identity, attempt_sha, input_sha, score_sha, gates, verdict)
    except v7score.V7ScoreError as error:
        raise V8ScoreError(str(error)) from error
    result = dict(base)
    result["schema"] = "causal_dual_memory_cell_d_score_terminal_v8"
    result["v7_physical_helper_failure"] = _v7_failure_payload()
    return result
...
def validate_terminal_payload(
    ...
    if not isinstance(value, Mapping) or set(value) != required or value.get("schema") != "causal_dual_memory_cell_d_score_terminal_v8":
        raise V8ScoreError("V8 terminal schema drift")
    try:
        checked_v7 = v7score.validate_terminal_payload(
            _v7_terminal_body(value), identity, attempt_sha, input_sha, _v7_score_body(score_payload), score_sha,
        )
    except v7score.V7ScoreError as error:
        raise V8ScoreError(str(error)) from error
    expected = dict(checked_v7)
    expected["schema"] = "causal_dual_memory_cell_d_score_terminal_v8"
    expected["v7_physical_helper_failure"] = _v7_failure_payload()
    if dict(value) != expected:
        raise V8ScoreError("V8 terminal canonical predecessor drift")
    return expected


def _attempt_payload(identity: plan.ScoreIdentity, pre_sha: str, auth_sha: str) -> dict[str, object]:
    try:
        base = v7score._attempt_payload(identity, pre_sha, auth_sha)
    except v7score.V7ScoreError as error:
        raise V8ScoreError(str(error)) from error
    result = dict(base)
    result["schema"] = "causal_dual_memory_cell_d_score_attempt_v8"
    result["v7_physical_helper_failure"] = _v7_failure_payload()
    return result


def _v7_failure_body(value: Mapping[str, object]) -> dict[str, object]:
    body = dict(value)
    predecessor = body.pop("v7_physical_helper_failure", None)
    if predecessor != _v7_failure_payload():
        raise V8ScoreError("V8 failure V7 PhysicalHelper predecessor binding drift")
    body["schema"] = "causal_dual_memory_cell_d_score_failure_v7"
    return body
...
def _failure_payload(
    ...
    try:
        base = v7score._failure_payload(identity, attempt_sha, input_sha, stage, error, runtime_progress)
    except (v7score.V7ScoreError, v1score.ScoreError) as error_value:
        raise V8ScoreError(str(error_value)) from error_value
    result = dict(base)
    result["schema"] = "causal_dual_memory_cell_d_score_failure_v8"
    result["v7_physical_helper_failure"] = _v7_failure_payload()
    return result
...
def validate_failure_payload(
    ...
    if not isinstance(value, Mapping) or set(value) != required or value.get("schema") != "causal_dual_memory_cell_d_score_failure_v8":
        raise V8ScoreError("V8 failure schema drift")
    try:
        checked_v7 = v7score.validate_failure_payload(_v7_failure_body(value), identity, attempt_sha, input_sha)
    except v7score.V7ScoreError as error:
        raise V8ScoreError(str(error)) from error
    expected = dict(checked_v7)
    expected["schema"] = "causal_dual_memory_cell_d_score_failure_v8"
    expected["v7_physical_helper_failure"] = _v7_failure_payload()
    if dict(value) != expected:
        raise V8ScoreError("V8 failure canonical predecessor drift")
    return expected


def _validate_preflight_hook(value: Mapping[str, object], identity: Any) -> Mapping[str, object]:
    if not isinstance(identity, plan.ScoreIdentity):
        raise V8ScoreError("V8 lifecycle identity type drift")
...
def _input_payload_hook(authority: Any, identity: Any) -> Mapping[str, object]:
    if not isinstance(identity, plan.ScoreIdentity) or not isinstance(authority, v1score.InputAuthority):
        raise V8ScoreError("V8 physical route must return inherited typed input authority")
...
def _validate_input_hook(value: Mapping[str, object], identity: Any, fixed: Any) -> Mapping[str, object]:
    if not isinstance(identity, plan.ScoreIdentity) or not isinstance(fixed, v1score.FixedEvaluationAuthority):
        raise V8ScoreError("V8 fixed evaluation authority type drift")
    try:
...
def _continue_after_budget(_budget: int, _gates: Mapping[int, Mapping[str, object]]) -> bool:
    """The V8 matrix is always complete unless execution/integrity fails."""
    return True


V8_LIFECYCLE_HOOKS = v1score.ProfiledLifecycleHooks(
...
def run_authorized_score_lifecycle(
    ...
    """Use the single shared V1 lifecycle with V8 typed hook codecs."""
    return v1score.run_profiled_score_lifecycle(
        ...
        preflight=preflight, authorization=authorization, hooks=V8_LIFECYCLE_HOOKS,
    )
