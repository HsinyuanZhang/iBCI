"""The M2 LOCAL support anchor: the audit's bitwise law on the SELECTED supports.
...
this anchor provides exactly the interface those modules consume (the same
duck-typed contract the SUA transfer introduced):
...
The M2-specific law is exactly the sealed prerequisite audit's prerequisite-(b)
PASS artifact: the support design uses the sealed carrier's CONTINUOUS
calibration angles with the ``fit_ridge_t4`` column order ``[cos, sin, 1]``
and penalty ``diag(n*lambda, n*lambda, 0)``, ``lambda = 0.1`` (algebraically
the Stage-O law, arithmetically the sealed fit), and the modulation column is
rebuilt with ``sqrt(a*a + c*c)`` (the sealed arithmetic, NOT ``hypot``), so
the zero-evidence fallback reproduces the sealed carrier bit-for-bit.
"""
...
class M2AnchorError(ValueError):
    """Fail-closed error for a malformed or drifting M2 anchor."""


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise M2AnchorError(message)
...
def support_penalty(support_rows: int, *, normalized_lambda: float) -> np.ndarray:
    """The exact sealed fixed-ridge penalty of the M2 trial-table fit."""
...
def format_t4_rows(coefficients: np.ndarray) -> np.ndarray:
    ...
    arithmetic, NOT ``hypot``: the zero-evidence fallback must reproduce the
    sealed ``fit_ridge_t4`` carrier bit-for-bit (the audit's law).
    """
...
class M2SupportAnchor:
    ...
    def __init__(
        ...
        *,
        ...
        per_group: Sequence[M2GroupAnchor],
    ...
    def from_labeled_support(
        ...
        *,
        groups: cdm_core.ComplementaryGroups,
        support_trial_rates: np.ndarray,
        support_angles_rad: Sequence[float],
        support_t4: np.ndarray,
        normalized_lambda: float = cdm_core.RIDGE_NORMALIZED_LAMBDA,
    ) -> "M2SupportAnchor":
        """Prove the zero-evidence fallback bitwise, then freeze the bank.

        ``b0`` is sliced from ONE full-unit design product, so the per-column
        LU solves are bitwise identical to the sealed whole-matrix solve (the
        audit's parity proof on the SELECTED support).
        """
        ...
        for group in range(groups.group_count):
            ...
            per_group.append(M2GroupAnchor(
        ...
        return cls(
            ...
            parity={
                "law": (
                    "per-group solve(A0, b0) -> [a, c, sqrt(a*a+c*c), b] float32 vs "
                    "the sealed fit_ridge_t4 carrier (the audit's prerequisite-(b) law "
    ...
    def payload(self) -> dict[str, Any]:
        return {
            "schema": "cdm_p1_m2_local_v1_support_anchor_v1",
