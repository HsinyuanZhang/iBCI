"""Target-GPU idle / UUID bind for the Tier 1/2 pack cell.

Copied from m1_emg_rsyn3_fold_local_v1.gpu with this cell's OWN_TOKEN and
PACK_LIMIT. Does not couple to the rSyn3 token.
"""
...
def expected_uuid(gpu_index: int) -> str:
    _require(gpu_index == plan.GPU1_INDEX, "this cell may only use GPU 1")
...
def assert_target_gpu_launchable(
    ...
    *,
    ...
    """Refuse foreign jobs. Packing allows up to pack_limit same-route arms on GPU 1."""
    _require(type(gpu_index) is int, "gpu_index")
    _require(gpu_index == plan.GPU1_INDEX, "this cell may only use GPU 1")
    _require(plan.GPU0_ALLOWED is False, "GPU 0 must stay refused")
    _require(type(pack) is bool, "pack flag")
    _require(type(pack_limit) is int and pack_limit == plan.PACK_LIMIT, "pack_limit")
...
def require_launch(
    ...
    *,
    ...
    _require(bool(gpu_authorized) is True, "GPU execute requires --gpu-authorized")
