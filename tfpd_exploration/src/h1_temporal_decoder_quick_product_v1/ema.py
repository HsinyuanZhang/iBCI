"""Decoder-only EMA. Does not alias RAW storage."""
...
class DecoderEMA:
            ...
            if self.n_updates == 0:
                ...
                for name, param in named.items():
                    self.shadow[name].mul_(self.decay).add_(
                        param.detach().to(dtype=torch.float32), alpha=one_minus
                    )
        self.n_updates += 1
    ...
    def score_with_ema(self, module: nn.Module, fn: Callable[[nn.Module], T]) -> T:
        named = {n: p for n, p in module.named_parameters() if p.requires_grad}
        raw = {name: param.detach().clone() for name, param in named.items()}
        ...
        try:
            ...
            with torch.no_grad():
                for name, param in named.items():
                    param.copy_(raw[name].to(device=param.device, dtype=param.dtype))
