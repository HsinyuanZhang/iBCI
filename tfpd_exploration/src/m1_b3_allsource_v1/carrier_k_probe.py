"""Frozen b3s_rsyn3 carrier-k probe: k-fit ridge, M10 encoder neural.

CPU only. Ranking R² is in-train source remaining-query after trial 10.
Later-day 10-trial files store identity/carrier hashes; R² is null.
...
class CarrierKProbeError(RuntimeError):
    """Fail closed for the local carrier-k probe."""


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise CarrierKProbeError(message)


SCHEMA = "m1_b3_allsource_carrier_k_probe_v1"
ARM = plan.CARRIER_K_ARM
POOL = int(plan.CARRIER_K_POOL_TRIALS)
QUERY_START = int(plan.CARRIER_K_QUERY_START)
EVAL_BATCH_SIZE = 128
BASELINE_NAME = "chronological_k10"


def _score_identity_batches(student, identity, batches) -> tuple[np.ndarray, np.ndarray]:
    ...
    _require(preds, "source session has no remaining-query windows")
    return np.concatenate(preds, axis=0), np.concatenate(targets, axis=0)


def score_probe(repo_root: Path) -> dict[str, Any]:
    ...
    sessions = OrderedDict((name, mapping[name]) for name in mapping)
    _require(tuple(sessions) == tuple(plan.SOURCE_SESSION_NAMES) + tuple(
        f"ses-{item}" for item in plan.LATER_CALIB_SESSIONS
    ), "public calib roster drift")
    ...
    with torch.inference_mode():
        ...
        for index, (name, path) in enumerate(sessions.items()):
            ...
            _require(int(dataset.calib_n_trials[name]) == POOL, "identity neural budget drift")
            if role == "source_in_train":
                _require(n_trials > QUERY_START, f"{name} has no post-M10 query trials")
                _require(query_trials > 0, f"{name} source query_trials is 0")
                _require(len(dataset) == int(audit["eligible_windows"]) > 0, "source window/audit mismatch")
            else:
                _require(n_trials == POOL, f"{name} later-day file is not 10 trials")
                _require(query_trials == 0, f"{name} later-day unexpectedly has query trials")
                _require(len(dataset) == 0, f"{name} later-day must have empty query")

            calibration_np = np.asarray(
                dataset.calib_trialized_neural_features[name][:POOL],
                dtype=np.float32,
            )
            _require(
                calibration_np.shape == (POOL, plan.TRIAL_BINS, plan.CHANNELS),
                f"unexpected M10 calibration shape for {name}: {calibration_np.shape}",
            )
            calibration = torch.from_numpy(calibration_np).unsqueeze(0)
            angles = calibration_target_angles(path, "m1")
            _require(int(angles.shape[0]) >= POOL, f"{name} has fewer than {POOL} tgt_loc rows")
            support = bank_module.load_public_calib_support(path, support_trials=POOL)
            synergy = carrier_k.trial_mean_synergy(support, basis, pool_trials=POOL)
            ...
            for variant in grid:
                ...
                side_np = np.ascontiguousarray(
                    bank_module.encode_record_selected(
                        support, rsyn3_fitted, selected, pool_trials=POOL,
                    ),
                    dtype=np.float32,
                )
                ...
                if role == "source_in_train":
                    pred_all, target_all = _score_identity_batches(student, identity, batches)
                    ...
                    row["r2_variance_weighted_last_bin"] = score
                    per_variant[variant["name"]]["source_preds"].append(pred_all)
                    per_variant[variant["name"]]["source_targets"].append(target_all)
                    print(
                        f"{name} {variant['name']} r2={score:.6f} selected={row['selected_trial_ids']}",
                        flush=True,
                    )
                else:
                    row["r2_variance_weighted_last_bin"] = None
                    print(
                        f"{name} {variant['name']} r2=null selected={row['selected_trial_ids']}",
                        flush=True,
                    )
    ...
    for variant in grid:
        ...
        source_rows = [row for row in payload["sessions"] if row["role"] == "source_in_train"]
        later_rows = [row for row in payload["sessions"] if row["role"] == "later_day_public_calib"]
        _require(len(source_rows) == 4, "need four in-train source sessions")
        _require(len(later_rows) == 3, "need three later-day sessions")
        _require(all(row["r2_variance_weighted_last_bin"] is None for row in later_rows), "later-day R2 leak")
        pred_all = np.concatenate(payload["source_preds"], axis=0)
        target_all = np.concatenate(payload["source_targets"], axis=0)
        pooled = m4_query.last_bin_r2(pred_all, target_all)
        mean_session = float(np.mean([row["r2_variance_weighted_last_bin"] for row in source_rows]))
        summaries.append(
            {
                "name": variant["name"],
                "method": variant["method"],
                ...
                "source_in_train": {
                    "n_sessions": len(source_rows),
                    "query_start_trial": QUERY_START,
                    "n_windows": int(sum(row["n_windows"] for row in source_rows)),
                ...
                "later_day": {
                    ...
                    "r2": None,
                    "identity_sha256_by_session": {
                        row["session"]: row["identity_sha256"] for row in later_rows
                    },
                    "carrier_sha256_by_session": {
                        row["session"]: row["carrier_sha256"] for row in later_rows
                    },
                },
    ...
    baseline_pooled = float(baseline["source_in_train"]["pooled_r2"])
    for row in summaries:
        row["source_in_train"]["delta_vs_chrono_k10_pooled"] = (
            float(row["source_in_train"]["pooled_r2"]) - baseline_pooled
        )
        row["formal_benchmark_verdict"] = False

    ranking = sorted(
        summaries,
        key=lambda row: float(row["source_in_train"]["pooled_r2"]),
        reverse=True,
    )
    return {
        "schema": SCHEMA,
        ...
        "ranking": [
            {
                ...
                "mean_session_r2": row["source_in_train"]["mean_session_r2"],
                "pooled_r2": row["source_in_train"]["pooled_r2"],
                "delta_vs_chrono_k10_pooled": row["source_in_train"]["delta_vs_chrono_k10_pooled"],
            }
            for row in ranking
        ],
        "variants": summaries,
        "note": plan.CARRIER_K_NOTE,
        "formal_benchmark_verdict": False,
...
def execute(repo_root: Path) -> tuple[dict[str, str], str | None, str | None]:
    ...
    _require(os.environ.get("CUDA_VISIBLE_DEVICES", None) in {None, ""}, "carrier-k probe is CPU-only")
    receipts.refuse_sealed_roots(repo_root / plan.RESULT_ROOT_RELATIVE)
    parent = repo_root / plan.CARRIER_K_PROBE_ROOT_RELATIVE
    parent.mkdir(parents=True, exist_ok=True)
    progress_state: dict[str, object] = {
        "mode": "carrier_k_probe",
        "arm": ARM,
        "n_variants": plan.CARRIER_K_N_VARIANTS,
    }

    def launch_builder() -> dict[str, object]:
        return {
            "schema": "m1_b3_allsource_carrier_k_probe_launch_v1",
            "arm": ARM,
            "status": "LAUNCHED",
    ...
    def body_publisher(artifact: Any) -> dict[str, str]:
        ...
        return {"carrier_k_probe.json": artifact.publish_json("carrier_k_probe.json", payload)}

    def terminal_builder(shas: Mapping[str, str]) -> dict[str, object]:
        return {
            "schema": "m1_b3_allsource_carrier_k_probe_terminal_v1",
            "status": "COMPLETE",
            "mode": "carrier_k_probe",
            "arm": ARM,
            "pool_trials": POOL,
    ...
    stage = Path(plan.carrier_k_probe_root_relative()).relative_to(plan.RESULT_ROOT_RELATIVE)
    return receipts.run_stage0(
        repo_root / plan.RESULT_ROOT_RELATIVE,
        relative=str(stage),
        attempt_payload={
            "schema": "m1_b3_allsource_carrier_k_probe_attempt_v1",
            "cell": plan.CELL,
            "phase": plan.PHASE,
            "mode": "carrier_k_probe",
            "arm": ARM,
