#!/usr/bin/env python3
"""Run the triggered H1 activity/carrier resampling factorial on GPU0."""
...
from h1_activity_carrier_resampling_factorial_v1.plan import MAIN_TERMINAL_SHA256, SCHEMA
from h1_support_resampled_postpool_v1.plan import EXPECTED_GPU0_UUID


RESULT_ROOT = REPO_ROOT / "tfpd_exploration/h1_series_20260830/results/h1_activity_carrier_resampling_factorial_v1"
MAIN_TERMINAL = REPO_ROOT / "tfpd_exploration/h1_series_20260830/results/h1_support_resampled_postpool_v1/terminal.json"
DESIGN = REPO_ROOT / "tfpd_exploration/h1_series_20260830/docs/DESIGN_H1_ACTIVITY_CARRIER_RESAMPLING_FACTORIAL_V1_20260904.md"
WORKORDER = REPO_ROOT / "tfpd_exploration/h1_series_20260830/docs/WORKORDER_H1_ACTIVITY_CARRIER_RESAMPLING_FACTORIAL_V1_20260904.md"
...
def _preflight() -> dict[str, str]:
    ...
    _need(_sha(MAIN_TERMINAL) == MAIN_TERMINAL_SHA256, "main experiment terminal drift")
    _need(DESIGN.is_file() and WORKORDER.is_file(), "factorial design/workorder missing")
    _need(RESULT_ROOT.parent.is_dir() and not RESULT_ROOT.exists(), "factorial result root unavailable or already exists")
    return {
        "gpu_uuid": gpu,
        "main_terminal_sha256": MAIN_TERMINAL_SHA256,
...
def main() -> int:
    ...
    attempt_sha = _publish(RESULT_ROOT / "attempt.json", {
        ...
        "status": "ATTEMPT_TRIGGERED_GPU0_SOURCE_ONLY_FACTORIAL",
        **authority,
    ...
    try:
        from h1_activity_carrier_resampling_factorial_v1.evaluate import run
