#!/usr/bin/env python3
"""Run H1 post-pool profile-gate source-only training and LODO scoring."""
...
from h1_postpool_profile_gate_v1.plan import (
    DIRECT_TERMINAL_SHA256, EXPECTED_GPU0_UUID, SCALAR_TERMINAL_SHA256, SCHEMA,
)


RESULT_ROOT = REPO_ROOT / "tfpd_exploration/h1_series_20260830/results/h1_postpool_profile_gate_v1"
DIRECT_ROOT = REPO_ROOT / "tfpd_exploration/h1_series_20260830/results/h1_cross_record_anchored_postpool_v1"
SCALAR_ROOT = REPO_ROOT / "tfpd_exploration/h1_series_20260830/results/h1_postpool_output_residual_v1"
DESIGN = REPO_ROOT / "tfpd_exploration/h1_series_20260830/docs/DESIGN_H1_POSTPOOL_PROFILE_GATE_V1_20260903.md"
WORKORDER = REPO_ROOT / "tfpd_exploration/h1_series_20260830/docs/WORKORDER_H1_POSTPOOL_PROFILE_GATE_V1_20260903.md"
...
def _preflight() -> dict[str, str]:
    ...
    _need(_sha(DIRECT_ROOT / "terminal.json") == DIRECT_TERMINAL_SHA256, "direct predecessor drift")
    _need(_sha(SCALAR_ROOT / "terminal.json") == SCALAR_TERMINAL_SHA256, "scalar predecessor drift")
    _need(DESIGN.is_file() and WORKORDER.is_file(), "design/workorder missing")
    _need(not RESULT_ROOT.exists(), "result root already exists; no retry/overwrite")
    return {
        "gpu_uuid": gpu,
        "design_sha256": _sha(DESIGN),
        "workorder_sha256": _sha(WORKORDER),
        "direct_predecessor_terminal_sha256": DIRECT_TERMINAL_SHA256,
        "scalar_predecessor_terminal_sha256": SCALAR_TERMINAL_SHA256,
    }
...
def main() -> int:
    ...
    attempt_sha = _publish(RESULT_ROOT / "attempt.json", {
        "schema": f"{SCHEMA}_attempt", "status": "ATTEMPT_GPU0_SOURCE_ONLY_PROFILE_TRAINING",
        **authority, "formal_heldout_opened": False, "evalai_opened": False, "gpu1_touched": False,
    })
    try:
        from h1_postpool_profile_gate_v1.evaluate import run
        ...
        _publish(RESULT_ROOT / "failure.json", {
            "schema": f"{SCHEMA}_failure", "status": "FAIL_NO_RETRY",
