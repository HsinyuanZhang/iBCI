#!/usr/bin/env python3
"""Run the frozen H1 post-pool scalar output-residual screen."""
...
from h1_postpool_output_residual_v1.plan import (
    EXPECTED_GPU0_UUID,
...
RESULT_ROOT = REPO_ROOT / "tfpd_exploration/h1_series_20260830/results/h1_postpool_output_residual_v1"
PREDECESSOR = REPO_ROOT / "tfpd_exploration/h1_series_20260830/results/h1_cross_record_anchored_postpool_v1"
DESIGN = REPO_ROOT / "tfpd_exploration/h1_series_20260830/docs/DESIGN_H1_POSTPOOL_OUTPUT_RESIDUAL_V1_20260903.md"
WORKORDER = REPO_ROOT / "tfpd_exploration/h1_series_20260830/docs/WORKORDER_H1_POSTPOOL_OUTPUT_RESIDUAL_SCREEN_V1_20260903.md"
...
def _preflight() -> dict[str, str]:
    ...
    _need(_sha(PREDECESSOR / "score.json") == PREDECESSOR_SCORE_SHA256, "predecessor score drift")
    _need(_sha(PREDECESSOR / "terminal.json") == PREDECESSOR_TERMINAL_SHA256, "predecessor terminal drift")
...
def main() -> int:
    ...
    attempt_sha = _publish(RESULT_ROOT / "attempt.json", {
        "schema": f"{SCHEMA}_attempt", "status": "ATTEMPT_SOURCE_ONLY_GPU0_SCREEN",
        **authority, "formal_heldout_opened": False, "evalai_opened": False, "gpu1_touched": False,
    })
    try:
        from h1_postpool_output_residual_v1.evaluate import run
