#!/usr/bin/env python3
"""Route-owned R / S2 sparsification cell runner (workorder 2026-08-18 §4-§6).
...
seed 42) with ONE change: the population-sparsification mask at the exact D
site (after `src = activity + identity`, before `fc_in`), driven by the exact
shared p stream (one p ~ U(0,1) per forward, SHA-recorded; distribution-
matched to sealed D, not bitwise random-path-matched) and gain 1/(1-p) with
no floor and no clamp; p==1 yields the all-zero tensor without division.
Evaluation/scoring forwards run with the mask as exact ones — bitwise equal
to the unsparsified parent path (asserted at launch and in tests).

Integrity block (launch + terminal): num_heads, mask structure, p stream
seed/SHA law, generator namespaces, gain rule, floor/clamp fields (none).
Per-epoch receipt fields: p min/max/quantiles, realized drop/keep
distributions, all-zero population counts, maximum gain, post-mask token
norms, gradient finiteness, loss; for S2 additionally valid/undefined
direction counts and realized angular-sector width statistics per session.
"""
...
def train_epoch_sparsified(model, optimizer, loader, lr_fn, device, p_stream,
...
def aggregate_mask_stats(mask_stats) -> dict:
    if not mask_stats:
        return {"n_forwards": 0}
    kept = [m["kept_fraction"] for m in mask_stats]
    gains = [m["max_gain"] for m in mask_stats if m.get("max_gain") is not None]
    return {
        "n_forwards": len(mask_stats),
        "kept_fraction_mean": float(np.mean(kept)),
        "kept_fraction_min": float(np.min(kept)),
        "kept_fraction_max": float(np.max(kept)),
        "all_zero_population_forwards": int(
            sum(1 for m in mask_stats if m.get("all_zero_population"))
        ),
        "max_gain_observed": float(np.max(gains)) if gains else None,
        "post_mask_token_norm_mean": float(np.mean(
            [m["post_mask_token_norm_mean"] for m in mask_stats
             if "post_mask_token_norm_mean" in m]
        ) or 0.0),
    }
...
def main() -> int:
    ...
    parser.add_argument("--cell", required=True, choices=["R", "S2"])
    parser.add_argument("--device", default="cuda:0")
    parser.add_argument("--output-root", type=Path, default=ROOT / "results/sparsification_v1")
    ...
    try:
        ...
        receipt_mod.write_receipt_transactionally(
            ...
            {
                "schema": "tfpd_sparsification_cell_v1",
                "cell": args.cell,
                "status": "CELL_FAILED",
                "smoke": args.smoke,
                "started_utc": started,
...
def _run(args, out_dir: Path, device, started: str) -> int:
    ...
    config = sparsification.CELLS[args.cell]
    ...
    model = sparsification.build_sparsified_model(seed=args.seed, cell=args.cell)
    ...
    model.eval()
    with torch.no_grad():
        sparsified_eval, _ = model(fx_neural[:4], calib_trials=fx_calib[:4],
    ...
    if not torch.equal(sparsified_eval, parent_eval):
        raise SystemExit("eval-path parity failure: sparsified decode != parent path")
    eval_parity_bitwise = True

    p_stream = sparsification.PStream(seed=sparsification.P_STREAM_SEED)
    masker = (
        sparsification.ElementwiseMasker(seed=sparsification.ELEMENTWISE_MASK_SEED)
        if args.cell == "R"
        else sparsification.SectorMasker(seed=sparsification.SECTOR_SEED)
    )
    model.masker = masker
    ...
    integrity_block = {
        ...
        "p_stream": {
            "law": "one p ~ Uniform(0,1) per training forward, PCG64(42), exact stream shared with the sibling cell",
            "distribution_matched_to_D": True,
            "bitwise_matched_to_D": False,
            "seed": sparsification.P_STREAM_SEED,
        },
        "generator_namespaces": {
            "p_stream": f"numpy PCG64({sparsification.P_STREAM_SEED})",
            "elementwise_mask": f"torch.Generator CPU seeded {sparsification.ELEMENTWISE_MASK_SEED} (cell R only)",
            "sector": f"numpy PCG64({sparsification.SECTOR_SEED}) (cell S2 only)",
        },
        "gain_rule": "1/(1-p), no clamp, no floor; p==1 -> all-zero tensor without division",
        "floor_or_clamp_present": False,
        "min_keep_present": False,
        "eval_mask": "exact ones; forward bitwise equal to unsparsified parent path",
        "theta_authority": {"path": str(THETA_AUTHORITY), "sha256": theta_sha,
                            "authority_sha256": theta_payload["authority_sha256"]},
        "behavior_scaling_convention": "unscaled standardized behavior (exact Arm A/D replica)",
    }

    launch = {
        "schema": "tfpd_sparsification_cell_v1_launch",
    ...
    for epoch in range(epochs):
        ...
        mask_agg = aggregate_mask_stats(model.mask_stats)
        sector_agg = (
            {**masker.summary(),
             "per_session_direction_counts": per_session_valid}
            if args.cell == "S2" else None
        )
        if mask_agg["n_forwards"] != stats["optimizer_steps"]:
            raise SystemExit("mask/forward count drift")
        ...
        if epoch in swa_local:
            ...
            torch.save(
                {
                    "kind": "tfpd_sparsification_ckpt_v1",
                    "cell": args.cell,
                    "epoch": epoch,
        ...
        print(json.dumps({
            ...
            "kept": round(mask_agg.get("kept_fraction_mean") or 0.0, 4),
        }), flush=True)
    ...
    swa_model = sparsification.build_sparsified_model(seed=args.seed, cell=args.cell)
