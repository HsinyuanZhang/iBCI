#!/usr/bin/env python3
"""Public Stage-1 CLI for fold-local EMG-rSyn3. Default is dry.

Live GPU execution requires --execute --gpu-authorized --gpu-index {0,1} --arm.
--pack allows a second same-route arm on an already occupied card.
"""
...
def _dry() -> dict[str, object]:
    ...
    payload["cli"] = "run_m1_emg_rsyn3_fold_local_stage1.py"
    return payload
...
def main(argv: list[str] | None = None) -> int:
    ...
    if arguments.execute_gpu:
        parser.error("Stage-1 public CLI cannot mint a GPU capability")
    ...
    if os.environ.get("PYTHONNOUSERSITE") != "1":
        raise SystemExit("Stage-1 execute requires PYTHONNOUSERSITE=1")
    from tfpd_exploration.src.m1_emg_rsyn3_fold_local_v1.stage1 import execute_arm
