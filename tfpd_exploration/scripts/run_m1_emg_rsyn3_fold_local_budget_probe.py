#!/usr/bin/env python3
"""Public CPU-only budget-sweep token-content probe CLI for fold-local EMG-rSyn3. Default is dry.
...
def _dry() -> dict[str, object]:
    ...
    payload["cli"] = "run_m1_emg_rsyn3_fold_local_budget_probe.py"
    return payload
...
def main(argv: list[str] | None = None) -> int:
    ...
    if arguments.execute_gpu:
        parser.error("budget probe is CPU-only and mints no GPU capability")
    if not arguments.execute:
        print(json.dumps(_dry(), sort_keys=True, separators=(",", ":")))
        return 0
    if os.environ.get("PYTHONNOUSERSITE") != "1":
        raise SystemExit("budget-probe execute requires PYTHONNOUSERSITE=1")
    os.environ["CUDA_VISIBLE_DEVICES"] = ""
    from tfpd_exploration.src.m1_emg_rsyn3_fold_local_v1.budget_probe import execute
