#!/usr/bin/env python3
"""Dry by default; reserve the post-fusion probe attempt or execute the probe.
...
from src.m2_postfusion_probe_v1 import plan  # noqa: E402

OWNED_RELATIVE = plan.OWNED_PATHS
PREDECESSOR_RELATIVE = plan.PREDECESSOR_RELATIVE
TESTS_RELATIVE = "tfpd_exploration/tests/test_m2_postfusion_probe_v1.py"
...
def _write_attempt() -> dict[str, object]:
    ...
    payload = {
        ...
        "cell": "M2_POSTFUSION_PROBE_V1",
        "authority": (
            "operator work order 2026-09-02: post-fusion identity-memory probe "
            "on M2, CPU-only (CUDA_VISIBLE_DEVICES empty, torch threads 4, 0 "
            "workers), inference-only on frozen weights -- does averaging "
            "identities AFTER the fusion MLP beat the pooled readout?"
        ),
        "inference_only": True,
        "pre_registration": {
            "foundation": {
            ...
            "identity_path": dict(plan.IDENTITY_PATH_LAW),
        ...
        "tests": {
            ...
            "scope": "no-data no-CUDA (per-trial identity law, running-mean "
                     "accumulation, anchor logic, gate boundaries at 1e-12)",
        },
...
def main() -> None:
    ...
    if args.stage == "attempt":
        ...
        return
    from src.m2_postfusion_probe_v1 import physical
