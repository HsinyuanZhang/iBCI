#!/usr/bin/env python3
"""Dry by default; reserve the chrono4 strict attempt or execute the cell.
...
from src.m2_chrono4_strict_v1 import plan  # noqa: E402

OWNED_RELATIVE = plan.OWNED_PATHS
PREDECESSOR_RELATIVE = plan.PREDECESSOR_RELATIVE
TESTS_RELATIVE = "tfpd_exploration/tests/test_m2_chrono4_strict_v1.py"
...
def _write_attempt() -> dict[str, object]:
    ...
    payload = {
        ...
        "cell": "M2_CHRONO4_STRICT_V1",
        "authority": (
            "operator work order 2026-09-02: the strict-caliber true-early-start "
            "cell on M2 (chronological-first-4 initialization, no D-opt "
            "selection), CPU-only (CUDA_VISIBLE_DEVICES='', torch threads 4, "
            "0 workers); sealed D-opt anchors are read from receipts and never "
            "rerun"
        ),
        "inference_only": True,
        "pre_registration": {
            "question": (
                "the true early-start deployment number and the D-opt "
                "selection-law contribution over it"
            ),
            "cells": {
        ...
        "tests": {
            ...
            "scope": "no-data no-CUDA (chrono4 selection law, first-4 T4 and "
                     "activity fit, anchor logic, verdict boundaries at 1e-12)",
        },
...
def main() -> None:
    ...
    if args.stage == "attempt":
        ...
        return
    from src.m2_chrono4_strict_v1 import physical
